...
class ShopUrlCore extends ObjectModel
{
	...
	protected static $main_agent_url = [];
    ...
    public static $definition = [
        ...
        'fields'  => [
			...
			'agent_url' => ['type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'size' => 255],
    ...
    public function getFields()
    {
		...
		$this->agent_url = trim($this->agent_url);
    ...
    {
        // @codingStandardsIgnoreStart
        if (!isset(static::$main_domain_ssl[(int) $idShop]) || !isset(static::$main_domain[(int) $idShop])|| !isset(static::$main_agent_url[(int) $idShop])) {
            $row = Db::getInstance()->getRow(
                (new DbQuery())
                    ->select('`domain`, `domain_ssl`, `agent_url`')
                    ->from('shop_url')
                    ->where('`main` = 1')
			...
			static::$main_agent_url[(int) $idShop] = $row['agent_url'];
        }
        // @codingStandardsIgnoreEnd
    }
