...
class ShopCore extends ObjectModel
{
    ...
    public function setUrl()
    {
        ...
        if (!Cache::isStored($cacheId)) {
            $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
                (new DbQuery())
                    ->select('su.physical_uri, su.virtual_uri, su.domain, su.domain_ssl, su.agent_url, t.id_theme, t.name, t.directory')
                    ->from('shop', 's')
    ...
    public function add($autoDate = true, $nullValues = false)
    {
        ...
        // @codingStandardsIgnoreStart
		Configuration::updateValue('EPH_ROUTE_product_rule', array_map(function() {return '{categories:/}{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_category_rule', array_map(function() {return '{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_education_rule', array_map(function() {return '{type:/}{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_education_type_rule', array_map(function() {return '{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_supplier_rule', array_map(function() {return '{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_manufacturer_rule', array_map(function() {return '{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_cms_rule', array_map(function() {return '{categories:/}{rewrite}';}, $langs));
        Configuration::updateValue('EPH_ROUTE_cms_category_rule', array_map(function() {return '{categories:/}{rewrite}';}, $langs));
        // @codingStandardsIgnoreEnd
    ...
    public static function initialize()
    {
        ...
        if (!($idShop = Tools::getValue('id_shop')) || defined('_PS_ADMIN_DIR_')) {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                (new DbQuery())
                    ->select('s.`id_shop`, CONCAT(su.`physical_uri`, su.`virtual_uri`) AS `uri`, su.`domain`, su.`agent_url`, su.`main`')
                    ->from('shop_url', 'su')
                    ->leftJoin('shop', 's', 's.`id_shop` = su.`id_shop`')
                    ->where('su.domain = \''.pSQL($host).'\' OR su.domain_ssl = \''.pSQL($host).'\' OR su.agent_url = \''.pSQL($host).'\'')
                    ->where('s.`active` = 1')
                    ->where('s.`deleted` = 0')
            ...
            if ($through && $idShop && !$isMainUri) {
                foreach ($result as $row) {
                    if ($row['id_shop'] == $idShop && $row['main']) {
						...
						if($_SERVER['SERVER_NAME'] == $this->agent_url) {
							$url = str_replace('//', '/', $row['agent_url'].$row['uri'].$requestUri);
						} else {
    ...
    public static function cacheShops($refresh = false)
    {
        ...
        $sql = (new DbQuery())
            ...
            ->select('su.`domain`, su.`domain_ssl`, su.`agent_url`, su.`physical_uri`, su.`virtual_uri`')
            ->from('shop_group', 'gs')
     ...
     * @param int  $idShopGroup
     *
     * @return PhenyxShopCollection Collection of Shop
     *
     * @since 1.9.1.0
     * @version 1.8.1.0 Initial version
     * @throws PhenyxShopException
    ...
    {
        $shops = new PhenyxShopCollection('Shop');
        if ($active) {
    ...
    public static function getSharedShops($shopId, $type)
    {
        if (!in_array($type, [self::SHARE_CUSTOMER, self::SHARE_ORDER, self::SHARE_STOCK])) {
            throw new PhenyxShopException('Wrong argument ($type) in Shop::getSharedShops() method');
        }
