        ...
        foreach ($optionList as $category => &$categoryData) {
            ...
            foreach ($categoryData['fields'] as $key => &$field) {
                ...
                if ($field['type'] === 'textarea' || $field['type'] === 'textareaLang') {
                    ...
                    $this->tpl->assign(
                        [
                            'iso'      => file_exists(_PS_ADMIN_DIR_ . '/js/tiny_mce/langs/' . $iso . '.js') ? $iso : 'en',
                ...
                if ($field['type'] === 'code') {
                    $this->context->controller->addJS(_PS_JS_ADMIN_DIR_ . 'ace/ace.js');
                    $this->context->controller->addJS(_PS_JS_ADMIN_DIR_ . 'ace/ext-language_tools.js');
                    $this->context->controller->addJS(_PS_JS_ADMIN_DIR_ . 'ace/snippets/' . $field['mode'] . '.js');
                    $this->context->controller->addCSS(_PS_JS_ADMIN_DIR_ . 'ace/aceinput.css');
                }
