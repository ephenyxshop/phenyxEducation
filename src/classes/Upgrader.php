...
class UpgraderCore {
	...
	public static function createEducation($educationToPush, $imageToPush, $attributeToUpdates = []) {

		$education = new Education();

		foreach ($educationToPush as $key => $value) {
			...
			if (array_key_exists('lang', $params) && $params['lang']) {
				...
				if (property_exists($education, $field)) {
					...
					foreach (Language::getIDs(false) as $idLang) {
						...
						if (!isset($educationToPush[$field][(int) $idLang]) || !is_array($educationToPush[$field])) {
							$education->{$field}
							= [];
						}

						$education->{$field}
						[(int) $idLang] = $educationToPush[$field][(int) $idLang];
					}
		...
		$education->id_education_type = EducationType::getIdEducationTypeByRef($education->reference_type);
			...
			foreach ($imageToPush as $key => $value) {
				...
				if ($value['cover'] == 1) {
					...
					if (file_exists(_PS_UPLOAD_DIR_ . $key)) {
						...
						ImageEducation::saveNewImage($education, $savePath, $value);
					}
						...
						if (array_key_exists('lang', $params) && $params['lang']) {
							...
							if (property_exists($combination, $field)) {
								...
								foreach (Language::getIDs(false) as $idLang) {
									...
									$combination->{$field}
									[(int) $idLang] = $declinaison[$field][(int) $idLang];
								}
