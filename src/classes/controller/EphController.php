...
abstract class EphControllerCore {
	...
	abstract public function generateParaGridScript();
	...
	abstract public function setAjaxMedia();
	...
	abstract public function ajaxProcessOpenTargetController();
    ...
    public function removeCSS($cssUri, $cssMediaType = 'all', $checkPath = true) {
        ...
        foreach ($cssUri as $cssFile => $media) {
            ...
            if (is_string($cssFile) && strlen($cssFile) > 1) {
                ...
                if ($checkPath) {
                    $cssPath = MediaAdmin::getCSSPath($cssFile, $media);
                } else {
                ...
                if ($checkPath) {
                    $cssPath = MediaAdmin::getCSSPath($media, $cssMediaType);
                } else {
    ...
    public function removeJS($jsUri, $checkPath = true) {
        ...
        if (is_array($jsUri)) {
            ...
            foreach ($jsUri as $jsFile) {
                ...
                if ($checkPath) {
                    $jsPath = MediaAdmin::getJSPath($jsFile);
                }
            ...
            if ($checkPath) {
                $jsPath = MediaAdmin::getJSPath($jsUri);
            }
    ...
    public function addJquery($version = null, $folder = null, $minifier = true) {
        ...
        $this->addJS(MediaAdmin::getJqueryPath($version, $folder, $minifier), false);
    }
    ...
    public function addJqueryUI($component, $theme = 'base', $checkDependencies = true) {
        ...
        foreach ($component as $ui) {
            $uiPath = MediaAdmin::getJqueryUIPath($ui, $theme, $checkDependencies);
    ...
    public function addJqueryPlugin($name, $folder = null, $css = true) {
        ...
        if (is_array($name)) {
            ...
            foreach ($name as $plugin) {
                $pluginPath = MediaAdmin::getJqueryPluginPath($plugin, $folder);
