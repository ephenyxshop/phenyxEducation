            ...
            if (($hookCallable || $hookRetroCallable) && Module::preCall($moduleInstance->name)) {
                ...
                if (!$arrayReturn && $array['live_edit'] && Tools::isSubmit('live_edit') && Tools::getValue('ad')
                    && Tools::getValue('liveToken') == Tools::getAdminToken(
                        ...
                        . (int) EmployeeMenu::getIdFromClassName('AdminModulesPositions') . (int) Tools::getValue('id_employee')
                    )
                ) {
                    $liveEdit = true;
...
// Return html string
    ...
    public static function getHookModuleExecList($hookName = null) {
        ...
        $context = Context::getContext();
        $cacheId = 'hook_module_exec_list_' . (isset($context->shop->id) ? '_' . $context->shop->id : '') . ((isset($context->student)) ? '_' . $context->student->id : '');
     ...
     * @return string
     */
    public static function newOrder($cart, $order, $student, $currency, $orderStatus) {
