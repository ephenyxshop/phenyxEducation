...
class ProductCore extends ObjectModel {
    ...
    public $professional_price;
    public $isBtoC;
    public $public_price;
    public $groups_allowed;
    public $flashsale;
    public $on_sold;
	
	public $id_product_account;
    ...
    public static $definition = [
        ...
        'fields'         => [
			...
			'quantity'                  => ['type' => self::TYPE_INT, 'validate' => 'isUnsignedId'],
            ...
            'groups_allowed'            => ['type' => self::TYPE_STRING, 'shop' => true, 'size' => 128],
            'flashsale'                 => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'on_sold'                   => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
    ...
    public function __construct($idProduct = null, $full = false, $idLang = null, $idShop = null, Context $context = null) {
		...
		if($this->id) {
			$this->id_product_account = ProductAccount::getIdProductAccount($this->id);
			$this->sell_account_local = ProductAccount::getProductAccount($this->id, 'sell_account_local');
			$this->sell_account_cee = ProductAccount::getProductAccount($this->id, 'sell_account_cee');
			$this->sell_account_export = ProductAccount::getProductAccount($this->id, 'sell_account_export');
			$this->sell_account_notax = ProductAccount::getProductAccount($this->id, 'sell_account_notax');
			$this->purchase_account_local = ProductAccount::getProductAccount($this->id, 'purchase_account_local');
			$this->purchase_account_cee = ProductAccount::getProductAccount($this->id, 'purchase_account_cee');
			$this->purchase_account_import = ProductAccount::getProductAccount($this->id, 'purchase_account_import');
			$this->purchase_account_notax = ProductAccount::getProductAccount($this->id, 'purchase_account_notax');
			
			
		} else {
			
			$this->sell_account_local = ProductAccount::getDefaultProductAccount('sell_account_local');
			$this->sell_account_cee = ProductAccount::getDefaultProductAccount('sell_account_cee');
			$this->sell_account_export = ProductAccount::getDefaultProductAccount('sell_account_export');
			$this->sell_account_notax = ProductAccount::getDefaultProductAccount('sell_account_notax');
			$this->purchase_account_local = ProductAccount::getDefaultProductAccount('purchase_account_local');
			$this->purchase_account_cee = ProductAccount::getDefaultProductAccount('purchase_account_cee');
			$this->purchase_account_import = ProductAccount::getDefaultProductAccount('purchase_account_import');
			$this->purchase_account_notax = ProductAccount::getDefaultProductAccount('purchase_account_notax');
		}
        ...
        if ($full && $this->id) {
            ...
            $this->professional_price = Product::getProfessionalPrice((int) $this->id, 5);
            $this->isBtoC = Product::getProfessionalPrice((int) $this->id, 5, 1);

            if ($this->isBtoC) {
                $this->public_price = Product::getPubliclPrice((int) $this->id, $this->tax_rate);
            }

            $this->groups_allowed = unserialize($this->groups_allowed);
    ...
    public static function getPriceStatic(
        ...
        if (!is_object($curCart) || (Validate::isUnsignedInt($idCart) && $idCart && $curCart->id != $idCart)) {
            /*
                                            * When a user (e.g., guest, customer, Google...) is on PhenyxShop, he has already its cart as the global (see /init.php)
                                            * When a non-user calls directly this method (e.g., payment module...) is on PhenyxShop, he does not have already it BUT knows the cart ID
    ...
    public static function priceCalculation(
        ...
        $taxManager = TaxManagerFactory::getManager($address, Product::getIdTaxRulesGroupByIdProduct((int) $idProduct, $context));
            ...
            if ($specificPrice['reduction_type'] == 'amount') {
                ...
                if (!$useTax && $specificPrice['reduction_tax']) {
                    ...
                    if (!$productTaxCalculator->getTotalRate()) {
                        $tax = new Tax(Configuration::get('EPH_DEFAULT_SPECIFIC_PRICE_RULE_TAX'));
    ...
    public static function isSold($id_product) {

        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'specific_price` WHERE `to` > CURDATE() AND `on_sold` = 1 AND `id_product` = ' . (int) $id_product;

        $requete = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
    ...
    public static function getTaxesInformations($row, Context $context = null) {
        ...
        $taxManager = TaxManagerFactory::getManager($address, Product::getIdTaxRulesGroupByIdProduct((int) $row['id_product'], $context));
    ...
    public static function getNewProducts($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = null, $orderWay = null, Context $context = null) {
        ...
        foreach ($result as $item) {
            $groups_allowed = unserialize($item['groups_allowed']);

            if (in_array($id_group, $groups_allowed)) {
        ...
        return Product::getProductsProperties((int) $idLang, $products);
    }
    ...
    public static function getProductsProperties($idLang, $queryResult) {
        ...
        if (is_array($queryResult)) {
            ...
            foreach ($queryResult as $row) {
                ...
                if ($row2 = Product::getProductProperties($idLang, $row)) {
                    $resultsArray[] = $row2;
                }
    ...
    public static function getProductProperties($idLang, $row, Context $context = null) {
        ...
        if (Combination::isFeatureActive() && $idProductAttribute === null
            && ((isset($row['cache_default_attribute']) && ($ipaDefault = $row['cache_default_attribute']) !== null)
                || ($ipaDefault = Product::getDefaultAttribute($row['id_product'], !$row['allow_oosp'])))
        ) {
        ...
        if ($idProductAttribute) {
            $row['attribute_price'] = (float) Product::getProductAttributePrice($idProductAttribute);
        }
        ...
        $row['id_image'] = Product::defineProductImage($row, $idLang);
        $row['features'] = Product::getFrontFeaturesStatic((int) $idLang, $row['id_product']);

        $row['attachments'] = [];

        if (!isset($row['cache_has_attachments']) || $row['cache_has_attachments']) {
            $row['attachments'] = Product::getAttachmentsStatic((int) $idLang, $row['id_product']);
        }
    ...
    public static function getRandomSpecial($idLang, $beginning = false, $ending = false, Context $context = null) {
        ...
        $productReductions = Product::_getProductIdByDate((!$beginning ? $currentDate : $beginning), (!$ending ? $currentDate : $ending), $context, true);
        ...
        if ($productReductions) {
            ...
            return Product::getProductProperties($idLang, $row);
        } else {
            return false;
        }
    ...
    protected static function _getProductIdByDate($beginning, $ending, Context $context = null, $withCombination = false) {
        ...
        $idCountry = isset($ids['id_country']) ? (int) $ids['id_country'] : (int) Configuration::get('PS_COUNTRY_DEFAULT');
    ...
    public static function getPricesDrop(
        ...
        $currentDate = date('Y-m-d H:i:00');
        $idsProduct = Product::_getProductIdByDate((!$beginning ? $currentDate : $beginning), (!$ending ? $currentDate : $ending), $context);
    ...
    public static function duplicateAttachments($idProductOld, $idProductNew) {
        ...
        Product::updateCacheAttachment((int) $idProductNew);
    ...
    public static function duplicateCustomizationFields($oldProductId, $productId) {
        ...
        if (($customizations = Product::_getCustomizationFieldsNLabels($oldProductId)) === false) {
            return false;
        }
    ...
    public static function getRealQuantity($idProduct, $idProductAttribute = 0, $idWarehouse = 0, $idShop = null) {
        ...
        if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && Product::usesAdvancedStockManagement($idProduct) &&
    ...
    public function deleteProductAttributes() {
        ...
        $result = true;
        $combinations = new PhenyxShopCollection('Combination');
    ...
    public function deleteDownload() {
        ...
        $result = true;
        $collectionDownload = new PhenyxShopCollection('ProductDownload');
     ...
     *                                   do something with it in ephenyx 1.1, maybe
    ...
    public function addAttribute(
        ...
        $idDefaultAttribute = Product::updateDefaultAttribute($this->id);
    ...
    public static function updateDefaultAttribute($idProduct) {
        ...
        $idDefaultAttribute = (int) Product::getDefaultAttribute($idProduct, 0, true);
        ...
        foreach ($attributes as &$attribute) {
            ...
            if ($setDefault) {
                ...
                if (!$this->hasAttributesInOtherShops()) {
                    $idShopListArray = Product::getShopsByProduct($this->id);
                    $idShopList = [];
    ...
    public function updateAttribute(
        ...
        $idDefaultAttribute = (int) Product::updateDefaultAttribute($this->id);
    ...
    public function getPrice(
        ...
        return Product::getPriceStatic((int) $this->id, $tax, $idProductAttribute, $decimals, $divisor, $onlyReduc, $usereduc, $quantity);
    }
    ...
    */
    ...
    public function getPublicPrice(
        ...
        return Product::getPriceStatic((int) $this->id, $tax, $idProductAttribute, $decimals, $divisor, $onlyReduc, $usereduc, $quantity, false, null, null, null, $specificPriceOutput, true, true, null, false);
    }
    ...
    public function getFrontFeatures($idLang) {
        ...
        return Product::getFrontFeaturesStatic($idLang, $this->id);
    }
    ...
    public function getAttachments($idLang) {
        ...
        return Product::getAttachmentsStatic($idLang, $this->id);
    }
    ...
    protected function _deleteOldLabels() {
        ...
        if ($extraFile > 0 && count($customizationFields[Product::CUSTOMIZE_FILE]) - $extraFile >= 0 &&
            ...
            AND `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field` >= ' . (int) $customizationFields[Product::CUSTOMIZE_FILE][count($customizationFields[Product::CUSTOMIZE_FILE]) - $extraFile]
            ))
        ) {
            return false;
        }
        ...
        if ($extraText > 0 && count($customizationFields[Product::CUSTOMIZE_TEXTFIELD]) - $extraText >= 0 &&
            ...
            AND `' . _DB_PREFIX_ . 'customization_field_lang`.`id_customization_field` = `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field`
            AND `' . _DB_PREFIX_ . 'customization_field`.`id_customization_field` >= ' . (int) $customizationFields[Product::CUSTOMIZE_TEXTFIELD][count($customizationFields[Product::CUSTOMIZE_TEXTFIELD]) - $extraText]
            ))
        ) {
            return false;
        }
    ...
    public function getAttributeCombinations($idLang) {
        ...
        $sql = 'SELECT pa.*, product_attribute_shop.*, ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, al.`name` AS attribute_name, pal.`name`, pal.`description`, pal.`description_short`,
                    a.`id_attribute`
                FROM `' . _DB_PREFIX_ . 'product_attribute` pa
                ' . Shop::addSqlAssociation('product_attribute', 'pa') . '
                LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_lang` pal ON pal.`id_product_attribute` = pa.`id_product_attribute`
                LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute` a ON a.`id_attribute` = pac.`id_attribute`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = ' . (int) $idLang . ')
    ...
    */
    ...
    public function modifierWsLinkRewrite() {
        ...
        foreach ($this->name as $idLang => $name) {
            ...
            if (!Validate::isLinkRewrite($this->link_rewrite[$idLang])) {
    ...
    public static function getProductsImageLink() {
        ...
        $images = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
                ->select('a.`id_product`, a.`id_shop_default`, b.`link_rewrite`, image_shop.`id_image` AS `Image`')
                ->from('product', 'a')
                ->leftJoin('product_lang', 'b', 'b.`id_product` = a.`id_product` AND b.`id_lang` = ' . (int) $context->language->id . ' AND b.`id_shop` = ' . (int) $context->shop->id)
        ...
        foreach ($images as $image) {
            ...
            if ($image['Image'] > 0) {
                $imageLinks[$image['id_product']] = '<img src="' . $context->link->getImageLink($image['link_rewrite'], $image['Image'], 'cart_default') . '" class="imgm img-thumbnail" style="width:50px">';
            } else {
                $uriPath = '../img/p/' . $context->language->iso_code . '-default-cart_default.jpg';
                $imageLinks[$image['id_product']] = '<img src="' . $uriPath . '" class="imgm img-thumbnail" style="width:50px">';
            }
    ...
    public static function getProfessionalPrice($id_product, $id_group, $checkBtoC = 0) {

        $context = Context::getContext();
        //$id_group = 4;

        $hasAttributes = Product::hasAttribute($id_product);
        ...
        if ($hasAttributes) {
            ...
            $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('  SELECT price
                    FROM `' . _DB_PREFIX_ . 'specific_price`
                    WHERE
                    `id_shop` =' . $context->shop->id . ' AND
            ...
            $price = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT price FROM `' . _DB_PREFIX_ . 'product_shop` WHERE id_product =' . (int) $id_product);
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('   SELECT price
                    FROM `' . _DB_PREFIX_ . 'specific_price`
                    WHERE
                    `id_shop` =' . $context->shop->id . ' AND
                    `id_group` =' . $id_group . ' AND
    ...
    public static function getPubliclPrice($id_product, $tax_rate) {
    ...
    public static function IsExistingProduct($article) {

        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
            SELECT `id_product`
            FROM `' . _DB_PREFIX_ . 'product`
            WHERE `reference` = \'' . pSQL($article) . '\'');
    ...
    public function IsNonAttributreProduct($article) {
    ...
    public static function hasAttribute($id_product) {
    ...
    public static function IsAttribute($article) {
    ...
    public function IsProductShop($article, $id_shop) {

        $shop = Db::getInstance(_PS_USE_SQL_SLAVE_) - ExecuteS('
            SELECT `id_shop`
            FROM `' . _DB_PREFIX_ . 'product_shop`
            WHERE `id_shop` = ' . $id_shop . ' AND `id_product` = ' . pSQL($result['id_product']));

    }

    public static function AddCielProduct($article, $id_shop_default, $majdc, $majcategory = 1) {

        if ((int) $id_product = Product::IsExistingProduct($article->fldCode)) {

            if (!Product::UpdateCielProduct($id_product, $article, $id_shop_default, $majdc, $majcategory)) {
                return false;
            } else {
                return true;
            }
        ...
        $product = new Product();
        $product->name = [$langId => $article->fldDesignation];
        $product->description_short = [$langId => Trim($article->fldDescription)];
        $product->id_shop_default = $id_shop_default;
        ...
        foreach ($groups as $group) {
            array_push($product_group, $group['id_group']);
        }

        $product->groups_allowed = $product_group;

        if (!empty($article->fldCategory)) {
            $category = explode("-", $article->fldCategory);
            $id_category_default = $category[0];
            $product->id_category_default = $id_category_default;
        } else {
            $product->id_category_default = 40;
        }

        if (!empty($article->fldMarque)) {
            $id_supplier = Db::getInstance()->getValue('
                        SELECT `id_supplier`
                        FROM `' . _DB_PREFIX_ . 'supplier`
                        WHERE `name` = \'' . pSQL($article->fldMarque) . '\'');
        ...
        $product->link_rewrite = [$langId => Tools::link_rewrite(trim($article->fldDesignation))];

        $product->id_tax_rules_group = $id_tax;

        if ($article->fldAttribute == 'Oui') {
            $id_product = Product::IsExistingProduct($article->fldRefDeclinaison);

            if (!$id_product) {
                $product->reference = $article->fldRefDeclinaison;
                $product->price = 0;

                if (!$product->add()) {
                    return false;
                } else {
                    $position = Product::getCategoryPosition((int) $product->id_category_default);
                    Db::getInstance()->execute('
                        INSERT INTO `' . _DB_PREFIX_ . 'category_product` ( `id_category`,`id_product`, `position`)
                        VALUES (' . (int) $product->id_category_default . ', ' . (int) $product->id . ', ' . (int) $position . ')
                    ');

                    if (Product::generateDeclinaison($product->id, $article, $id_shop_default)) {
                        return true;
                    }
                ...
                if (Product::generateDeclinaison($id_product, $article, $id_shop_default)) {
                    return true;
                }
            ...
            $product->reference = $article->fldCode;
            $product->supplier_reference = $article->fldSuplyerRef;

            if (!empty($article->fldupc)) {
                $product->upc = $article->fldupc;
            }

            $product->weight = (float) Trim($article->fldWeight);
            $product->wholesale_price = Trim($article->fldPAHT);
            $product->price = Trim($article->fldPVHT);
            $product->quantity = intval($article->fldStock);

            if (isset($product->packDetail) && $product->packDetail) {
            ...
            if ($product->add()) {
                ...
                if (isset($article->tarifDetail) && $article->tarifDetail) {

                    foreach ($article->tarifDetail as $tarifDetail) {
                        Product::generateSpecificPrice($article, $product->id, $tarifDetail, $langId, $id_shop_default);
                    }

                }

                if (isset($article->packDetail) && $article->packDetail) {
                    Product::generatePack($product, $article);
    ...
    public static function UpdateCielProduct($id_product, $article, $id_shop, $majdc = 0, $majcategory = 1) {
        ...
        if ($article->fldAttribute == 'Oui') {
            ...
            $id_product_attribute = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
                SELECT `id_product_attribute`
                FROM `' . _DB_PREFIX_ . 'product_attribute`
                WHERE `reference` = \'' . pSQL($article->fldCode) . '\'');

            if ($id_product_attribute) {
                fwrite($file, "exist ");

                if (!Product::updateDeclinaison($id_product, $id_product_attribute, $article, $id_shop)) {
                    return false;
                }

            } else {

                if (!Product::generateDeclinaison($id_product, $article, $id_shop)) {
                    return false;
                }
            ...
            $Isshop = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
                SELECT `id_shop`
                FROM `' . _DB_PREFIX_ . 'product_shop`
            ...
            if ($Isshop) {
                ...
                $product->price = $article->fldPVHT;
                $product->wholesale_price = $article->fldPAHT;
                $product->supplier_reference = $article->fldSuplyerRef;
                $product->weight = (float) (Trim($article->fldWeight));
                $product->quantity = $article->fldStock;
                $product->upc = $article->fldupc;
                $price_item = $article->fldPVHT;

                if ($majdc == 1) {
                    $product->name = [$langId => $article->fldDesignation];
                    $product->description_short = [$langId => Trim($article->fldDescription)];
                    $product->link_rewrite = [$langId => Tools::link_rewrite($product->name)];
                }

                if ($majcategory == 1) {

                    if (!empty($article->fldCategory)) {
                        $old_category = $product->id_category_default;
                        $category = explode("-", $article->fldCategory);
                        $id_category_default = $category[0];
                        $product->id_category_default = $id_category_default;
                    }
                ...
                if (!empty($article->fldMarque)) {
                    ...
                    if ($id_supplier) {
                        $product->id_manufacturer = $id_supplier;
                    }

                }

                //$product->link_rewrite = array($langId => Tools::link_rewrite($product->name));
                ...
                if ($majcategory == 1 && $product->id_category_default != $old_category) {
                    $position = Product::getCategoryPosition((int) $product->id_category_default);

                    //Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'category_product` WHERE `id_product` = '.(int)$product->id.' AND `id_category` = '.(int)$old_category);
                ...
                if (isset($article->tarifDetail) && $article->tarifDetail) {
                    ...
                    foreach ($article->tarifDetail as $tarifDetail) {
                        ...
                        if (!Product::generateSpecificPrice($article, $id_product, $tarifDetail, $langId, $id_shop, 1)) {
                            echo 'bug';
                            return false;
                        }
                    ...
                    Db::getInstance()->execute('
                        DELETE FROM `' . _DB_PREFIX_ . 'specific_price` WHERE `id_product` = ' . (int) $product->id);
                }

                if (isset($article->packDetail) && $article->packDetail) {

                    if (!Product::generatePack($id_product, $article)) {
                        return false;
                    }
    ...
    public static function generateDeclinaison($id_product, $article, $id_shop) {

        $id_attribute = Db::getInstance()->getValue('
                    SELECT `id_attribute`
                    FROM `' . _DB_PREFIX_ . 'attribute_lang`
                    WHERE `id_lang`= ' . (int) (Configuration::get('PS_LANG_DEFAULT')) . ' AND `name` = \'' . Trim($article->fldCombination) . '\'');

        if ($id_attribute) {

            $combination = new Combination();
            $combination->id_product = $id_product;
            $combination->reference = $article->fldCode;
            $combination->supplier_reference = $article->fldSuplyerRef;

            if (!empty($article->fldupc)) {
                $combination->upc = $article->fldupc;
            }

            $combination->weight = (float) Trim($article->fldWeight);
            $combination->wholesale_price = Trim($article->fldPAHT);
            $combination->price = Trim($article->fldPVHT);
            $combination->quantity = intval($article->fldStock);

            if ($combination->add()) {
                $result = Db::getInstance()->execute('
                    INSERT INTO `' . _DB_PREFIX_ . 'product_attribute_combination` (`id_attribute`, `id_product_attribute`) VALUES (' . $id_attribute . ', ' . $combination->id . ')');
            ...
            if (isset($article->tarifDetail) && $article->tarifDetail) {
                ...
                foreach ($article->tarifDetail as $tarifDetail) {
                    Product::generateSpecificPrice($article, $id_product, $tarifDetail, $langId, $id_shop, $id_product_attribute);
                }
    ...
    public static function updateDeclinaison($id_product, $id_product_attribute, $article, $id_shop) {
        ...
        if (isset($article->tarifDetail) && $article->tarifDetail) {
            ...
            foreach ($article->tarifDetail as $tarifDetail) {
                Product::generateSpecificPrice($article, $id_product, $tarifDetail, $langId, $id_shop, 1, $id_product_attribute);
            }
    ...
    public static function AddSpecificPrice($id_product, $price, $grille, $type, $group, $id_shop = 0, $id_product_attribute = 0, $coef = 0) {
        ...
        $specificPrice->id_shop_group = $id_shop;
        $specificPrice->id_currency = 0;
        $specificPrice->id_country = 0;
        $specificPrice->id_group = $group;
        $specificPrice->id_customer = 0;
        $specificPrice->price = $price;
        $specificPrice->from_quantity = (int) ($grille);
        $specificPrice->reduction = $coef;
        $specificPrice->reduction_type = $type;
    ...
    public static function generatePack($id_product, $article) {
        ...
        foreach ($article->packDetail as $packDetail) {

            if ($result = Product::IsAttribute($packDetail->fldComposant)) {
                $id_item = $result['id_product'];
                $id_product_attribute = $result['id_product_attribute'];
            } else {
                $id_item = Product::IsExistingProduct($packDetail->fldComposant);
                $id_product_attribute = 0;
            }

            if (isset($id_item) && $id_item) {
                Pack::addItem($id_product, $id_item, $packDetail->composantQty, $id_product_attribute);
            }
    ...
    public static function generateSpecificPrice($article, $id_product, $tarifDetail, $langId, $id_shop, $clean = 0, $id_product_attribute = 0) {

        $from_quantity = (int) $tarifDetail->fldValeur;
        $type = 'amount';
        $prix = str_replace(',', '.', str_replace(' ', '', $tarifDetail->fldTarifPro));

        $id_group = Db::getInstance()->getValue('
            SELECT `id_group`
            FROM `' . _DB_PREFIX_ . 'group_lang`
            WHERE `id_lang`= ' . $langId . ' AND `name` = \'' . Trim($tarifDetail->fldPro) . '\'');

        if ($clean == 1 || $prix == 0) {

            if (!Product::cleanSpecificPrice($id_product, $from_quantity, $id_group, $id_shop, $id_product_attribute)) {
                return false;
            }

        }

        if ($prix != 0 || $prix != $article->fldPVHT) {

            if (!Product::AddSpecificPrice($id_product, $prix, $from_quantity, $type, $id_group, $id_shop, $id_product_attribute)) {
                return false;
            }

        }

        $prix = str_replace(',', '.', str_replace(' ', '', $tarifDetail->fldTarifDistri));

        $id_group = Db::getInstance()->getValue('
            SELECT `id_group`
            FROM `' . _DB_PREFIX_ . 'group_lang`
            WHERE `id_lang`= ' . $langId . ' AND `name` = \'' . $tarifDetail->fldDistri . '\'');
        ...
        $prix = str_replace(',', '.', str_replace(' ', '', $tarifDetail->fldTarifDistriVIP));

        $id_group = Db::getInstance()->getValue('
            SELECT `id_group`
            FROM `' . _DB_PREFIX_ . 'group_lang`
            WHERE `id_lang`= ' . $langId . ' AND `name` = \'' . $tarifDetail->fldDistriVIP . '\'');
    ...
    public static function cleanSpecificPrice($id_product, $from_quantity, $id_group, $id_shop, $id_product_attribute = 0) {
        ...
        $sql .= ' AND `from_quantity`=' . $from_quantity . ' AND `id_group`= ' . $id_group . ' AND `id_shop`= ' . $id_shop;
    ...
    public static function getCategoryPosition($id_category_default) {
    ...
    public static $_aqd_cart_quantity = [];

    public static function getAllPackedAdvDatas($id_cart, $id_lang = null, $only_in_cart = true) {
        ...
        if (!$AdvPacked_datas = Db::getInstance()->executeS('
            SELECT * FROM `' . _DB_PREFIX_ . 'pack_adv_cart`
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            if (!$name = Db::getInstance()->getValue(
                'SELECT `name`
            FROM `' . _DB_PREFIX_ . 'product_lang`
            WHERE `id_product` = ' . (int) $result[$i]['id_product'])) {
                return false;
            }
        ...
        return $AdvPacked_datas;
    }

    public static function getGestProducts($id_lang, $id_shop, $id_category, $val, $table, $total) {

        $sql = 'SELECT p.*, pl.*, tx.`rate`
                    FROM `' . _DB_PREFIX_ . 'product` p
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl  ON pl.`id_product` = p.`id_product`
                    LEFT JOIN `' . _DB_PREFIX_ . 'tax` tx ON tx.`id_tax` =  p.`id_tax_rules_group` ';

        $sql .= '   WHERE pl.`id_shop` = ' . $id_shop . ' AND pl.`id_lang` = ' . $id_lang . ' ';

        if ($id_category) {
            $sql .= 'AND p.`id_category_default` = ' . $id_category;
        }

        $products_ini = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $articles = [];

        foreach ($products_ini as $product) {

            $product['tax_incl'] = $product['price'] * $product['rate'] / 100;
            $product['price_tax_incl'] = $product['price'] + $product['tax_incl'];
            $sql = 'SELECT pa.*, agl.`public_name`, al.`name`
                    FROM `' . _DB_PREFIX_ . 'product_attribute` pa
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
                    LEFT JOIN `' . _DB_PREFIX_ . 'attribute` a ON a.`id_attribute` = pac.`id_attribute`
            ...
            if ($attributes) {
                ...
                foreach ($attributes as $attribute) {
                    $attribute['tax_incl'] = $attribute['price'] * $product['rate'] / 100;
                    $attribute['price_tax_incl'] = $attribute['price'] + $attribute['tax_incl'];
                    array_push($sub_articles, $attribute);
                }

                $product['has_attribute'] = 1;
                $product['attributes'] = $sub_articles;
            }
    ...
    public static function getCategoryDefault($id_product) {
    ...
    public static function getGestProduct($id_product, $id_lang, $id_shop) {

        $sql = 'SELECT p.*, pl.*, tx.`rate`, i.*
                    FROM `' . _DB_PREFIX_ . 'product` p
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl  ON pl.`id_product` = p.`id_product`
                    LEFT JOIN `' . _DB_PREFIX_ . 'tax` tx ON tx.`id_tax` =  p.`id_tax_rules_group`
        ...
        $product['tax_incl'] = $product['price'] * $product['rate'] / 100;
        $product['price_tax_incl'] = $product['price'] + $product['tax_incl'];
        $groups = Group::getCustomGroups($id_lang);
        $tarif_groups = [];
        $nb_specific = [];
        $sql = 'SELECT *
                FROM `' . _DB_PREFIX_ . 'product_attribute`
                WHERE `id_product`= ' . $product['id_product'];
        $product_attributes = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $attributes = [];

        foreach ($product_attributes as $attribute) {
            $sql = 'SELECT agl.`public_name`, al.`name`, i.`id_image`
                    FROM `' . _DB_PREFIX_ . 'product_attribute` pa
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
                    LEFT JOIN `' . _DB_PREFIX_ . 'attribute` a ON a.`id_attribute` = pac.`id_attribute`
                    LEFT JOIN `' . _DB_PREFIX_ . 'attribute_group_lang` agl ON (agl.`id_attribute_group` = a.`id_attribute_group` AND agl.`id_lang` = ' . $id_lang . ')
                    LEFT JOIN `' . _DB_PREFIX_ . 'attribute_lang` al ON al.`id_attribute` = pac.`id_attribute`
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_image` i ON i.`id_product_attribute` = pa.`id_product_attribute`
                    WHERE pa.`id_product_attribute` = ' . $attribute['id_product_attribute'];

            $attribute['attributes'] = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        ...
        if ($attributes) {
            ...
            foreach ($attributes as $attribute) {
                ...
                foreach ($groups as $group) {
                   ...
                   WHERE `id_product` = ' . $product['id_product'] . '
                   AND `id_product_attribute` = ' . $attribute['id_product_attribute'] . '
                   AND `id_group` = ' . $group['id_group'];

                    $specific_prices = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
                    $group['nb_specific'] = count($specific_prices);

                    if ($specific_prices && $specific_prices) {

                        for ($i = 0; $i < count($specific_prices); $i++) {

                            if ($specific_prices[$i]['reduction_type'] == 'percentage') {
                                $specific_prices[$i]['price'] = $attribute['price'] * (1 - $specific_prices[$i]['reduction']);
                            }

                            $group['specific_price'] = $specific_prices[$i];
                        }

                    } else {
                        $group['specific_price']['price'] = $attribute['price'];
                    }

                    array_push($tarif_groups, $group);
                    array_push($nb_specific, count($specific_prices));
                }

                $attribute['tarif_group'] = $tarif_groups;
                array_push($sub_articles, $attribute);
            ...
            foreach ($groups as $group) {
                ...
                for ($i = 0; $i < count($specific_prices); $i++) {
                    ...
                    if ($specific_prices[$i] && $specific_prices[$i]) {

                        if ($specific_prices[$i]['reduction_type'] == 'percentage') {
                            $specific_prices[$i]['price'] = $product['price'] * (1 - $specific_prices[$i]['reduction']);
                        }

                        $group['specific_price'][$i] = $specific_prices[$i];
                    } else {
                        $group['specific_price'][$i]['price'] = $product['price'];
                    }
            ...
            $product['tarif_group'] = $tarif_groups;
            $product['has_attribute'] = 0;
        }

        $product['nb_specific'] = max($nb_specific);
        return $product;
    }

    public static function generateAccessory($id_product, $id_accessory) {

        if (Db::getInstance()->getRow('SELECT `id_product_1`, `id_product_2` FROM `' . _DB_PREFIX_ . 'accessory` WHERE `id_product_1` = ' . (int) $id_product . ' AND `id_product_2` =' . (int) $id_accessory)) {
            return true;
        } else {

            if (Db::getInstance()->insert('accessory', [
                'id_product_1' => (int) $id_product,
                'id_product_2' => (int) $id_accessory,
            ])) {
                return true;
            } else {
                return false;
