...
abstract class FluxCore {
    ...
    public static function getInstance($master = true) {
        ...
        if (!static::$_servers) {
            static::$_servers = [
                ['server' => _DB_SERVER_, 'user' => _DB_FLUX_USER_, 'password' => _DB_FLUX_PASSWD_, 'database' => _DB_FLUX_NAME_], /* MySQL Master server */
            ];
        }
    ...
    public function autoExecute($table, $data, $type, $where = '', $limit = 0, $useCache = true, $useNull = false) {
        ...
        default:
            throw new PhenyxShopDatabaseException('Wrong argument (miss type) in static::autoExecute()');
        }
     ...
     * @param bool   $addPrefix  Add or not _DB_FLUX_PREFIX_ before table name
     *
     * @return bool
     * @throws PhenyxShopDatabaseException
     *
    ...
    public function insert($table, $data, $nullValues = false, $useCache = true, $type = self::INSERT, $addPrefix = true) {
        ...
        if ($addPrefix && _DB_FLUX_PREFIX_ && strncmp(_DB_FLUX_PREFIX_, $table, strlen(_DB_FLUX_PREFIX_)) !== 0) {
            $table = _DB_FLUX_PREFIX_ . $table;
        }
        ...
        if ($type == static::INSERT) {
            ...
            throw new PhenyxShopDatabaseException('Bad keyword, must be static::INSERT or static::INSERT_IGNORE or static::REPLACE');
        }
            ...
            foreach ($rowData as $key => $value) {
                ...
                if (!$firstLoop) {
                    ...
                    if (!in_array("`$key`", $keys)) {
                        throw new PhenyxShopDatabaseException('Keys form $data subarray don\'t match');
                    }

                    if ($duplicateKeyStringified != '') {
                        throw new PhenyxShopDatabaseException('On duplicate key cannot be used on insert with more than 1 VALUE group');
                    }
    ...
    public function update($table, $data, $where = '', $limit = 0, $nullValues = false, $useCache = true, $addPrefix = true) {
        ...
        if ($addPrefix && strncmp(_DB_FLUX_PREFIX_, $table, strlen(_DB_FLUX_PREFIX_)) !== 0) {
            $table = _DB_FLUX_PREFIX_ . $table;
        }
     ...
     *
     * @param string|DbFlux $sql      Query to execute
     ...
     *
     * @param string|DbFlux $sql      the select query (without "LIMIT 1")
     * @param bool           $useCache Deprecated, the internal query cache is no longer used
     *
