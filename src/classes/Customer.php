...
class CustomerCore extends ObjectModel {
	...
	public function __construct($id = null) {
		...
		if ($this->id) {
			...
			$this->id_student = $this->getIdStudent();			
	...
	public function getIdStudent() {
		...
		return Db::getInstance()->getValue(
			...
			->select('`id_student`')
			->from('student')
			->where('`email` LIKE  \'' . pSQL($this->email) . '\' ')
		);		
	}
	...
	public static function getLastStudentEducation($idCustomer) {
		...
		$education = Db::getInstance()->getRow(
			(new DbQuery())
				->select('*')
				->from('student_education')
				->where('`id_customer` = ' . $idCustomer)
		...
		if ($education['id_student_education'] > 0) {
			return Education::getEducationDetails($education['id_education'], $education['id_education_attribute'], false);
