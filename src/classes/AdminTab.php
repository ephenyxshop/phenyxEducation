...
abstract class AdminTabCore {
    ...
    public function __construct() {
        ...
        $this->id = EmployeeMenu::getIdFromClassName(get_class($this));
        $this->_conf = [
    ...
    public function display() {
        ...
        // Include current tab
        else if ((Tools::getValue('submitAdd' . $this->table) && count($this->_errors)) || isset($_GET['add' . $this->table])) {
        ...
        } else if (isset($_GET['update' . $this->table])) {
    ...
    public function includeSubTab($methodname, $actions = []) {
        ...
        foreach ($this->_includeTab as $subtab => $extraVars) {
            ...
            } else if (file_exists(_PS_ADMIN_DIR_ . '/tabs/' . $classname . '.php')) {
    ...
    public function displayListHeader($token = null) {
        ...
        $totalPages = ceil($this->_listTotal / Tools::getValue('pagination', (isset($this->context->cookie->{$this->table . '_pagination'}) ? $this->context->cookie->{$this->table . '_pagination'}
            : $this->_pagination[0])));
        ...
        $selectedPagination = Tools::getValue('pagination', (isset($this->context->cookie->{$this->table . '_pagination'}) ? $this->context->cookie->{$this->table . '_pagination'}
            : null));
            ...
            foreach ($this->_list as $tr) {
                ...
                foreach ($this->fieldsDisplay as $key => $params) {
                    ...
                    if (isset($params['active']) && isset($tr[$key])) {
                        ...
                        if ($key == 'price') {
                            $echo = round($tr[$key], 2);
                        } else if (isset($params['maxlength']) && mb_strlen($tr[$key]) > $params['maxlength']) {
                ...
                if ($this->edit || $this->delete || ($this->view && $this->view !== 'noActionColumn')) {
                    ...
                    if ($this->delete && (!isset($this->_listSkipDelete) || !in_array($id, $this->_listSkipDelete))) {
                        $this->_displayDeleteLink($id, $token);
                    }
     ...
     * @deprecated 1.0.0
     */
    protected function _displayViewLink($id, $token = null) {
     ...
     * @deprecated 1.0.0
     */
    protected function _displayEditLink($id, $token = null) {
     ...
     * @deprecated 1.0.0
     */
    protected function _displayDeleteLink($id, $token = null) {
     ...
     * @deprecated 1.0.0
     */
    protected function _displayDuplicate($id, $token = null) {
...
//default behavior
    ...
    public function postProcess() {
        ...
        else if (Tools::isSubmit('submitFilter' . $this->table) || $this->context->cookie->{'submitFilter' . $this->table}
            !== false) {
            ...
            foreach ($this->optionsList as $category => $categoryData) {
                ...
                if (!count($this->_errors)) {
                    ...
                    foreach ($fields as $key => $options) {
                        ...
                        } else if (isset($options['type']) && in_array($options['type'], ['textLang', 'textareaLang'])) {
                            $list = [];
    ...
    public function displayOptionTypeImage($key, $field, $value) {
        ...
        */
        ...
        foreach ($field['list'] as $theme) {
            ...
            echo '<img src="../frontShop/' . $theme['name'] . '/preview.jpg" alt="' . mb_strtolower($theme['name']) . '">';
            echo '</label>';
            echo '</td>';
    ...
    public function getFieldValue($obj, $key, $id_lang = null, $idShop = null) {
        ...
        if ($id_lang) {
            $defaultValue = ($obj->id && isset($obj->{$key}
                [$id_lang])) ? $obj->{$key}
            [$id_lang] : '';
        } else {
