        ...
        foreach ($result as &$row) {
            ...
            switch (Configuration::get('PS_ROUND_TYPE')) {
                ...
                case Order::ROUND_LINE:
                    $row['total'] = Tools::ps_round($row['price_with_reduction_without_tax'] * (int) $row['cart_quantity'], _EPH_PRICE_DATABASE_PRECISION_);
                    $row['total_wt'] = Tools::ps_round($row['price_with_reduction'] * (int) $row['cart_quantity'], _EPH_PRICE_DATABASE_PRECISION_);
                    break;

                case Order::ROUND_ITEM:
                default:
                    $row['total'] = Tools::ps_round($row['price_with_reduction_without_tax'], _EPH_PRICE_DATABASE_PRECISION_) * (int) $row['cart_quantity'];
                    $row['total_wt'] = Tools::ps_round($row['price_with_reduction'], _EPH_PRICE_DATABASE_PRECISION_) * (int) $row['cart_quantity'];
                    break;
            }
    ...
    */
    ...
    public function updateQty(
        ...
        if (!empty($idProductAttribute)) {
            $minimalQuantity = (int) Attributes::getAttributeMinimalQty($idProductAttribute);
        } else {
    ...
    public function getSummaryDetails($idLang = null, $refresh = false)
    {
        ...
        foreach ($cartRules as &$cartRule) {
            ...
            if ($cartRule['free_shipping'] && (empty($cartRule['code']) || preg_match('/^'.CartRule::BO_ORDER_CODE_PREFIX.'[0-9]+/', $cartRule['code']))) {
                ...
                $cartRule['value_real'] = Tools::ps_round($cartRule['value_real'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                $cartRule['value_tax_exc'] = Tools::ps_round($cartRule['value_tax_exc'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
            ...
            if ($cartRule['gift_product']) {
                foreach ($products as $key => &$product) {
                    if (empty($product['gift']) && $product['id_product'] == $cartRule['gift_product'] && $product['id_product_attribute'] == $cartRule['gift_product_attribute']) {
                        ...
                        $totalProductsWt = Tools::ps_round($totalProductsWt - $product['price_wt'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                        $totalProducts = Tools::ps_round($totalProducts - $product['price'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                        ...
                        $cartRule['value_real'] = Tools::ps_round($cartRule['value_real'] - $product['price_wt'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                        $cartRule['value_tax_exc'] = Tools::ps_round($cartRule['value_tax_exc'] - $product['price'], (int) $context->currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);

                        // Update product quantity
                        $product['total_wt'] = Tools::ps_round($product['total_wt'] - $product['price_wt'], (int) $currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                        $product['total'] = Tools::ps_round($product['total'] - $product['price'], (int) $currency->decimals * _EPH_PRICE_DATABASE_PRECISION_);
                        $product['cart_quantity']--;
	...
	public function addAccessories()
    {
		...
		$file = fopen("testaddAccessories.txt","w");
		$context = Context::getContext();
		...
		fwrite($file,$group.PHP_EOL);
        $id_main_product_attribute = Tools::getValue('ipa', 0);
        if (!Tools::getValue('ajax') && !$this->isEnableBlockCartAjax()) {
            $accessories = AccessoriesGroupProductAbstract::getAccessoriesByIdProducts(array($id_product));
        ...
        if (self::$has_cart_been_added) {
            ...
            if (!self::$is_executed) {
                ...
                foreach ($this->accessories as $id_accessory) {
                    ...
                    $product = new Product($id_accessory, true, $context->language->id);
                    ...
                    if ($product->hasAttributes()) {
                        if (!Product::isAvailableWhenOutOfStock($product->out_of_stock) && !Attributes::checkAttributeQty($id_product_attribute, $accessory_qty[$i])) {
                    ...
                    if ($accessory_qty[$i] > 0) {
                        $this->updateQty($accessory_qty[$i], $id_accessory, $id_product_attribute);
                        // add table accessory cart product
                        $cart_product = new AccessoryCartProductAbstract($context->cart->id, $id_product, $id_main_product_attribute, $id_accessory, $id_product_attribute);
                        if (Validate::isLoadedObject($cart_product)) {
                            ...
                            $cart_product->id_cart = (int) $context->cart->id;
        ...
        if ($operation === 'down') {
            ...
            if (!empty($products)) {
                ...
                if ($current_product_quantity_incart > 0) {
                    $total_product_incart = AccessoryCartProductAbstract::getTotalProductInCart($this->context->cart->id, $id_product, $id_main_product_attribute);
                    if ($total_product_incart > 0 && $current_product_quantity_incart < $total_product_incart) {
                        $qty = (int) Tools::getValue('qty', 1);
                        $product_accessories = AccessoryCartProductAbstract::getProductByIdCartAccessory($this->context->cart->id, $id_product, $id_main_product_attribute);
                        if (!empty($product_accessories)) {
                            foreach ($product_accessories as $product_accessory) {
                                $accessory_cart_product = new AccessoryCartProductAbstract($this->context->cart->id, $product_accessory['id_product'], $product_accessory['id_product_attribute'], $product_accessory['id_accessory'], $product_accessory['id_accessory_attribute']);
