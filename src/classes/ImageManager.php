...
class ImageManagerCore {
    ...
    public static function resize(
        ...
        //Hook::exec('actionOnImageResizeAfter', array('dst_file' => $dstFile, 'file_type' => $fileType));
        @imagedestroy($srcImage);

        return $writeFile;
    }

    public static function actionOnImageResizeAfter($dstFile, $newFile) {
    ...
    public function actionOnImageUploadAfter($dstFile, $fileType) {

        try {
            $source = $params['dst_file'];
            $destination = str_replace("." . $params['file_type'], '.webp', $params['dst_file']);
            WebPConvert::convert($source, $destination, WebPGeneratorConfig::getConverterSettings());
        } catch (Exception $exception) {
    ...
    protected static function resizeWebP($destination) {
    ...
    public static function resizeOtherImage($image, $type, $types = []) {
        ...
        $process = [
            ...
            'img'          => _PS_IMG_DIR_,
            'racetrack'    => _PS_RACETRACK_IMG_DIR_,
        ];
        $dir = $process[$type];
        $result = false;
        ...
        if (preg_match('/^\d*\.jpg$/', $image)) {
            ...
            foreach ($types as $imageType) {
                ...
                $result = self::resizeWebP($newFile);
    ...
    public static function resizeProductImage($imageId, $types = []) {
    ...
    public static function webpSupport($checkAccept = false) {
        ...
        if ($supported === null) {
            ...
            try {
                $supported = Configuration::get('EPH_USE_WEBP')
                && !empty($config['webp'])
	...
	public static function deleteImageType($type) {
		...
		$dir = _PS_EDUC_IMG_DIR_;
		$iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(_PS_EDUC_IMG_DIR_));
    ...
    public static function getImages() {
        ...
        foreach ($singularTypes as $key => $singularType) {
            ...
            $result = ImageManager::getFolderImg($singularType['dir'], $singularType['iterate']);
