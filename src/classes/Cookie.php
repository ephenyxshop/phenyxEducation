...
class CookieCore {
    ...
    public function __construct($name, $path = '', $expire = null, $sharedUrls = null, $standalone = false, $secure = false) {
        ...
        $this->_salt = $this->_standalone ? str_pad('', 8, md5('phenyx' . __FILE__)) : _COOKIE_IV_;
    ...
    /**
     * Check student informations saved into cookie and return student validity
     *
     * @deprecated 1.0.0 use Customer::isLogged() instead
     *
     * @param bool $withGuest
     *
     * @return bool student validity
     */
    public function isLogged() {
        ...
        if ($this->logged == 1 && $this->id_student && Validate::isUnsignedId($this->id_student) && Student::checkPassword((int) ($this->id_student), $this->passwd)) {
            return true;
        }
    ...
    /**
     * Soft logout, delete everything links to the student
    ...
    public function mylogout() {
        ...
        unset($this->_content['is_agent']);
        unset($this->_content['id_connections']);
        unset($this->_content['student_lastname']);
        unset($this->_content['student_firstname']);
        unset($this->_content['passwd']);
        unset($this->_content['logged']);
        unset($this->_content['email']);
        unset($this->_content['is_agent']);
        unset($this->_content['id_agent']);
        $this->_modified = true;
    }
