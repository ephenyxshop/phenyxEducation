...
/**
 * Class PhenyxFluxCollectionCore
 *
 * @since 1.9.1.0
 */
class PhenyxFluxCollectionCore implements Iterator, ArrayAccess, Countable {
    ...
    public function __construct($classname, $idLang = null) {
        ...
        if (!isset($this->definition['primary'])) {
            throw new PhenyxShopException('Miss primary in definition for class ' . $this->classname);
        }

        $this->query = new DbFlux();
    }
        ...
        if (!isset($this->fields[$field])) {
            ...
            if ($fieldname == $definition['primary'] || (!empty($definition['is_lang']) && $fieldname == 'id_lang')) {
                ...
                if (!isset($definition['fields'][$fieldname])) {
                    throw new PhenyxShopException('Field ' . $fieldname . ' not found in class ' . $definition['classname']);
                }

                // Test field validity for language fields

                if (empty($definition['is_lang']) && !empty($definition['fields'][$fieldname]['lang'])) {
                    throw new PhenyxShopException('Field ' . $fieldname . ' is declared as lang field but is used in non multilang context');
                } else
                if (!empty($definition['is_lang']) && empty($definition['fields'][$fieldname]['lang'])) {
                    throw new PhenyxShopException('Field ' . $fieldname . ' is not declared as lang field but is used in multilang context');
                }
        ...
        if (!isset($this->association_definition[$association])) {
            ...
            for ($i = 0, $totalAssociation = count($split); $i < $totalAssociation; $i++) {
                ...
                if (!isset($definition['associations'][$asso])) {
                    throw new PhenyxShopException('Association ' . $asso . ' not found for class ' . $this->definition['classname']);
                }
    ...
    public function where($field, $operator, $value, $method = 'where') {
        ...
        if ($method != 'where' && $method != 'having') {
            throw new PhenyxShopException('Bad method argument for where() method (should be "where" or "having")');
        }
        ...
        if (is_array($value)) {
            ...
            default:
                throw new PhenyxShopException('Operator not supported for array value');
            }
        ...
        // Create WHERE clause
            ...
            case 'notregexp':
                ...
                break;
            default:
                throw new PhenyxShopException('Operator not supported');
            }
    ...
    public function orderBy($field, $order = 'asc') {
        ...
        if ($order != 'asc' && $order != 'desc') {
            throw new PhenyxShopException('Order must be asc or desc');
        }
    ...
    public function getAll($displayQuery = false) {
        ...
        $this->results = Flux::getInstance(_PS_USE_SQL_SLAVE_)->executeS($this->query);
    ...
    public function offsetGet($offset) {
        ...
        if (!isset($this->results[$offset])) {
            throw new PhenyxShopException('Unknown offset ' . $offset . ' for collection ' . $this->classname);
        }
    ...
    public function offsetSet($offset, $value) {
        ...
        if (!$value instanceof $this->classname) {
            throw new PhenyxShopException('You cannot add an element which is not an instance of ' . $this->classname);
        }
