...
class MediaAdminCore {
	...
	public static function getJqueryPath($version = null, $folder = null, $minifier = true) {
		...
		if ($folder === null) {
			$folder = _PS_JS_ADMIN_DIR_ . 'jquery/';
		}
		...
		if ($addNoConflict) {
			$return[] = MediaAdmin::getJSPath(_PS_JS_ADMIN_DIR_ . 'jquery/jquery.noConflict.php?version=' . $version);
		}

		//added query migrate for compatibility with new version of jquery will be removed in ps 1.6
		//$return[] =  MediaAdmin::getJSPath(_PS_JS_ADMIN_DIR_ . 'jquery/jquery-migrate-1.2.1.min.js');
		$return[] = MediaAdmin::getJSPath(_PS_JS_ADMIN_DIR_ . 'jquery/jquery-migrate-3.3.2.min.js');
	...
	public static function getJqueryUIPath($component, $theme, $checkDependencies) {
		...
		$uiPath = ['js' => [], 'css' => []];
		$folder = _PS_JS_ADMIN_DIR_ . 'jquery/ui/';
	...
	public static function getJqueryPluginPath($name, $folder = null) {
		...
		if ($folder === null) {
			$folder = _PS_JS_ADMIN_DIR_ . 'jquery/plugins/';
		}
