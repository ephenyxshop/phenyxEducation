...
class PerformerCore {
    ...
    protected function loadRoutes($idShop = null) {
        ...
        $prodroutes = 'EPH_ROUTE_product_rule';
        $catroutes = 'EPH_ROUTE_category_rule';
    ...
    public function dispatch() {
        ...
        case static::FC_ADMIN:
            ...
            if ($tab->module) {
                ...
                $controllers = Performer::getControllers([_PS_ADMIN_DIR_ . '/tabs/', _PS_ADMIN_CONTROLLER_DIR_]);

                if (!isset($controllers[strtolower($this->controller)])) {
                    // If this is a parent tab, load the first child

                    if (Validate::isLoadedObject($tab) && $tab->id_parent == 0 && ($tabs = EmployeeMenu::getEmployeeMenus(Context::getContext()->language->id, $tab->id)) && isset($tabs[0])) {
        ...
        try {
            ...
            // Execute hook >Performer

            if (isset($paramsHookActionDispatcher)) {
                Hook::exec('actionPerformer', $paramsHookActionDispatcher);
            }
