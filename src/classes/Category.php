...
class CategoryCore extends ObjectModel {
        ...
        if (($maxDepth == 0 || $currentDepth < $maxDepth) && $subcats && count($subcats)) {
            ...
            foreach ($subcats as &$subcat) {
                ...
                if (!$subcat['id_category']) {
                    break;
                } else if (!is_array($excludedIdsArray) || !in_array($subcat['id_category'], $excludedIdsArray)) {
     ...
     * @param int $idLang
     *
     * @return PhenyxShopCollection Collection of Category
     *
     * @since   1.8.1.0
     * @version 1.8.5.0
     * @throws PhenyxShopException
    ...
    public function getAllChildren($idLang = null) {
        ...
        $categories = new PhenyxShopCollection('Category', $idLang);
    ...
    public function getProducts($idLang, $p, $n, $orderBy = null, $orderWay = null, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $checkAccess = true, Context $context = null) {
        ...
        if ($orderBy == 'id_product' || $orderBy == 'date_add' || $orderBy == 'date_upd') {
            ...
            $orderByPrefix = 'pl';
        } else if ($orderBy == 'manufacturer' || $orderBy == 'manufacturer_name') {
            $orderByPrefix = 'm';
            $orderBy = 'name';
        ...
        foreach ($results as $result) {
            $groups_allowed = unserialize($result['groups_allowed']);

            if (is_array($groups_allowed) && in_array($id_group, $groups_allowed)) {
    ...
    public function addShop($idShop) {
        ...
        } else if (!$this->existsInShop($idShop)) {
            $data[] = [
                'id_category' => (int) $this->id,
                'id_shop'     => (int) $idShop,
            ];
