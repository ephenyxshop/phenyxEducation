...
class TopMenuCore extends ObjectModel {
	...
	public function getNameValue() {
		...
		$sql = 'SELECT atp.*, atpl.*,
                ...
                LEFT JOIN ' . _DB_PREFIX_ . 'education_type_lang etl ON (etl.id_education_type = atp.`id_education_type` AND etl.`id_lang` = ' . (int) $context->language->id . ')
    ...
    public function delete() {
        ...
        foreach ($languages as $language) {
            ...
            if (file_exists(_PS_ROOT_DIR_ . '/img/menu_icons/' . (int) $this->id . '-' . $language['iso_code'] . '.' . (isset($this->image_type[$language['id_lang']]) && !preg_match('/^i-(fa|mi)$/', $this->image_type[$language['id_lang']]) ? $this->image_type[$language['id_lang']] : 'jpg'))) {
                @unlink(_PS_ROOT_DIR_ . '/img/menu_icons/' . (int) $this->id . '-' . $language['iso_code'] . '.' . (isset($this->image_type[$language['id_lang']]) && !preg_match('/^i-(fa|mi)$/', $this->image_type[$language['id_lang']]) ? $this->image_type[$language['id_lang']] : 'jpg'));
            }
    ...
    public static function getMenus($id_lang, $active = true, $get_from_all_shops = false, $groupRestrict = false) {
        ...
        $sql = 'SELECT atp.*, atpl.*,
                ...
                ccl.name as cms_category_name
                FROM `' . _DB_PREFIX_ . 'topmenu` atp
                ' . (Shop::isFeatureActive() ? self::addSqlAssociation('topmenu', 'atp', 'id_topmenu', true, null, ($get_from_all_shops ? 'all' : false)) : '') . '
                LEFT JOIN `' . _DB_PREFIX_ . 'topmenu_lang` atpl ON (atp.`id_topmenu` = atpl.`id_topmenu` AND atpl.`id_lang` = ' . (int) $id_lang . ')
                LEFT JOIN ' . _DB_PREFIX_ . 'education_type_lang etl ON (etl.id_education_type = atp.`id_education_type` AND etl.`id_lang` = ' . (int) $id_lang . ')
                LEFT JOIN ' . _DB_PREFIX_ . 'cms c ON (c.id_cms = atp.`id_cms`)
                ...
                LEFT JOIN ' . _DB_PREFIX_ . 'cms_category cc ON (cc.id_cms_category = atp.`id_cms_category`)
                ' . Shop::addSqlAssociation('cms_category', 'cc', false, true, null, ($get_from_all_shops ? 'all' : false)) . '
                ...
                AND atp.`id_cms` = 0 AND atp.`id_cms_category` = 0
    ...
    public static function getAdminOutputNameValue($row, $withExtra = true, $type = false, $id = null) {
        ...
        if ($id > 0) {
            ...
            if ($withExtra && isset($row['have_icon']) && trim($row['have_icon'])) {
        ...
        if ($row['type'] == 7) {
            ...
            if ($withExtra && trim($row['have_icon'])) {
                ...
                if (in_array($row['image_type'], ['i-fa', 'i-mi'])) {
                    ...
                    $return .= '<img src="' . _PS_IMG_ . $type . '_icons/' . $row['id_topmenu_' . $type] . '-' . $_iso_lang . '.' . ($row['image_type'] ?: 'jpg') . '" alt="' . htmlentities($row['name'], ENT_COMPAT, 'UTF-8') . '" title="' . htmlentities($row['name'], ENT_COMPAT, 'UTF-8') . '" class="iconOnly" />';
                }
