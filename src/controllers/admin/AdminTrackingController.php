...
class AdminTrackingControllerCore extends AdminController
{
    ...
    public function initContent()
    {
		...
		$file = fopen("testTracking.txt","w");
        $methods = get_class_methods($this);
		fwrite($file,print_r($methods, true));
        $tplVars['arrayList'] = [];
        foreach ($methods as $methodName) {
            if (preg_match('#getCustomList(.+)#', $methodName, $matches)) {
                $this->clearListOptions();
                call_user_func([$this, $matches[0]]);
            }
        }
    ...
    public function displayDeleteLink($id, $token = null,  $name = null)
    {
        ...
        return $this->_helper_list->displayDeleteLink($id,$this->_list_token,  $name);
    }
