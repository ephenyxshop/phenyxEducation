...
class AdminImportControllerCore extends AdminController
{
    ...
    public function setMedia()
    {
        ...
        $this->addJS(__PS_BASE_URI__.$this->admin_webpath.'/'.$backOfficeTheme.'/js/jquery.iframe-transport.js');
        $this->addJS(__PS_BASE_URI__.$this->admin_webpath.'/'.$backOfficeTheme.'/js/jquery.fileupload.js');
        $this->addJS(__PS_BASE_URI__.$this->admin_webpath.'/'.$backOfficeTheme.'/js/jquery.fileupload-process.js');
        $this->addJS(__PS_BASE_URI__.$this->admin_webpath.'/'.$backOfficeTheme.'/js/jquery.fileupload-validate.js');
    ...
    {
        $this->addJS(__PS_BASE_URI__.$this->admin_webpath.'/js/import.js');
    ...
    public function attributeImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        foreach (Attributes::getAttributes($defaultLanguage) as $attribute) {
            ...
            foreach (explode($this->multiple_value_separator, $info['attribute']) as $key => $attribute) {
                ...
                if (isset($groupsAttributes[$key])) {
                    ...
                    if (!isset($attributes[$group.'_'.$attribute]) && count($groupsAttributes[$key]) == 2) {
                        ...
                        $obj->position = (!$position && isset($groups[$group])) ? Attributes::getHigherPosition($groups[$group]) + 1 : $position;
