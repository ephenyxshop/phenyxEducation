...
class AdminEmployeeMenusControllerCore extends AdminController {
	...
	public function __construct() {
		...
		//$this->extracss = $this->pushCSS([__PS_BASE_URI__ . $this->admin_webpath . '/themes/' . $this->bo_theme . '/css/employee_menus.css']);
	...
	public function getEmployeeMenuRequest($idParent = 0) {
		...
		$employee_menus = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				->select('b.name, a.id_employee_menu, a.class_name, a.id_parent, a.id_employee_menu as id,  a.function, a.active, a.position, a.is_synch, a.is_shop , a.is_education')
		...
		foreach ($employee_menus as &$employee_menu) {
			...
			if ($employee_menu['is_synch'] == 1) {
				$employee_menu['is_synch'] = '<div class="fa fa-check" style="color: green" onClick="menuSynchOn(' . $employee_menu['id_employee_menu'] . ')"></div>';
			} else {
				$employee_menu['is_synch'] = '<div class="fa fa-times" style="color:red" onClick="menuSynchOff(' . $employee_menu['id_employee_menu'] . ')"></div>';
			}

			if ($employee_menu['is_shop'] == 1) {
				$employee_menu['is_shop'] = '<div class="fa fa-check" style="color: green"></div>';
			} else {
				$employee_menu['is_shop'] = '<div class="fa fa-times" style="color:red"></div>';
			}

			if ($employee_menu['is_education'] == 1) {
				$employee_menu['is_education'] = '<div class="fa fa-check" style="color: green"></div>';
			} else {
				$employee_menu['is_education'] = '<div class="fa fa-times" style="color:red"></div>';
			}
			...
			$employee_menu['openLink'] = $this->context->link->getAdminLink($this->controller_name) . '&id_employee_menu=' . $employee_menu['id_employee_menu'] . '&updateemployee_menu';
			$employee_menu['addLink'] = $this->context->link->getAdminLink($this->controller_name) . '&action=addObject&id_parent=' . $employee_menu['id_employee_menu'] . '&addemployee_menu';
			$employee_menu['deleteLink'] = $this->context->link->getAdminLink($this->controller_name) . '&id_employee_menu=' . $employee_menu['id_employee_menu'] . '&id_object=' . $employee_menu['id_employee_menu'] . '&deleteemployee_menu&action=deleteEmployeeMenu&ajax=true';
	...
	public function buildChildrenTree($idEmployeeMenus) {

		$employee_menus = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				->select('b.name, a.id_employee_menu, a.class_name, a.id_employee_menu as id,  a.active, a.position')
				->from('employee_menu', 'a')
				->leftJoin('employee_menu_lang', 'b', 'b.`id_employee_menu` = a.`id_employee_menu` AND b.`id_lang` = ' . $this->context->language->id)
				->where('a.`id_parent` = ' . (int) $idEmployeeMenus)
				->orderBy('a.`position` ASC')
		);
		...
		foreach ($employee_menus as &$employee_menu) {
			...
			if ($employee_menu['active'] == 1) {
				$employee_menu['active'] = '<div class="fa fa-check" style="color: green"></div>';
				$employee_menu['enable'] = true;
			} else {
				$employee_menu['active'] = '<div class="fa fa-times" style="color:red"></div>';
				$employee_menu['enable'] = false;
			}

			$employee_menu['employee_menuPosition'] = $employee_menu['position'];
			$idParent = $employee_menu['id_parent'];

			if ($employee_menu['id_parent'] == 0) {
				$idParent = 'parent';
			}

			$employee_menu['position'] = '<div class="dragGroup"><div class="employee_menuPosition_' . $idParent . ' positions" data-id="' . $employee_menu['id_employee_menu'] . '">' . $employee_menu['position'] . '</div></div>';

			$employee_menu['childrens'] = $this->buildChildrenTree($employee_menu['id_employee_menu']);
	...
	public function ajaxProcessEditObject() {
		...
		if ($this->tabAccess['edit'] == 1) {
			...
			$li = '<li id="uperEdit' . $targetController . '" data-controller="AdminDashboard"><a href="#contentEdit' . $targetController . '">Edition du menu ' . $object->name[$this->context->language->id] . '</a><button type="button" class="close tabdetail" data-id="uperEdit' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
	...
	public function ajaxProcessAddObject() {
		...
		if ($this->tabAccess['add'] == 1) {
			...
			$li = '<li id="uperAdd' . $targetController . '" data-controller="AdminDashboard"><a href="#contentEdit' . $targetController . '">Ajouter un nouvel Onglet </a><button type="button" class="close tabdetail" data-id="uperAdd' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
			$html = '<div id="contentEdit' . $targetController . '" class="panel col-lg-12" style="display; flow-root;">' . $data->fetch() . '</div>';
			$result = [
				'success' => true,
				'li'      => $li,
