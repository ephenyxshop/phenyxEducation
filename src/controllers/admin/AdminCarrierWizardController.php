...
class AdminCarrierWizardControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        try {
            $this->tabAccess = Profile::getProfileAccess($this->context->employee->id_profile, EmployeeMenu::getIdFromClassName('AdminCarriers'));
    ...
    public function setMedia() {
        ...
        $this->addJs(__PS_BASE_URI__ . $this->admin_webpath . '/js/carrier_wizard.js');
    }
    ...
    public function getActualCurrency() {
        ...
        try {
            ...
            } else if ($this->type_context == Shop::CONTEXT_GROUP) {
    ...
    public function changeZones($id) {
        ...
        foreach ($zones as $zone) {
            ...
            } else if (isset($_POST['zone_' . $zone['id_zone']]) && $_POST['zone_' . $zone['id_zone']]) {
    ...
    public function processRanges($idCarrier) {
        ...
        if ($rangeType != Carrier::SHIPPING_METHOD_FREE) {
            ...
            foreach ($rangeInf as $key => $delimiter1) {
                ...
                if ($rangeType == Carrier::SHIPPING_METHOD_WEIGHT) {

                    if (!RangeWeight::rangeExist(null, (float) $delimiter1, (float) $rangeSup[$key], $carrier->id_reference)) {
                ...
                if ($rangeType == Carrier::SHIPPING_METHOD_PRICE) {

                    if (!RangePrice::rangeExist(null, (float) $delimiter1, (float) $rangeSup[$key], $carrier->id_reference)) {
