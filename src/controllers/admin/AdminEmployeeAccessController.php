...
class AdminEmployeeAccessControllerCore extends AdminController {
	...
	public function ajaxProcessOpenTargetController() {
		...
		if($domaine == 'phenyx.education') {
			...
			$data = $this->createTemplate('controllers/employee_access.tpl');
			...
			foreach($licenses as $license) {
				...
				$tabs[$license->id] = EmployeeMenu::getPartnerEmployeeMenus($license, $this->context->language->id);
				foreach ($profiles as $profile) {
            	 	$accesses[$license->id][$profile['id_profile']] = Profile::getProfilePartnerAccesses($license, $profile['id_profile']);
				}
				//fwrite($file, "access truc ". print_r($accesses[$license->id], true));
			
				foreach ($tabs[$license->id] as $key => $tab) {
			...
			$data = $this->createTemplate('controllers/menuSingleeAccess.tpl');
			$currentProfile = (int)$this->getCurrentProfileId();
