...
class AdminStatsControllerCore extends AdminStatsTabController {
     ...
     * @return array|int
     */
    public static function getVisits($dateFrom, $dateTo, $granularity = false, $unique = false) {
	...
	public static function getPrevisionnel($dateFrom, $dateTo, $granularity = false) {
		...
		if($domaine == 'phenyx.education') {
			...
			foreach ($licenses as $licence) {
				$invoices[$licence->id] = $licence->getPhenyxPrevisionnel($dateFrom, $dateTo);
			}
			
			foreach($invoices as $key => $invoice) {				
				foreach ($invoice as $key => $row) {
					$previsionnel[$key] = (float)$row;
				}
			}
			...
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
            SELECT
            LEFT(es.`session_date`, 10) as date, SUM(se.price) as amount
            FROM `' . _DB_PREFIX_ . 'student_education` se
			LEFT JOIN `' . _DB_PREFIX_ . 'education_session` es ON (es.id_education_session = se.id_education_session)
            WHERE se.`id_student_education_state` > 5 AND se.`id_student_education_state` < 8 AND  es.`session_date` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59" 
            GROUP BY LEFT(es.`session_date`, 10)'
            );
    ...
    public static function getOrders($dateFrom, $dateTo, $granularity = false) {
		...
		if ($granularity == 'day') {
			...
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
            SELECT
            LEFT(es.`session_date`, 10) as date, SUM(sac.amount) as commission
            FROM `' . _DB_PREFIX_ . 'student_education` se
			LEFT JOIN `' . _DB_PREFIX_ . 'education_session` es ON (es.id_education_session = se.id_education_session)
			LEFT JOIN `' . _DB_PREFIX_ . 'sale_agent_commission` sac ON (sac.id_student_education = se.id_student_education)
			LEFT JOIN `' . _DB_PREFIX_ . 'sale_agent` sa ON (sa.id_sale_agent = se.id_sale_agent)
            WHERE se.`id_student_education_state` > 3 AND se.`id_sale_agent` > 0 AND es.`session_date` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59" AND sac.`due` = 1 AND sa.sale_commission_amount > 0
            GROUP BY LEFT(es.`session_date`, 10)'
            );
            ...
            foreach ($result as $row) {
                ...
                $orders[strtotime($row['date'])] = $row['commission'];
            }
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
             SELECT
            LEFT(es.`session_date`, 7) as date, count(se.id_student_education) as nbSale
            FROM `' . _DB_PREFIX_ . 'student_education` se
			LEFT JOIN `' . _DB_PREFIX_ . 'education_session` es ON (es.id_education_session = se.id_education_session)
            WHERE se.`id_student_education_state` > 3 AND se.`id_sale_agent` > 0 AND es.`session_date` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"
            GROUP BY LEFT(es.`session_date`, 7)'
            );

            foreach ($result as $row) {
                $orders[strtotime($row['date'] . '-01')] = $row['nbSale']*100;
            }
	...
	public static function getVdiSales($dateFrom, $dateTo, $granularity = false) {
		...
		$agents = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SELECT id_sale_agent, firstname, lastname FROM `' . _DB_PREFIX_ . 'sale_agent` WHERE sale_commission_amount > 0');
		
		foreach($agents as &$agent) {
			
			$saleAgent = new SaleAgent($agent['id_sale_agent']);
			
			$dateTwo = new DateTime($saleAgent->date_add);
			$agent['existing_days'] = $dateTwo->diff($now)->format("%a");
			$agent['total_sale'] = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
				'SELECT SUM(sac.amount) 
				FROM `' . _DB_PREFIX_ . 'student_education` se
				LEFT JOIN `' . _DB_PREFIX_ . 'education_session` es ON (es.id_education_session = se.id_education_session)
				LEFT JOIN `' . _DB_PREFIX_ . 'sale_agent_commission` sac ON (sac.id_student_education = se.id_student_education)
				WHERE se.`id_student_education_state` > 3 AND se.id_sale_agent = '.$agent['id_sale_agent'].' AND sac.`due` = 1 AND es.`session_date` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"');
			$agent['nb_sale'] = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
				'SELECT COUNT(se.id_student_education) 
				FROM `' . _DB_PREFIX_ . 'student_education` se
				LEFT JOIN `' . _DB_PREFIX_ . 'education_session` es ON (es.id_education_session = se.id_education_session)
				LEFT JOIN `' . _DB_PREFIX_ . 'sale_agent_commission` sac ON (sac.id_student_education = se.id_student_education)
				WHERE se.`id_student_education_state` > 3 AND se.id_sale_agent = '.$agent['id_sale_agent'].' AND sac.`due` = 1 AND es.`session_date` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"');
    ...
    public static function getTotalSales($dateFrom, $dateTo, $granularity = false) {
		...
		if ($granularity == 'day') {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
             SELECT LEFT(`date_add`, 10) as date, SUM(total_tax_excl) as sales
           ...
           '
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
             SELECT LEFT(`date_add`, 7) as date, SUM(total_tax_excl) as sales
            FROM `' . _DB_PREFIX_ . 'customer_pieces` 
            WHERE  `date_add` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"
            GROUP BY LEFT(`date_add`, 7)
           '
            ...
            return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                '
            SELECT SUM(total_tax_excl) as sales
            FROM `' . _DB_PREFIX_ . 'customer_pieces`
            WHERE `date_add` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"
            '
            );
    ...
    public static function getExpenses($dateFrom, $dateTo, $granularity = false) {
		...
		if ($granularity == 'day') {
			 ...
			 $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
             SELECT LEFT(`date_add`, 10) as date, (SUM(total_tax_excl) - SUM(piece_margin) ) as expense
            FROM `' . _DB_PREFIX_ . 'customer_pieces` 
            WHERE  `date_add` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"
            GROUP BY LEFT(`date_add`, 10)
           '
            ...
            foreach ($result as $row) {
                $purchases[strtotime($row['date'])] = $row['expense'];
            }
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                '
            SELECT `id_education_supplies`, COUNT(id_student_education) as total_education
            FROM `' . _DB_PREFIX_ . 'student_education`
            WHERE `id_student_education_state` > 3 AND `date_add` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"'
            );
			
			foreach ($result as $row) {
				$purchases[strtotime($row['date'])] = $row['total_education']*$pcCost;
            }
			return $purchases;
        }
    ...
    public static function getPurchases($dateFrom, $dateTo, $granularity = false) {
		...
		if ($granularity == 'day') {
            ...
            $purchases = [];
			 ...
			 $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
                '
             SELECT LEFT(`date_add`, 10) as date, SUM(total_tax_incl) as sales, SUM(piece_margin) as margin
            FROM `' . _DB_PREFIX_ . 'customer_pieces` 
            WHERE  `date_add` BETWEEN "' . pSQL($dateFrom) . ' 00:00:00" AND "' . pSQL($dateTo) . ' 23:59:59"
            GROUP BY LEFT(`date_add`, 10)
           '
            ...
            foreach ($result as $row) {
                $purchasess[strtotime($row['date'])] = $row['sales'] - $row['margin'];
            }
