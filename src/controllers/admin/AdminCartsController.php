...
class AdminCartsControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->extracss = $this->pushCSS([
			$this->admin_webpath . '/themes/' . $this->bo_theme . '/css/cart.css',
	...
	public function setMedia() {
		...
		MediaAdmin::addJsDef([
			'AjaxLinkAdminCarts' => $this->context->link->getAdminLink($this->controller_name),

		]);
		$this->addCSS(__PS_BASE_URI__ . $this->admin_webpath . '/themes/' . $this->bo_theme . '/css/cart.css', 'all', 0);
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			if (Tools::getValue('only_display') != 1) {
				...
				foreach ($customizationFields as $customizationField) {
					...
					if ($customizationField['type'] == Product::CUSTOMIZE_TEXTFIELD) {
					...
					} else if ($customizationField['type'] == Product::CUSTOMIZE_FILE) {
						...
						if (!ImageManager::resize($tmpName, _PS_UPLOAD_DIR_ . $fileName)) {
							continue;
						} else if (!ImageManager::resize($tmpName, _PS_UPLOAD_DIR_ . $fileName . '_small', (int) Configuration::get('PS_PRODUCT_PICTURE_WIDTH'), (int) Configuration::get('PS_PRODUCT_PICTURE_HEIGHT'))) {
							$errors[] = Tools::displayError('An error occurred during the image upload process.');
	...
	public function ajaxProcessUpdateQty() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			} else if (!($idProduct = (int) Tools::getValue('id_product')) || !($product = new Product((int) $idProduct, true, $this->context->language->id))) {
				$errors[] = Tools::displayError('Invalid product');
			...
			if (isset($product) && $product->id) {
				...
				} else if (!$product->checkQty((int) $qty)) {
					$errors[] = Tools::displayError('There is not enough product in stock.');
				}
	...
	public function ajaxProcessUpdateOrderMessage() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			} else if (Validate::isLoadedObject($message)) {
				$message->delete();
			}
	...
	public function ajaxProcessAddVoucher() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			} else if ($err = $cartRule->checkValidity($this->context)) {
				$errors[] = $err;
			}
