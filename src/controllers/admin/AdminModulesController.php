...
class AdminModulesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->categorySelector = '<div class="pq-theme"><select id="categorySelect"><option value="">' . $this->l('--Select--') . '</option>';

        foreach ($this->modules_categories as $key => $category) {
            $this->categorySelector .= '<option value="' . $key . '">' . $category . '</option>';
        }

        $this->categorySelector .= '</select></div>';
        ...
        foreach ($modules as $km => $module) {
            ...
            if (Module::initUpgradeModule($module)) {
                ...
                if ($object = Adapter_ServiceLocator::get($module->name)) {
                    ...
                    } else if ((count($confModuleList = $object->getConfirmations()))) {
            ...
            if ($module->active && isset($module->warning) && !empty($module->warning) && !$this->ajax) {
                ...
                $this->context->smarty->assign('text', $module->displayName . ':' . $module->warning);
    ...
    public function getModuleRequest() {
        ...
        foreach ($modules as $km => $module) {
            ...
            if ($module->active) {
                $module->enable = true;
                $module->active = '<div class="fa fa-check"></div>';
            } else {
                $module->enable = false;
                $module->active = '<div class="fa fa-times"></div>';
            }
			...
			if($module->installed) {
				$module->is_installed = '<div class="fa fa-check"></div>';
			} else {
				$module->is_installed = '<div class="fa fa-times"></div>';
			}
			
			
		
            $module->image = '<img src="' . __PS_BASE_URI__ . basename(_PS_MODULE_DIR_) . DIRECTORY_SEPARATOR . $module->name . DIRECTORY_SEPARATOR . 'logo.png" class="imgm img-thumbnail" style="width:70px" width="70">';
    ...
    public function isModuleFiltered($module) {
        ...
        } else if ((int) Db::getInstance()->getValue(
            (new DbQuery())
        ...
        if ($showTypeModules == 'nativeModules' && !in_array($module->name, $this->list_natives_modules)) {
            return true;
        } else if ($showTypeModules == 'partnerModules' && !in_array($module->name, $this->list_partners_modules)) {
            return true;
        } else if ($showTypeModules == 'addonsModules' && (!isset($module->type) || $module->type != 'addonsBought')) {
            return true;
        } else if ($showTypeModules == 'mustHaveModules' && (!isset($module->type) || $module->type != 'addonsMustHave')) {
            return true;
        } else if ($showTypeModules == 'otherModules' && (in_array($module->name, $this->list_partners_modules) || in_array($module->name, $this->list_natives_modules))) {
            return true;
        } else if (strpos($showTypeModules, 'authorModules[') !== false) {
    ...
    public function postProcessDownload() {
        ...
        if ($this->tabAccess['add'] === '1') {
            ...
            } else if (substr($_FILES['file']['name'], -4) != '.tar' && substr($_FILES['file']['name'], -4) != '.zip'
                && substr($_FILES['file']['name'], -4) != '.tgz' && substr($_FILES['file']['name'], -7) != '.tar.gz'
            ) {
                $this->errors[] = Tools::displayError('Unknown archive type.');
            } else if (!move_uploaded_file($_FILES['file']['tmp_name'], _PS_MODULE_DIR_ . $_FILES['file']['name'])) {
    ...
    public function postProcessCallback() {
        ...
        foreach ($this->map as $key => $method) {
            ...
            } else if (($modules = Tools::getValue($key)) && $key != 'checkAndUpdate' && $key != 'updateAll') {
