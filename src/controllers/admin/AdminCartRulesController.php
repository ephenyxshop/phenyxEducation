...
class AdminCartRulesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->l('Gestion des promotions');
        $this->lang = true;

        parent::__construct();

        $this->paragridScript = EmployeeConfiguration::get('EXPERT_CARTRULES_SCRIPT');

        if (empty($this->paragridScript)) {
            EmployeeConfiguration::updateValue('EXPERT_CARTRULES_SCRIPT', $this->generateParaGridScript(true));
            $this->paragridScript = EmployeeConfiguration::get('EXPERT_CARTRULES_SCRIPT');
        }

        $this->configurationField = Tools::jsonDecode(EmployeeConfiguration::get('EXPERT_CARTRULES_FIELDS'), true);

        if (empty($this->configurationField)) {
            EmployeeConfiguration::updateValue('EXPERT_CARTRULES_FIELDS', Tools::jsonEncode($this->getCartRuleFields()));
            $this->configurationField = Tools::jsonDecode(EmployeeConfiguration::get('EXPERT_CARTRULES_FIELDS'), true);
        }

        $this->extracss = $this->pushCSS([
            $this->admin_webpath . '/js/jquery/plugins/autocomplete/jquery.autocomplete.css',
            $this->admin_webpath . '/js/jquery/ui/themes/base/jquery.ui.datepicker.css',
    ...
    public function setAjaxMedia() {
        ...
        return $this->pushJS([
            $this->admin_webpath . '/js/jquery/ui/i18n/jquery.ui.datepicker-fr.js',
            ...
            $this->admin_webpath . '/js/jquery/plugins/jquery.jscroll.js',
        ]);
    }
    ...
    public function generateParaGridScript($regenerate = false) {
        ...
        $paragrid->toolbar = [
            'items' => [
                ...
                [
                    'type'     => '\'button\'',
                    'icon'     => '\'ui-icon-disk\'',
                    'label'    => '\'' . $this->l('Ajouter un bon de Réduction') . '\'',
        ...
        $paragrid->title = '\'' . $this->l('Gestion des bons de réduction') . '\'';
                    ...
                    items: {
                        ...
                        "edit": {
                            ...
                            callback: function(itemKey, opt, e) {
                                ...
                                editAjaxObject("' . $this->controller_name . '", rowData.id_cart_rule)
                            }
                        },
                        ...
                        "delete": {
                            ...
                            callback: function(itemKey, opt, e) {
                                 deleteCartRule(rowData.id_cart_rule);
                            }
                        },
    ...
    public function getCartRuleRequest() {

        $cartRules = Db::getInstance()->executeS(
            (new DbQuery())
                ->select('a.*, b.*, case when a.`active` =1 then \'<div class="p-active"></div>\' else \'<div class="p-inactive"></div>\' end as `active`, a.`active` as enable')
                ->from('cart_rule', 'a')
                ->leftJoin('cart_rule_lang', 'b', 'b.`id_cart_rule` = a.`id_cart_rule` AND b.`id_lang` = ' . (int) $this->context->language->id)
                ->orderBy('a.`id_cart_rule` DESC')
        );
    ...
    public function ajaxProcessgetCartRuleRequest() {

        die(Tools::jsonEncode($this->getCartRuleRequest()));

    }

    public function getCartRuleFields() {
    ...
    public function ajaxProcessgetCartRuleFields() {

        die(EmployeeConfiguration::get('EXPERT_CARTRULES_FIELDS'));
    }
    ...
    public function ajaxProcessUpdateCartRule() {

        $id_cart_rule = Tools::getValue('id_cart_rule');
        $carRule = new CartRule($id_cart_rule);

        foreach ($_POST as $key => $value) {

            if (property_exists($carRule, $key) && $key != 'id_cart_rule') {
                $carRule->{$key}
                = $value;
            }
        ...
        $classVars = get_class_vars(get_class($carRule));
        $fields = [];
            ...
            if (array_key_exists('lang', $params) && $params['lang']) {
                ...
                foreach (Language::getIDs(false) as $idLang) {
                    ...
                    if (Tools::isSubmit($field . '_' . (int) $idLang)) {
                        ...
                        if (!isset($carRule->{$field}) || !is_array($carRule->{$field})) {
                            $carRule->{$field}
                            = [];
                        }
        ...
        $result = $carRule->update();

        if ($result) {
            $return = [
                'success' => true,
                'message' => $this->l('Le bon de réduction a été mis à jour avec succès'),
            ];
        } else {
            $return = [
                'success' => false,
                'message' => $this->l('Jeff a merdé'),
            ];
        }
    ...
    public function ajaxProcessAddNewCartRule() {
        ...
        if ($result) {
            $return = [
                'success' => true,
                'message' => $this->l('Le bon de réduction a été ajouté avec succès'),
            ];
        } else {
            $return = [
                'success' => false,
