...
class AdminCustomerPiecesControllerCore extends AdminController {
	...
	public function setAjaxMedia() {
		...
		return $this->pushJS([
			$this->admin_webpath . '/js/customerpieces.js',
			...
			$this->paramToolbar = [
				'items' => [
					...
					[
						'type'     => '\'button\'',
						'icon'     => '\'ui-icon-disk\'',
						'label'    => '\'' . $this->l('Corriger les écarts de numérotations') . '\'',
						'cls'      => '\'showFields changes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only\'',
	...
	public function getCustomerPiecesRequest() {
		...
		foreach ($orders as &$order) {
			...
			$order['deleteLink'] = $orderLink . '&id_customer_piece=' . $order['id_customer_piece'] . '&id_object=' . $order['id_customer_piece'] . '&deletecustomer_pieces&action=deleteObject&ajax=true';
	 ...
	 * @since 1.8.1.0
	 */
	public function printPDFIcons($idCustomerPieces, $tr) {

		static $validCustomerPiecesState = [];

		$order = new CustomerPieces($idCustomerPieces);

		if (!Validate::isLoadedObject($order)) {
			return '';
		}

		if (!isset($validCustomerPiecesState[$order->current_state])) {
			$validCustomerPiecesState[$order->current_state] = Validate::isLoadedObject($order->getCurrentCustomerPiecesState());
		}

		if (!$validCustomerPiecesState[$order->current_state]) {
			return '';
		}
	 ...
	 * @since 1.8.1.0
	 */
	public function printNewCustomer($idCustomerPieces, $tr) {
	 ...
	 *
	 * @param CustomerPieces|null $order
	 *
	 * @since 1.8.1.0
	 */
	...
	/**
	 * @param CustomerPiecesDetail $orderDetail
	 ...
	 *
	 * @param CustomerPiecesInvoice $orderInvoice
	 ...
	 *
	 * @param CustomerPiecesDetail       $orderDetail
	 * @param CustomerPieces             $order
	 * @param CustomerPiecesInvoice|null $orderInvoice
	 *
	 * @return void
	 *
	 * @since 1.8.1.0
	...
	public function ajaxProcessPrintPieceToPdf() {
		...
		if ($customerPiece->is_education) {
			$model = Configuration::get('PS_STUDENT_MODEL');
			$template = new InvoiceModel($model);
	...
	public function ajaxProcessCorrectNumberInvoice() {
		...
		$result = [
			'success' => true,
			'message' => $this->l('La numérotation des pièces ont été mises à jour avec succès'),
		];
