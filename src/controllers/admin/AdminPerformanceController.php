...
class AdminPerformanceControllerCore extends AdminController {
    ...
    public function postProcess() {
        ...
        if (Tools::isSubmit('EPH_PAGE_CACHE_IGNOREPARAMS')) {
        ...
        if ((bool) Tools::getValue('ciphering_up')) {
            // && Configuration::get('PS_CIPHER_ALGORITHM') != (int) Tools::getValue('PS_CIPHER_ALGORITHM')) {

            if ($this->tabAccess['edit'] === '1') {
                $algo = (int) Tools::getValue('PS_CIPHER_ALGORITHM');
                $prevSettings = file_get_contents(_PS_ROOT_DIR_ . '/app/settings.inc.php');
                ...
                if (!count($this->errors)) {
                    ...
                    if ($newSettings == $prevSettings || (
                        copy(_PS_ROOT_DIR_ . '/app/settings.inc.php', _PS_ROOT_DIR_ . '/app/settings.old.php')
                        && (bool) file_put_contents(_PS_ROOT_DIR_ . '/app/settings.inc.php', $newSettings)
                    )
                    ) {
        ...
        if (Tools::isSubmit('submitAddconfiguration')) {
            ...
            if (!empty($debugModeStatus)) {
                ...
                case static::DEBUG_MODE_ERROR_NO_DEFINITION_FOUND:
                    $this->errors[] = Tools::displayError(sprintf($this->l('Error: could not detect whether debug mode is enabled. Make sure that the correct permissions are set on the file %s'), _PS_ROOT_DIR_ . '/app/defines.inc.php'));
                    break;
                case static::DEBUG_MODE_ERROR_NO_WRITE_ACCESS:
                    $this->errors[] = Tools::displayError(sprintf($this->l('Error: could not write to file. Make sure that the correct permissions are set on the file %s'), _PS_ROOT_DIR_ . '/app/defines.inc.php'));
                    break;
                case static::DEBUG_MODE_ERROR_NO_WRITE_ACCESS_CUSTOM:
                    $this->errors[] = Tools::displayError(sprintf($this->l('Error: could not write to file. Make sure that the correct permissions are set on the file %s'), _PS_ROOT_DIR_ . '/app/defines_custom.inc.php'));
                    break;
                case static::DEBUG_MODE_ERROR_NO_READ_ACCESS:
                    $this->errors[] = Tools::displayError(sprintf($this->l('Error: could not read file. Make sure that the correct permissions are set on the file %s'), _PS_ROOT_DIR_ . '/app/defines.inc.php'));
                    break;
                default:
                    break;
                }
    ...
    public function isDebugModeEnabled() {
        ...
        if ($this->isDefinesReadable(true)) {
            $definesClean = php_strip_whitespace(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php');
        }
    ...
    public function isDefinesReadable($custom = false) {
        ...
        if ($custom) {
            return is_readable(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php');
        }
    ...
    public function enableDebugMode() {
        ...
        if ($this->isDefinesReadable(true)) {
            ...
            $definesCustomClean = php_strip_whitespace(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php');
            $definesCustom = file_get_contents(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php');
            ...
            if (!empty($definesCustomClean) && preg_match('/define\(\'_PS_MODE_DEV_\', ([a-zA-Z]+)\);/Ui', $definesCustomClean)) {
                ...
                if (!@file_put_contents(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php', $definesCustom)) {
                    return static::DEBUG_MODE_ERROR_NO_WRITE_ACCESS_CUSTOM;
                }

                if (function_exists('opcache_invalidate')) {
                    opcache_invalidate(_PS_ROOT_DIR_ . '/app/defines_custom.inc.php');
                }
