...
class AdminCustomersControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->extracss = $this->pushCSS([__PS_BASE_URI__ . $this->admin_webpath . '/themes/' . $this->bo_theme . '/css/student.css']);
	...
	public function setAjaxMedia() {
		...
		return $this->pushJS([
			__PS_BASE_URI__ . $this->admin_webpath . '/js/customer.js',
			__PS_BASE_URI__ . $this->admin_webpath . '/js/bank.js',
		]);
	}
...
';
                ...
                return {
                    ...
                    items: {
                        ...
                        "delete": {
                            ...
                            callback: function(itemKey, opt, e) {
                              deleteObject("' . $this->controller_name . '", "' . $this->className . '", "Supprimer un client", "Etes vous sure de vouloir supprimer "+rowData.firstname+" "+ rowData.lastname+ " ?", "Oui", "Annuler",rowData.id_customer);
                            }
                        },
                        "bulkdelete": {
	...
	public function getCustomerRequest($pq_curpage, $pq_rpp, $pq_filter, $pq_sort) {
		...
		$file = fopen("testgetCustomerRequest.txt", "w");
		$nbRecords = Db::getInstance()->getValue(
			(new DbQuery())
				->select('count( * )')
				->from($this->table)
		);
		fwrite($file, $pq_curpage . PHP_EOL);
		fwrite($file, $pq_rpp . PHP_EOL);
		...
		if (is_array($pq_filter) && count($pq_filter)) {
			...
			if ($pq_curpage > 1) {
				fwrite($file, 'pq_curpage >1' . PHP_EOL);
		...
		foreach ($customers as &$customer) {
			...
			$hasEducation = Customer::getLastStudentEducation($customer['id_customer']);
			...
			if ($hasEducation) {
				$customer['lastEducation'] = '<div>' . $hasEducation['name'] . '</div>';

			} else {
				$customer['lastEducation'] = '<div class="p-inactive"><i class="fa fa-times" aria-hidden="true" style="color:red;"></i></div>';
	...
	public function ajaxProcessgetCustomerRequest() {
		...
		$file = fopen("testProcessgetCustomerRequest.txt", "w");
	...
	public function ajaxProcessDeleteCustomer() {
		...
		$result = [
			'success' => true,
			'message' => 'Le client a été supprimé avec succès de la base de donnée.',
		];
	...
	public function ajaxProcessAddNewAjaxObject() {
		...
		if (!$address1 = Tools::getValue('address1')) {
			$return = [
				'success' => false,
				'message' => 'L‘adresse est obligatoire, merci de renseigner les champs.',
			];
			die(Tools::jsonEncode($return));
		}
		...
		if (!$birth_city = Tools::getValue('birth_city')) {
			$return = [
				'success' => false,
				'message' => 'La ville ou le pays de naissance sont obligatoires, merci de renseigner ce champs.',
			];
			die(Tools::jsonEncode($return));
		}
