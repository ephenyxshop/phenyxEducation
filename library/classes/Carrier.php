 ...
 * @since 1.9.1.0
 */
class CarrierCore extends PhenyxObjectModel {
    ...
    public static function getIdTaxRulesGroupByIdCarrier($idCarrier, Context $context = null) {
        ...
        $key = 'carrier_id_tax_rules_group_' . (int) $idCarrier;
    ...
    public static function getDeliveryPriceByRanges($rangeTable, $idCarrier) {
        ...
        return Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
                ...
                ->where('d.`id_' . bqSQL($rangeTable) . '` != 0 ')
     ...
     * @throws PhenyxShopException
     */
    public static function getAvailableCarrierList(Product $product, $idWarehouse, $idAddressDelivery = null, $idCompany = null, $cart = null, &$error = []) {
        ...
        $cacheId = 'Carrier::getAvailableCarrierList_' . (int) $product->id . '-' . (int) $idCompany;
        ...
        if (!empty($carriersForProduct)) {
            ...
            foreach ($carriersForProduct as $carrier) {
...
//no linked carrier are available for this zone
        ...
        if (!Cache::isStored($cacheId)) {
            $result = Db::getInstance(_EPH_USE_SQL_SLAVE_)->getValue(
                (new DbQuery())
                    ...
                    ->where('d.`id_carrier` = ' . (int) $this->id)
        ...
        foreach ($priceList as $values) {
            ...
            if ($delete) {
                Db::getInstance()->execute(
                    ...
                    WHERE  id_carrier=' . (int) $values['id_carrier'] .
    ...
    public function copyCarrierData($oldId) {
        ...
        if (!Validate::isUnsignedId($oldId)) {
            throw new PhenyxShopException('Incorrect identifier for carrier');
        }
                ...
                foreach ($res as $val) {
                    ...
                    Db::getInstance()->execute(
                        ...
                        INSERT INTO `' . _DB_PREFIX_ . 'delivery` (`id_carrier`,  `id_range_price`, `id_range_weight`, `id_zone`, `price`) (
                            SELECT ' . (int) $this->id . ',  ' . (int) $idRangePrice . ', ' . (int) $idRangeWeight . ', `id_zone`, `price`
    ...
    public function getRangeTable() {
        ...
        if ($shippingMethod === static::SHIPPING_METHOD_WEIGHT) {
            return 'range_weight';
        } else if ($shippingMethod === static::SHIPPING_METHOD_PRICE) {
            return 'range_price';
        }
    ...
    */
    public function getRangeObject($shippingMethod = false) {
        ...
        } else if ((int) $shippingMethod === static::SHIPPING_METHOD_PRICE) {
