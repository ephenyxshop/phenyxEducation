...
class MediaCore {
    ...
    public static function getJqueryPath($version = null, $folder = null, $minifier = true) {
        ...
        $return[] = Media::getJSPath(_EPH_JS_DIR_ . 'jquery/jquery-migrate-1.2.1.min.js');
    ...
    public static function getJqueryUIPath($component, $theme, $checkDependencies) {
        ...
        if (isset(Media::$jquery_ui_dependencies[$component]) && Media::$jquery_ui_dependencies[$component]['theme'] && $checkDependencies) {
            // @codingStandardsIgnoreEnd
            $themeCss = Media::getCSSPath($folder . _EPH_THEMES_DIR_ . $theme . '/jquery.ui.theme.css');
            $compCss = Media::getCSSPath($folder . _EPH_THEMES_DIR_ . $theme . '/jquery.' . $component . '.css');
        ...
        if ($checkDependencies && array_key_exists($component, static::$jquery_ui_dependencies)) {
            ...
            foreach (static::$jquery_ui_dependencies[$component]['dependencies'] as $dependency) {
                ...
                if (static::$jquery_ui_dependencies[$dependency]['theme']) {
                    $depCss = Media::getCSSPath($folder . _EPH_THEMES_DIR_ . $theme . '/jquery.' . $dependency . '.css');
                }
    ...
    public static function getJqueryPluginPath($name, $folder = null) {
        ...
        } else if (@file_exists($fileUri . $name . '/' . $file)) {
    ...
    public static function ieCssSplitter($compiledCss, $cachePath, $refresh = false) {
        ...
        if (!$refresh) {
            ...
            if (is_array($files)) {
                ...
                foreach (array_diff($files, ['..', '.']) as $file) {
                    $cssUrl = str_replace(_EPH_ROOT_DIR_, '', $protocolLink . Tools::getMediaServer('') . $cachePath . DIRECTORY_SEPARATOR . $file);
        ...
        foreach ($compiledCss as $css => $media) {
            ...
            $cssContent = file_get_contents(_EPH_ROOT_DIR_ . $fileInfo['path']);
            ...
            if (($count / $cssRuleLimit) > 0) {
                ...
                for ($i = $count; $i > 0; $i -= $cssRuleLimit) {
                    ...
                    $cssUrl = str_replace(_EPH_ROOT_DIR_, '', $protocolLink . Tools::getMediaServer('') . $cachePath . DIRECTORY_SEPARATOR . $newCssName);
