...
class CompanyCore extends PhenyxObjectModel {
    ...
    public function setUrl() {
        ...
        if (!Cache::isStored($cacheId)) {
            $row = Db::getInstance(_EPH_USE_SQL_SLAVE_)->getRow(
                (new DbQuery())
                    ->select('cu.physical_uri, cu.virtual_uri, cu.domain, cu.domain_ssl, cu.agent_url, t.id_theme, t.name, t.directory')
                    ->from('company', 'c')
	...
	public static function initialize() {
        ...
        $file = fopen("testCompanyInitialise.txt","w");
		if (!($idCompany = Tools::getValue('id_company')) || defined('_EPH_ROOT_DIR_')) {
            fwrite($file,"soit manque idcompany soir est definit la base".PHP_EOL);
			$foundUri = '';
			$isMainUri = false;
			$host = Tools::getHttpHost();
			$requestUri = rawurldecode($_SERVER['REQUEST_URI']);
            fwrite($file,"requestUri :".$requestUri.PHP_EOL);

			$result = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
				(new DbQuery())
					->select('c.`id_company`, CONCAT(cu.`physical_uri`, cu.`virtual_uri`) AS `uri`, cu.`domain`, cu.`agent_url`, cu.`main`')
					->from('company_url', 'cu')
					->leftJoin('company', 'c', 'c.`id_company` = cu.`id_company`')
					->where('cu.domain = \'' . pSQL($host) . '\' OR cu.domain_ssl = \'' . pSQL($host) . '\' OR cu.agent_url = \''.pSQL($host).'\'')
					->where('c.`active` = 1')
					->where('c.`deleted` = 0')
			...
			if ($through && $idCompany && !$isMainUri) {
                fwrite($file,"pas main uri".PHP_EOL);
				foreach ($result as $row) {
        ...
        fwrite($file,"httpHost :".$httpHost.PHP_EOL);
		if ((!$idCompany && defined('_EPH_ROOT_DIR_')) || Tools::isPHPCLI()) {
			// If in admin, we can access to the shop without right URL
            fwrite($file,"soit manque idcompany soir est definit la base 2".PHP_EOL);
            ...
            fwrite($file,"idcompany existe chic".PHP_EOL);
	...
	public function add($autoDate = true, $nullValues = false) {
        ...
        $langs = Language::getLanguages(false, $this->id);
    ...
    public static function cacheShops($refresh = false) {
        ...
        $sql = (new DbQuery())
            ...
            ->select('cu.`domain`, cu.`domain_ssl`, cu.`agent_url`,  cu.`physical_uri`, cu.`virtual_uri`')
            ->from('company', 'c')
