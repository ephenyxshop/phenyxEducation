...
class ManufacturerCore extends PhenyxObjectModel {
    ...
    public static function getProducts(
        ...
        $sql = (new DbQuery())
            ...
            ->select(Combination::isFeatureActive() ? 'pa.`minimal_quantity` AS `product_attribute_minimal_quantity`, IFNULL(pa.`id_product_attribute`,0) AS `id_product_attribute`' : '')
            ->select('pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`')
            ->select('pl.`meta_title`, pl.`name`, pl.`available_now`, pl.`available_later`, i.`id_image` AS `id_image`, il.`legend`, m.`name` AS `manufacturer_name`')
            ...
            ->leftJoin('image_lang', 'il', 'i.`id_image` = il.`id_image` AND il.`id_lang` = ' . (int) $idLang)
            ->leftJoin('manufacturer', 'm', 'm.`id_manufacturer` = p.`id_manufacturer`')
