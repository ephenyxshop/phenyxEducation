...
class GroupCore extends PhenyxObjectModel {
     ...
     * @throws PhenyxShopException
     */
    public static function addModulesRestrictions($idGroup, $modules, $companys = [1]) {

        if (!is_array($modules) || !count($modules) || !is_array($companys) || !count($companys)) {
            return false;
        }
     ...
     * @throws PhenyxShopException
     */
    public static function addRestrictionsForModule($idModule, $companys = [1]) {

        if (!is_array($companys) || !count($companys)) {
            return false;
        }
        ...
        foreach ($companys as $company) {
            ...
            (SELECT ' . (int) $idModule . ',  id_group FROM `' . _DB_PREFIX_ . 'group`)'
            );
        }
     ...
     * @throws PhenyxShopException
     */
    public function getCustomers($count = false, $start = 0, $limit = 0, $companyFiltering = false) {
