...
class CategoryCore extends PhenyxObjectModel {
     ...
     * @version 1.8.5.0
     */
    public static function getRootCategory($idLang = null, Company $company= null) {
        ...
        if (!$company) {
            ...
            return new Category($company->getCategory(), $idLang);
        }

        $isMoreThanOneRootCategory = count(Category::getCategoriesWithoutParent()) > 1;

        $category = new Category($company->getCategory(), $idLang);
    ...
    public static function setNewGroupForHome($idGroup) {
        ...
        try {
            return Db::getInstance()->insert(
                'category_group',
                [
                    'id_category' => (int) Context::getContext()->company->getCategory(),
     ...
     * @throws PhenyxShopException
     */
    public static function inShopStatic($idCategory, Company $company= null) {

        if (!$company|| !is_object($company)) {
            $company= Context::getContext()->company;
        }

        if (!$interval = Category::getInterval($company->getCategory())) {
            return false;
        }
    ...
    public static function movePositionatTop($idCategoryParent) {
        ...
        foreach($childrens as $child) {
            $sql = 'UPDATE `' . _DB_PREFIX_ . 'category` SET position = '.$k. ' WHERE id_category = '.$child['id_category'];
            Db::getInstance(_EPH_USE_SQL_SLAVE_)->execute($sql);
            $category = new Category($child['id_category']);
            $k++;
        }
    }
    ...
    public function add($autoDate = true, $nullValues = false) {
        ...
        if (isset($this->wayAdd)) {
            if ($this->wayAdd == 'up') {
                Category::movePositionatTop((int) $this->id_parent);
                $this->position = 0;
            } else if ($this->wayAdd == 'down') {                
                $this->position = (int) Category::getLastPosition((int) $this->id_parent);
            }
     ...
     * @throws PhenyxShopException
     */
    public function inShop(Shop $company= null) {
