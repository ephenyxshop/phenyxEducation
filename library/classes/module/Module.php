...
abstract class ModuleCore {
    ...
    const CACHE_FILE_TAB_MODULES_LIST = '/app/xml/tab_modules_list.xml';
    ...
    protected $_lang = [];
    /** @var string Module web path (eg. '/shop/plugins/modulename/') */
    protected $_path = null;
    /** @var string Module local path (eg. '/home/prestashop/plugins/modulename/') */
    ...
    public function __construct($name = null, Context $context = null) {
        ...
        if (strlen($this->eph_versions_compliancy['min']) == 3) {
            $this->ephs_versions_compliancy['min'] .= '.0.0';
        }
        ...
        if ($this->name != null) {
            ...
            if (static::$modules_cache == null && !is_array(static::$modules_cache)) {
                ...
                foreach ($result as $row) {
                    ...
                    static::$modules_cache[$row['name']]['active'] = $row['active'];
                }
     ...
     * @since 2.1.0.0
     */
    public static function getInstanceByName($moduleName, $full = true) {
        ...
        if (!isset(static::$_INSTANCE[$moduleName])) {
            ...
            return Module::coreLoadModule($moduleName, $full);
        }
     ...
     * @since 2.1.0.0
     */
    protected static function coreLoadModule($moduleName, $full) {
    ...
    public static function getModuleNameFromClass($currentClass) {
        ...
        // i.e. in plugins/[module name]/[iso_code].php
    ...
    public static function getModulesOnDisk($useConfig = false, $loggedOnAddons = false, $idEmployee = false) {
        ...
        $result = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
                ->select('m.id_module, m.`name`, m.`version`, m.active, m.enable_device, mp.`interest`')
                ->from('module', 'm')
        ...
        if (!empty($moduleNameList)) {
            ...
            $results = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
                (new DbQuery())
                    ->select('m.`id_module`, m.`name`, m.active, COUNT(m.`id_module`) AS `total`')
                    ->from('module', 'm')
            ...
            foreach ($results as $result) {
                ...
                if (isset($modulesNameToCursor[mb_strtolower($result['name'])])) {
                    ...
                    $moduleCursor->active = $result['active'];
                }
        ...
        foreach ($moduleList as $key => &$module) {
            require_once _EPH_MODULE_DIR_ . $module->name . '/' . $module->name . '.php';
            $tmpModule = Adapter_ServiceLocator::get($module->name);
            if (isset($modulesInstalled[$module->name])) {
                if(method_exists($tmpModule, 'reset')) {
                    $module->has_reset = true;
                } else {
                    $module->has_reset = false;
                }
                $module->id = $modulesInstalled[$module->name]['id_module'];
                ...
                $module->active = $modulesInstalled[$module->name]['active'];
            } else {
                //$module->id = $modulesInstalled[$module->name]['id_module'];
    ...
    public static function getPaymentModules() {
        ...
        if (isset($context->employee)) {
            $frontend = false;
        } else if (isset($context->customer) && $useGroups) {
        ...
        return Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
                ...
                ->from('module', 'm')
                ->join($frontend ? 'LEFT JOIN `' . _DB_PREFIX_ . 'module_country` mc ON (m.`id_module` = mc.`id_module`)' : '')
                ->join($frontend && $useGroups ? 'INNER JOIN `' . _DB_PREFIX_ . 'module_group` mg ON (m.`id_module` = mg.`id_module`' . ')' : '')
    ...
    public function install() {
        ...
        if (!Validate::isModuleName($this->name)) {
            $return = [
                'success' => false,
                'message' => Tools::displayError('Unable to install the module (Module name is not valid).'),
            ];
    ...
    public function installOverrides() {
        ...
        foreach (Tools::scandir($this->getLocalPath() . 'override', 'php', '', true) as $file) {
            ...
            if (PhenyxShopAutoload::getInstance()->getClassPath($class . 'Core') || Module::getModuleIdByName($class)) {
    ...
    public function addOverride($classname) {
        ...
        $origPath = $path = PhenyxShopAutoload::getInstance()->getClassPath($classname . 'Core');
    ...
    public function removeOverride($classname) {
        ...
        if ($origPath && !$file = PhenyxShopAutoload::getInstance()->getClassPath($classname)) {
            return true;
        } else if (!$origPath && Module::getModuleIdByName($classname)) {
            $path = 'includes/plugins' . DIRECTORY_SEPARATOR . $classname . DIRECTORY_SEPARATOR . $classname . '.php';
        }
        ...
        if ($origPath) {
            ...
            for ($i = 0; $i < $count; ++$i) {
                ...
                } else if (preg_match('/(^\s*\/\*)/i', $overrideFile[$i])) {
     ...
     * @param array $companyList List of shop
     *
     * @return bool result
     *
     * @since 2.1.0.0
     */
    public function unregisterHook($hookId, $companyList = null) {
        ...
        $result = Db::getInstance()->delete(
            'hook_module',
            '`id_module` = ' . (int) $this->id . ' AND `id_hook` = ' . (int) $hookId
        );

        // Clean modules position
        $this->cleanPositions($hookId, $companyList);
     ...
     * @since 2.1.0.0
     */
    public function cleanPositions($idHook, $companyList = null) {
        ...
        foreach ($results as $row) {
            ...
            Db::getInstance()->update(
                ...
                '`id_hook` = ' . (int) $idHook . ' AND `id_module` = ' . $row['id_module']
            );
            $position++;
        }
    ...
    public function enableDevice($device)
    {
        Db::getInstance()->update(
            ...
            '(`enable_device` &~ '.(int) $device.' OR `enable_device` = 0) AND `id_module` = '.(int) $this->id
        );
    ...
    public function disableDevice($device)
    {
        Db::getInstance()->update(
            ...
            'enable_device & '.(int) $device.' AND id_module='.(int) $this->id
        );
     ...
     * @param array           $companyList List of shop linked to the hook (if null, link hook to all shops)
     *
     * @return bool result
     * @throws PhenyxShopException
     */
    ...
    public static function getPermissionStatic($idModule, $variable, $employee = null) {
        ...
        if (!isset(static::$cache_permissions[$employee->id_profile][$idModule])) {
            throw new PhenyxShopException('No access reference in table module_access for id_module ' . $idModule . '.');
        }
