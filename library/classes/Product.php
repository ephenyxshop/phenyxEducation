 ...
 * @since 1.8.1.0
 */
class ProductCore extends PhenyxObjectModel {
    ...
    public static function priceCalculation(
        ...
        if ($idShop !== null && $context->company->id != (int) $idShop) {
            $context->company = new Shop((int) $idShop);
        }
    ...
    public static function getIdTaxRulesGroupByIdProduct($idProduct, Context $context = null) {
        ...
        $key = 'product_id_tax_rules_group_' . (int) $idProduct . '_' . (int) $context->company->id;
        ...
        if (!Cache::isStored($key)) {
            $result = Db::getInstance(_EPH_USE_SQL_SLAVE_)->getValue(
                '
                            ...
                            WHERE `id_product` = ' . (int) $idProduct . ' AND id_shop=' . (int) $context->company->id
            );
    ...
    public static function getNewProducts($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = null, $orderWay = null, Context $context = null) {
        ...
        if ($count) {
            $sql = 'SELECT COUNT(p.`id_product`) AS nb
                    FROM `' . _DB_PREFIX_ . 'product` p
                    ' . Shop::addSqlAssociation('product', 'p') . '
                    WHERE product_shop.`active` = 1 AND p.is_education < 1
        ...
        $sql->leftJoin('image_shop', 'image_shop', 'image_shop.`id_product` = p.`id_product` AND image_shop.cover=1 AND image_shop.id_shop=' . (int) $context->company->id);
        $sql->leftJoin('image_lang', 'il', 'image_shop.`id_image` = il.`id_image` AND il.`id_lang` = ' . (int) $idLang);
        $sql->leftJoin('manufacturer', 'm', 'm.`id_manufacturer` = p.`id_manufacturer`');

        $sql->where('product_shop.`active` = 1');
        $sql->where('p.is_education < 1');
        ...
        if (Combination::isFeatureActive()) {
            ...
            $sql->leftJoin('product_attribute_shop', 'product_attribute_shop', 'p.`id_product` = product_attribute_shop.`id_product` AND product_attribute_shop.`default_on` = 1 AND product_attribute_shop.id_shop=' . (int) $context->company->id);
        }
    ...
    public static function getEducationProducts($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = null, $orderWay = null, Context $context = null) {
        ...
        if ($count) {
            $sql = 'SELECT COUNT(p.`id_product`) AS nb
                    ...
                    ' . Shop::addSqlAssociation('product', 'p') . '
                    WHERE product_shop.`active` = 1 AND p.is_education = 1
                    AND product_shop.`date_add` > "' . date('Y-m-d', strtotime('-' . (Configuration::get('EPH_NB_DAYS_EDUCATION') ? (int) Configuration::get('EPH_NB_DAYS_EDUCATION') : 20) . ' DAY')) . '"
                    ' . ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '') . '
                    ' . $sqlGroups;
        ...
        $sql->select(
            'p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`,
            pl.`meta_keywords`, pl.`meta_title`, pl.`name`, pl.`available_now`, pl.`available_later`, image_shop.`id_image` id_image, il.`legend`, m.`name` AS manufacturer_name,
            product_shop.`date_add` > "' . date('Y-m-d', strtotime('-' . (Configuration::get('EPH_NB_DAYS_EDUCATION') ? (int) Configuration::get('EPH_NB_DAYS_EDUCATION') : 20) . ' DAY')) . '" as new'
        );
        ...
        $sql->where('product_shop.`date_add` > "' . date('Y-m-d', strtotime('-' . (Configuration::get('EPH_NB_DAYS_EDUCATION') ? (int) Configuration::get('EPH_NB_DAYS_EDUCATION') : 20) . ' DAY')) . '"');
    ...
    public static function getProductProperties($idLang, $row, Context $context = null) {
        ...
        if (!isset($row['id_category_default']) && $row['id_category_default']) {
            ...
            if (!$row['id_category_default']) {
                $row['id_category_default'] = Context::getContext()->company->id_category;
            }
        ...
        $row['price_tax_exc'] = Product::getPriceStatic(
            ...
            false,
            $idProductAttribute,
            (static::$_taxCalculationMethod == EPH_TAX_EXC ? 2 : 6)
        );

        if (static::$_taxCalculationMethod == EPH_TAX_EXC) {
            ...
            $row['price'] = Tools::ps_round(
                ...
                (int) Configuration::get('EPH_PRICE_DISPLAY_PRECISION')
            );
            $row['price_without_reduction'] = Product::getPriceStatic(
                (int) $row['id_product'],
                true,
        ...
        if ($productReductions) {
            ...
            $sql = 'SELECT product_shop.id_product, IFNULL(product_attribute_shop.id_product_attribute,0) id_product_attribute
                    ...
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_shop` product_attribute_shop
                        ON (p.`id_product` = product_attribute_shop.`id_product` AND product_attribute_shop.`default_on` = 1 AND product_attribute_shop.id_shop=' . (int) $context->company->id . ')
            ...
            $sql = 'SELECT p.*, product_shop.*, stock.`out_of_stock` out_of_stock, pl.`description`, pl.`description_short`,
                    ...
                    LEFT JOIN `' . _DB_PREFIX_ . 'image_shop` image_shop
                        ON (image_shop.`id_product` = p.`id_product` AND image_shop.cover=1 AND image_shop.id_shop=' . (int) $context->company->id . ')
    ...
    public static function getPricesDrop(
        ...
        LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_shop` product_attribute_shop
            ON (p.`id_product` = product_attribute_shop.`id_product` AND product_attribute_shop.`default_on` = 1 AND product_attribute_shop.id_shop=' . (int) $context->company->id . ')
        ' . Product::sqlStock('p', 0, false, $context->company) . '
        LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (
            p.`id_product` = pl.`id_product`
            AND pl.`id_lang` = ' . (int) $idLang . Shop::addSqlRestrictionOnLang('pl') . '
        )
    ...
    public static function isDiscounted($idProduct, $quantity = 1, Context $context = null) {
        ...
        return (bool) SpecificPrice::getSpecificPrice((int) $idProduct, $context->company->id, $idCurrency, $idCountry, $idGroup, $quantity, null, 0, 0, $quantity);
    }
    ...
    protected static function _getCustomizationFieldsNLabels($productId, $idShop = null) {
        ...
        if (Shop::isFeatureActive() && !$idShop) {
            $idShop = (int) Context::getContext()->company->id;
        }
    ...
    public static function resetEcoTax() {
        ...
        return PhenyxObjectModel::updateMultishopTable(
            'product',
            [
                'ecotax' => 0,
            ]
    ...
    public static function getRealQuantity($idProduct, $idProductAttribute = 0, $idWarehouse = 0, $idShop = null) {
        ...
        if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && Product::usesAdvancedStockManagement($idProduct) &&
    ...
    public static function getColorsListCacheId($idProduct, $full = true) {
        ...
        if ($full) {
            $cacheId .= '|' . (int) Context::getContext()->company->id . '|' . (int) Context::getContext()->cookie->id_lang;
        }
    ...
    /**
     * @see PhenyxObjectModel::getFieldsShop()
     * @return array
     *
     * @since   1.8.1.0
     * @version 1.8.1.0 Initial version
    ...
    /**
     * @see PhenyxObjectModel::validateField()
     *
     * @param string $field
     * @param mixed  $value
     * @param null   $idLang
    ...
    public function productAttributeExists($attributesList, $currentProductAttribute = false, Context $context = null, $allShops = false, $returnId = false) {
        ...
        $result = Db::getInstance()->executeS(
            ...
            WHERE 1 ' . (!$allShops ? ' AND pas.id_shop =' . (int) $context->company->id : '') . ' AND pa.`id_product` = ' . (int) $this->id .
    ...
    public function update($nullValues = false) {
        ...
        if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && StockAvailable::dependsOnStock($this->id, Context::getContext()->company->id)) {
            Db::getInstance()->update(
                'stock',
                [
        ...
        if ($this->getType() == Product::PTYPE_VIRTUAL && $this->active && !Configuration::get('EPH_VIRTUAL_PROD_FEATURE_ACTIVE')) {
    ...
    public function addCombinationEntity(
        ...
        $result = PhenyxObjectModel::updateMultishopTable(
            'Combination',
            [
    ...
    */
    ...
    public static function getCover($idProduct, Context $context = null) {
        ...
        $cacheId = 'Product::getCover_' . (int) $idProduct . '-' . (int) $context->company->id;
    ...
    public function setCoverWs($idImage) {
        ...
        Db::getInstance()->execute(
            ...
            AND image_shop.id_shop=' . (int) Context::getContext()->company->id
        );
        Db::getInstance()->execute(
    ...
    public function setAdvancedStockManagement($value) {
        ...
        if (Context::getContext()->company->getContext() == Shop::CONTEXT_GROUP && Context::getContext()->company->getContextShopGroup()->share_stock == 1) {
            Db::getInstance()->execute(
                '
    ...
    public function getDefaultCategory() {
        ...
        if (!$defaultCategory) {
            return ['id_category_default' => Context::getContext()->company->id_category];
        } else {
    ...
    */
    ...
    public function modifierWsLinkRewrite() {
        ...
        foreach ($this->name as $idLang => $name) {
            ...
            } else if (!Validate::isLinkRewrite($this->link_rewrite[$idLang])) {
