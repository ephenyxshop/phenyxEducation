...
class HelperListCore extends Helper {
    ...
    public function generateList($list, $fieldsDisplay) {
        ...
        $this->orderBy = !is_null($this->orderBy) ? preg_replace('/^([a-z _]*!)/Ui', '', $this->orderBy) : null;
        $this->orderWay = !is_null($this->orderWay) ? preg_replace('/^([a-z _]*!)/Ui', '', $this->orderWay) : null;
...
//        $order = '';
        ...
        $selectedPagination = Tools::getValue(
            ...
            isset($this->context->cookie->{$this->list_id . '_pagination'}) ? $this->context->cookie->{$this->list_id . '_pagination'}

            : $this->_default_pagination
        );
        ...
        $this->header_tpl->assign(
            array_merge(
                [
                    ...
                    'id_cat'            => $idCat,
                    'shop_link_type'    => $this->companyLinkType,
        ...
        if (isset($this->fields_list['position'])) {
            ...
            if ($this->position_identifier) {
                ...
                if (isset($this->position_group_identifier)) {
                    ...
                    $positionGroupIdentifier = (int) Tools::getValue('id_' . ($this->is_cms ? 'cms_' : '') . 'category', ($this->is_cms ? '1' : ''));
                }
        ...
        foreach ($this->_list as $index => $tr) {
            ...
            foreach ($this->actions as $action) {
                ...
                if (!array_key_exists($action, $this->list_skip_actions) || !in_array($id, $this->list_skip_actions[$action])) {
                    ...
                    if (method_exists($this->context->controller, $methodName)) {
                        $this->_list[$index][$action] = $this->context->controller->$methodName($id, $this->token, $name);
                    } else
                    if ($this->module instanceof Module && method_exists($this->module, $methodName)) {
                        $this->_list[$index][$action] = $this->module->$methodName($id, $this->token, $name);
                    } else
                    if (method_exists($this, $methodName)) {
                        $this->_list[$index][$action] = $this->$methodName($id, $this->token, $name);
                    }
                ...
                if ($isFirst && isset($this->_list[$index][$action])) {
                    ...
                    if (!preg_match('/a\s*.*class\s*=\s*\".*btn.*\"/', $this->_list[$index][$action])) {
                        $this->_list[$index][$action] = preg_replace(
                            '/a(\s*.*)class\s*=\s*\"(.*)\"/',
            ...
            foreach ($this->fields_list as $key => $params) {
                ...
                if (isset($params['active'])) {
                    ...
                    if (isset($this->module) && method_exists($this->module, 'displayEnableLink')) {
                ...
                if (isset($params['image'])) {
                    ...
                    if ($params['image'] != 'p' || Configuration::get('EPH_LEGACY_IMAGES')) {
                ...
                if (isset($params['type']) && $params['type'] == 'float') {
     ...
     * @version 1.8.5.0
     */
    public function displayEnableLink($id, $token, $value, $active, $idCategory = null, $idProduct = null, $ajax = false) {
     ...
     * @throws SmartyException
     */
    public function displayDuplicateLink($id, $token = null, $name = null) {
     ...
     * @throws SmartyException
     */
    public function displayDetailsLink($id, $token = null, $name = null) {
     ...
     * @throws SmartyException
     */
    public function displayViewLink($id, $token = null, $name = null) {
    ...
    public function displayEditLink($id, $token = null, $name = null) {
        ...
        $tpl->assign(
            [
                'href'   => $this->currentIndex . '&' . $this->identifier . '=' . $id . '&update' . $this->table . ($this->page && $this->page > 1 ? '&page=' . (int) $this->page : '') . '&action=viewAjax&token=' . ($token != null ? $token : $this->token),
     ...
     * @version 1.8.5.0
     */
    public function displayDeleteLink($id, $token = null, $name = null) {
     ...
     * @version 1.8.5.0
     */
    public function displayDefaultLink($id, $token = null, $name = null) {
