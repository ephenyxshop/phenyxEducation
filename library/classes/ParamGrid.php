...
class ParamGridCore {
	...
	public function generateParaGridOption() {
        ...
        $this->getHeightModel = 'getHeightModel() {
            var p = $("#grid_'.$this->paramController.'").offset();
			return screenHeight = $(window).height() -parseInt(p.top);
		}';

		$this->heightModel = (!empty($this->heightModel)) ? $this->heightModel : 'getHeightModel()';

		$this->dataModel = (!empty($this->dataModel)) ? $this->dataModel : $this->paramController . 'ModelData';
		...
		$this->paragrid_option['paragrids'][] = (!empty($this->paragrid_option)) ? $this->paragrid_option : [
			...
			'builder'               => [
				'height'         => (!empty($this->heightModel)) ? $this->heightModel : 'getHeightModel()',
				'width'          => '\'' . $this->width . '\'',
	...
	public function generateParagridScript() {
		...
		foreach ($this->paragrid_option['paragrids'] as $key => $value) {
			...
			if (empty($this->recIndx)) {
				...
				$jsScript .= 'var ' . $this->paramController . 'ModelData = ' . $this->requestModel . ';' . PHP_EOL;
                $jsScript .= 'function ' . $this->getHeightModel . ';' . PHP_EOL;
