...
class PackCore extends Product {
        ...
        foreach ($result as $row) {
            ...
            if (isset($row['id_product_attribute_item']) && $row['id_product_attribute_item']) {
                ...
                foreach ($combinations as $k => $combination) {
                    $p->name[$idLang] .= ' ' . $combination['group_name'] . '-' . $combination['attribute_name'];
                }
    ...
    public static function getItemTable($idProduct, $idLang, $full = false) {
        ...
        $sql = 'SELECT p.*,  pl.*, i.`id_image` id_image, il.`legend`, cl.`name` AS category_default, a.quantity AS pack_quantity, p.`id_category_default`, a.id_product_pack, a.id_product_attribute_item
                ...
                WHERE a.`id_product_pack` = ' . (int) $idProduct . '
    ...
    public static function getPacksTable($idProduct, $idLang, $full = false, $limit = null) {
        ...
        $sql = '
        SELECT p.*,  pl.*, i.`id_image` id_image, il.`legend`, IFNULL(pa.id_product_attribute, 0) id_product_attribute
