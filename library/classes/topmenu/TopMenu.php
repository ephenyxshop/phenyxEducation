    ...
    public static function getMenus($id_lang, $active = true, $get_from_all_shops = false, $groupRestrict = false) {
        ...
        $sql_grouEPH_where = '';
         ...
         $sql = 'SELECT atp.*, atpl.*,
				...
				if($active) {
					$sql .= 'WHERE atp.`active` = 1 AND (atp.`active_desktop` = 1 || atp.`active_mobile` = 1) ';
				} else {
					$sql .= 'WHERE 1 ';
				}
                 
                $sql .= 'AND atp.`id_cms` = 0 AND atp.`id_cms_category` = 0
        ...
        if (is_array($menus) && count($menus)) {
            ...
            foreach ($menus as &$menu) {
                ...
                $menu['outPutNameValue'] = self::getAdminOutputNameValue($menu, true);
        ...
        if ($row['type'] == 7) {
            ...
            if ($withExtra && trim($row['have_icon'])) {
                ...
                if (in_array($row['image_type'], ['i-fa', 'i-mi'])) {
                    ...
                    $return .= '<img src="' . _EPH_IMG_ . $type . '_icons/' . $row['id_topmenu_' . $type] . '-' . $_iso_lang . '.' . ($row['image_type'] ?: 'jpg') . '" alt="' . htmlentities($row['name'], ENT_COMPAT, 'UTF-8') . '" title="' . htmlentities($row['name'], ENT_COMPAT, 'UTF-8') . '" class="iconOnly" />';
                }
