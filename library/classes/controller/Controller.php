...
abstract class ControllerCore {
        ...
        if ($this->checkAccess()) {
            ...
            if ($this->ajax) {
                ...
                if (!empty($action) && method_exists($this, 'displayAjax' . $action)) {
                    ...
                    ();
                } else if (method_exists($this, 'displayAjax')) {
                    $this->displayAjax();
                }
    ...
    protected function smartyOutputContent($content) {
        ...
        if (in_array($this->controller_type, ['front', 'modulefront']) && !empty($html) && $this->getLayout()) {
            ...
            echo ((!isset($this->ajax) || !$this->ajax) ? '</body></html>' : '');
        } else {
            echo $html;
        }
        ...
        if (is_array($jsUri)) {
            ...
            foreach ($jsUri as $jsFile) {
                ...
                if (defined('_EPH_CEF_URL_')) {
                    $jsPath = 'https://' .$context->company->domain_ssl.$jsPath;
                }
    ...
    public function addCSS($cssUri, $cssMediaType = 'all', $offset = null, $checkPath = true) {
        ...
        $file = fopen("testaddCSS.txt","a");
        $context = Context::getContext();
        foreach ($cssUri as $cssFile => $media) {

            if (is_string($cssFile) && strlen($cssFile) > 1) {
                fwrite($file,$cssFile.PHP_EOL);
                if (defined('_EPH_CEF_URL_')) {
                    $cssFile = Tools::str_replace_once(_EPH_ROOT_DIR_, '', $cssFile);
                    $cssFile = 'https://' .$context->company->domain_ssl.$cssFile;                
                ...
                fwrite($file,print_r($cssFile, true).PHP_EOL);
                if ($checkPath) {
                    $cssPath = Media::getCSSPath($media, $cssMediaType);
                } else {
                    $cssPath = [$media => is_string($cssMediaType) ? $cssMediaType : 'all'];
