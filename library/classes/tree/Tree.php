...
class TreeCore
{
    ...
    {
        if (!is_object($value)) {
            throw new PhenyxShopException('Toolbar must be a class object');
        }

        $reflection = new ReflectionClass($value);

        if (!$reflection->implementsInterface('ITreeToolbarCore')) {
            throw new PhenyxShopException('Toolbar class must implements ITreeToolbarCore interface');
        }
    ...
    {
        //Adding tree.js
        $adminWebpath = str_ireplace(_EPH_CORE_DIR_, '', _EPH_ROOT_DIR_);
        ...
        $jsPath = '/js/tree.js?v='._EPH_VERSION_;
