...
class AdminAgendaControllerCore extends AdminController {
    ...
    public function __construct() {
		...
		$this->available_providers = Employee::getEmployees();
		
		$this->available_cef = SaleAgent::getSaleAgents();
		...
		Configuration::updateValue('EPH_TIMEZONE', Tools::jsonEncode($this->timezones));

        $this->extracss = $this->pushCSS([
            _EPH_ROOT_DIR_ . '/js/calendar/bootstrap.min.css',
            _EPH_ROOT_DIR_ . '/js/trumbowyg/ui/trumbowyg.min.css',
            _EPH_ROOT_DIR_ . '/js/select2/select2.min.css',
            _EPH_ROOT_DIR_ . _EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/calendar/backend.min.css',
            _EPH_ROOT_DIR_ . _EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/calendar/general.css',
            _EPH_ROOT_DIR_ . '/js/jquery-fullcalendar/fullcalendar.min.css',
        ]);
    }
    ...
    public function setAjaxMedia() {
        ...
        return $this->pushJS([
            _EPH_ROOT_DIR_ . _EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/calendar/bootstrap/js/bootstrap.bundle.min.js',
            _EPH_ROOT_DIR_ . '/js/popper/popper.min.js',
            _EPH_ROOT_DIR_ . '/js/tippy/tippy-bundle.umd.min.js',
            _EPH_ROOT_DIR_ . '/js/jquery-ui/jquery-ui.touch-punch.min.js',
            _EPH_ROOT_DIR_ . '/js/moment/moment.min.js',
            _EPH_ROOT_DIR_ . '/js/moment/moment-timezone-with-data.min.js',
            _EPH_ROOT_DIR_ . '/js/datejs/date.min.js',
            _EPH_ROOT_DIR_ . '/js/trumbowyg/trumbowyg.min.js',
            _EPH_ROOT_DIR_ . '/js/select2/select2.min.js',
            _EPH_ROOT_DIR_ . '/js/jquery-fullcalendar/fullcalendar.min.js',
            _EPH_ROOT_DIR_ . '/js/jquery-jeditable/jquery.jeditable.min.js',
            _EPH_ROOT_DIR_ . '/js/jquery-ui/jquery-ui-timepicker-addon.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/working_plan_exceptions_modal.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_default_view.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_table_view.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_google_sync.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_appointments_modal.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_unavailability_events_modal.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend_calendar_api.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/backend.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/polyfill.min.js',
            _EPH_ROOT_DIR_ . '/js/calendar/general_functions.min.js',
        ]);
    }
    ...
    public function ajaxProcessOpenTargetController() {
        ...
        $jsDef = [
            ...
            'availableSaleagent'  => $this->available_cef,
		 ...
		 $html_select = '';
			$html_select .= '<option value="0">Tous</option>';
		foreach($this->available_providers as $employee) {
			 $html_select .= '<option value="' . $employee['id_employee'] . '">' . $employee['firstname'] . ' '. $employee['lastname'].'</option>';
		}
        ...
        $li = '<li id="uper' . $targetController . '" data-controller="AdminDashboard"><a href="#content' . $targetController . '">Agenda</a><button type="button" class="close tabdetail" data-id="uper' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
    ...
    public function ajaxProcessGetCalendarAppointment() {
        ...
        try
        {
            ...
            $filter_type = Tools::getValue('filter_type');
			 fwrite($file,$filter_type . PHP_EOL);
            ...
            if ($filter_type == 'employee') {
                ...
                $query->where('id_employee = ' . Tools::getValue('record_id'));
            } else {

                $query->where('id_sale_agent = ' . Tools::getValue('record_id'));
            }
            ...
            foreach ($response['appointments'] as &$appointment) {
				...
				if ($filter_type == 'employee') {
                	$appointment['provider'] = Employee::getAppointmentEmployee($appointment['id_employee']);			
				} else {
					$appointment['provider'] = SaleAgent::getAppointmentSaleAgent($appointment['id_sale_agent']);	
				}
	...
	public function ajaxProcessSaveAppointment() {
		...
		if($result) {
			...
			$result = [
        		'success' => true,
        		'message' => $this->la('La réservation a été ajoutée avec succès'),
        	];
				} else {
					$result = [
               			'success' => false,
               			'message' => $this->la('Nous avons rencontré une erreur lors de la création de la réservation'),
            		];
				}
	...
	public function ajaxProcessUpdateAppointment() {
		...
		if($result) {
			...
			$result = [
        		'success' => true,
        		'message' => $this->la('La réservation a été mise à jour avec succès'),
        	];
				} else {
					$result = [
               			'success' => false,
               			'message' => $this->la('Nous avons rencontré une erreur lors de la mise à jour de la réservation'),
            		];
				}
