...
class AdminProductsControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->lang = true;
		$this->publicName = $this->la('Products');

		if (!Tools::getValue('id_product')) {
			$this->multishop_context_group = false;
		}

		$this->availableSelector = '<div class="pq-theme"><select id="availableSelect" class="selectmenu"><option value="">' . $this->la('--Select--') . '</option><option value="0">' . $this->la('Disable') . '</option><option value="1">' . $this->la('Enable') . '</option></select></div>';

		

		$this->taxSelector .= '</select></div>';

		$this->taxSelector = '<div class="pq-theme"><select id="taxSelect" class="selectmenu"><option value="">' . $this->la('--Select--') . '</option>';
		...
		$this->available_tabs_lang = [
			'Informations'   => $this->la('Information'),
			'Pack'           => $this->la('Pack'),
			'VirtualProduct' => $this->la('Virtual Product'),
			'Prices'         => $this->la('Prices'),
			'Seo'            => $this->la('SEO'),
			'Images'         => $this->la('Images'),
			'Associations'   => $this->la('Associations'),
			'Shipping'       => $this->la('Shipping'),
			'Combinations'   => $this->la('Combinations'),
			'Features'       => $this->la('Features'),
			'Customization'  => $this->la('Customization'),
			'Attachments'    => $this->la('Attachments'),
			'Quantities'     => $this->la('Quantities'),
			'Accounting'     => $this->la('Book Keeping'),
			'Suppliers'      => $this->la('Suppliers'),
		];
		...
		$this->available_tabs =  [
			'Informations'   => [
                'position' => 0,
                'desc'     => $this->la('General Information')
            ],
            'Prices'         => [
                'position' => 1,
                'desc'     => $this->la('General Information')
	...
	public function generateParaGridScript() {
		...
		$this->paramToolbar = [
			'items' => [
				[
					'type'     => '\'button\'',
					'icon'     => '\'ui-icon-disk\'',
					'label'    => '\'' . $this->la('Filter by Categories') . '\'',
				...
				[
					'type'     => '\'button\'',
					'icon'     => '\'ui-icon-disk\'',
					'label'    => '\'' . $this->la('Réinitialiser le filtre par catégorie') . '\'',
				...
				[
					'type'  => '\'button\'',
					'icon'  => '\'ui-icon-disk\'',
					'label' => '\'' . $this->la('Gérer les champs affiché') . '\'',
					'cls'   => '\'showCategory changes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only\'',
		...
		$this->paramTitle = '\'' . $this->la('Gérer mes produits') . '\'';
		$this->dropOn = 1;
		$this->dragOn = 1;
			...
			'#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        ...
                        "edit": {
                            name : \'' . $this->la('Edit the Product: ') . '\'' . '+rowData.reference,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
						...
						"image": {
                            name : \'' . $this->la('Ajouter une image pour: ') . '\'' . '+rowData.reference,
                            icon: "image",
                            callback: function(itemKey, opt, e) {
                        ...
                        "duplicate": {
                            name : \'' . $this->la('Dupliquer ce produit : ') . '\'' . '+rowData.reference,
                            icon: "copy",
                            callback: function(itemKey, opt, e) {
                        ...
                        "unselect": {
                            name: \'' . $this->la('Unselect all item') . '\',
                            icon: "list-ul",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 2) {
                        ...
                        "bulkdiscount": {
                            name : \'' . $this->la('Apply bulk discount on selected products ') . '\' ,
                            icon: "tags",
                            callback: function(itemKey, opt, e) {
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Delete the product:') . '\'' . '+rowData.reference,
                            icon: "delete",
                            visible: function(key, opt) {
                                if (selected == 1) {
                                    return true;
                        ...
                        "bulkdelete": {
                            name: \'' . $this->la('Delete the selected product') . '\',
                            icon: "delete",
                            visible: function(key, opt) {
                                if (selected < 2) {
                                    return false;
                        ...
                        "sep3": "---------",
                        "searchReplace": {
                            name: \'' . $this->la('Multi criteria modification') . '\',
                            icon: "search",
	...
	public function getProductRequest() {
		...
		$query = new DbQuery();
		$query->select('a.`id_product`, b.`link_rewrite`,a.`id_tax_rules_group`, b.`description_short`, b.`description`, b.`meta_description`, image.`id_image` AS `Image`, `reference`, b.`name`, cl.`name` AS `CategoryName` , a.`price`, a.`ecotax`, tl.`name` as `Tax`, t.rate, a.`wholesale_price`, a.`active` as enable, a.`available_for_order` as available, a.`id_category_default`, a.`id_manufacturer`, case  when cp.`position` >= 0 then CONCAT(\'<div class="dragGroup"><div class="productPosition_\', a.`id_category_default`,\' positions" data-id="\', a.`id_product`,\'" data-position="\',cp.`position`,\'">\',cp.`position`, \'</div></div>\') end as `position`, cp.`position` as `productPosition`, a.`cache_default_attribute` as `id_product_attribute`');
		$query->from('product', 'a');
		$query->leftJoin('product_lang', 'b', 'b.`id_product` = a.`id_product` AND b.`id_lang` = ' . (int) $this->context->language->id);
		$query->leftJoin('category_lang', 'cl', 'a.`id_category_default` = cl.`id_category` AND b.`id_lang` = cl.`id_lang`');
		...
		foreach ($products as &$product) {
			...
			$product['Image'] = '<img src="' . $this->context->link->getImageLink($product['link_rewrite'], $product['Image'], 'cart_default') . '" class="imgm img-thumbnail">';
	...
	public function ajaxProcessgetProductRequest() {
		...
		$return = Tools::jsonEncode($this->getProductRequest());
		die($return);
	...
	public function getProductFields() {
		...
		return [
			...
			[
				'title'     => $this->la('Short Description'),
				'width'     => 200,
				'exWidth'   => 65,
			...
			[
				'title'     => $this->la('Meta Description'),
				'width'     => 200,
				'exWidth'   => 65,
			...
			[
				'title'     => $this->la('Category'),
				'titleForm' => $this->la('Select category for selected products'),
				'minWidth'  => 150,
				'exWidth'   => 40,
			...
			[
				'title'        => $this->la('WholeSale price'),
				'titleForm'    => $this->la('Change wholesale price for selected products'),
				'maxWidth'     => 150,
				'exWidth'      => 20,
				...
				'numberFormat' => '#,##0.00_-"€ ' . $this->la('Tax excl.') . '"',
				'valign'       => 'center',
				'dataType'     => 'float',
				'dataForm'     => 'float',
				'format'       => '# ##0,00 €' . $this->la('Tax excl.'),
				'hidden'       => true,
				'updatable'    => true,
			],
			[
				'title'        => $this->la('Base price'),
				'titleForm'    => $this->la('Change resell price Tax excl. for selected products'),
				'exWidth'      => 20,
				'dataIndx'     => 'price',
				'align'        => 'right',
				'valign'       => 'center',
				...
				'dataForm'     => 'float',
				'updatable'    => true,
				'format'       => "#.###,00 € " . $this->la('Tax excl.'),
			],
			[
				'title'        => $this->la('Eco Tax'),
				'titleForm'    => $this->la('Change ecotax amount for selected products'),
				'maxWidth'     => 100,
				'exWidth'      => 20,
				'dataIndx'     => 'ecotax',
				'align'        => 'right',
			...
			[
				...
				'numberFormat' => '#,##0.00_-"€"',
				'editable'     => false,
				'format'       => "#.###,00 € " . $this->la('Tax incl.'),
				'updatable'    => false,
				'sortable'     => false,
			],
			[
				'title'     => $this->la('Quantity'),
				'maxWidth'  => 100,
				'exWidth'   => 20,
				'dataIndx'  => 'quantity',
				'align'     => 'center',
			...
			[
				...
				'filter'     => [
					...
					'attr'   => "id=\"enableSelecor\", placeholder=" . $this->la('--Select--') . " readonly",
					'crules' => [['condition' => "equal"]],
			...
			[
				...
				'titleForm' => $this->la('Select status for selected products'),
				'minWidth'  => 100,
				'dataIndx'  => 'active',
				'dataForm'  => 'select',
				'align'     => 'center',
			...
			[
				'title'    => $this->la('Available for order'),
				'minWidth' => 100,
				...
				'hidden'   => true,
				'filter'   => [
					'attr'     => "id=\"availableSelector\", placeholder=" . $this->la('--Select--') . " readonly",
					'crules'   => [['condition' => "equal"]],
	...
	public function getDeclinaisonFields() {
		...
		return [
			...
			[
				'title'    => $this->la('Attribute - value pair'),
				'width'    => 200,
				'dataIndx' => 'attributes',
				'dataType' => 'html',
				'editable' => false,
		...
		if ($id_product > 0) {
			...
			if (Validate::isLoadedObject($product)) {
				...
				if ($result) {
					...
					$return = [
						'success' => true,
						'message' => $this->la('Ce produit a été mise à jour avec succès'),
					];
				} else {
					$return = [
						'success' => false,
						'message' => $this->la('Une erreur s’est produite en essayant de mettre à jour ce produit'),
					];
				}
				...
				$return = [
					'success' => false,
					'message' => $this->la('Une erreur s’est produite en essayant de de charger ce produit'),
				];
			}
			...
			if ($result) {
				$languages = Language::getLanguages(false);
				$this->updateTags($languages, $product);
				Logger::addLog(sprintf($this->la('%s addition', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $product->id, true, (int) $this->context->employee->id);
				...
				if (Configuration::get('EPH_FORCE_ASM_NEW_PRODUCT') && Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && $product->getType() != Product::PTYPE_VIRTUAL) {
					...
					$product->save();
					$idCompanys = [Context::getContext()->company->id];

					foreach ($idCompanys as $idCompany) {
						StockAvailable::setProductDependsOnStock($product->id, true,  0);
					}
				...
				if (empty($this->errors)) {
					...
					$return = [
						'success'    => true,
						'message'    => $this->la('The product has been created successfully'),
					...
					$return = [
						'success' => false,
						'message' => $this->la('We encounter an error trying to create this Product'),
					];
	...
	public function ajaxProcessAddnewCombination() {
		...
		if (!empty($this->errors)) {
			...
			$result = [
				...
				'message'     => $this->la('Declinaisons has been added with success'),
			];
		}
			...
			if ($declinaison->id_product != $id_product) {
				...
				if (!isset($result) || !$result) {
					...
					$result = [
						...
						'message'   => $this->la('Declinaisons has been updated with success'),
					];
					die(Tools::jsonEncode($result));
				}
	...
	public function ajaxProcessEditObject() {
		...
		if ($this->tabAccess['edit'] == 1) {
			...
			$_GET['updateproduct'] = "";
            $cacheId = 'scriptHeader_BackOfficeHeader';
            if (!Cache::isStored($cacheId)) {
                $scripHeader = Hook::exec('displayBackOfficeHeader', []);
                Cache::store($cacheId, $scripHeader);
            } else {
                $scripHeader = Cache::retrieve($cacheId);
            }
            $cacheId = 'scriptFooter_BackOfficeHeader';
            if (!Cache::isStored($cacheId)) {
                $scriptFooter = Hook::exec('displayBackOfficeFooter', []);
                Cache::store($cacheId, $scriptFooter);
            } else {
                $scriptFooter = Cache::retrieve($cacheId);
            }
			
			$this->object = new Product($idProduct, true);

			$iso = (file_exists(_SHOP_ROOT_DIR_ . '/js/tinymce/langs/' . $this->context->language->iso_code . '.js') ? $iso : 'en');
			...
			$data->assign([
				...
				'ps_force_friendly_product' => Configuration::get('EPH_FORCE_FRIENDLY_PRODUCT'),
				'tinymce'                   => true,
				'iso'                       => $iso,
	...
	public function ajaxProcessAddObject() {
		...
		$pusjJs = $this->pushJS([
			...
			_EPH_JS_DIR_ . 'colorpicker/colorpicker.js',
			_EPH_JS_DIR_ . 'jquery/plugins/jquery.typewatch.js',
            _EPH_JS_DIR_ . 'jquery/plugins/smartWizard/jquery.smartWizard.js',
            _EPH_JS_DIR_ . 'jquery/plugins/jquery.typewatch.js',
			_EPH_JS_DIR_ . 'product_wizard.js',
		...
		$data->assign([
			...
			'ad'                        => __EPH_BASE_URI__ . basename(_EPH_ROOT_DIR_),
			'product_type'              => (int) Tools::getValue('type_product', $this->object->getType()),
            'labels'            => ['next' => $this->la('Next'), 'previous' => $this->la('Previous'), 'finish' => $this->la('Finish')],
		]);
    ...
    public function getValidationRules() {
        ...
        $definition = PhenyxObjectModel::getDefinition('Product');
	...
	public function ajaxProcessUpdateProductPositions() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			if (empty($this->errors)) {
				...
				$result = [
					'success' => true,
					'message' => $this->la('Product position has been successfully updated.'),
				];
			} else {
				$this->errors = array_unique($this->errors);
				$result = [
	...
	public function ajaxProcessAddAttachment() {
		...
		if ($this->tabAccess['edit'] === '0') {
			$this->ajaxDie(json_encode(['error' => $this->la('You do not have the right permission')]));
		}
			...
			foreach ($files as $image) {
				...
				if ($res) {
					...
					$result = [
						...
						'message'    => $this->la('File has been attached successfully'),
					];
				}
	...
	public function ajaxProcessProcessDuplicate() {
		...
		if (Validate::isLoadedObject($product = new Product((int) Tools::getValue('id_product')))) {
			...
			if ($product->add()
				...
				$result = [
					'success' => true,
					'message' => $this->la('Product successfuly duplicated'),
				];
			} else {

				$result = [
					'success' => false,
					'message' => $this->la('An error occurred while creating an object.'),
				];
			}
				...
				*/
				...
				if (!count($this->errors)) {
					...
					if ($object->delete()) {
						...
						Logger::addLog(sprintf($this->la('%s deletion', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $object->id, true, (int) $this->context->employee->id);
	...
	public function processPricesModification() {
		...
		foreach ($idSpecificPrices as $key => $idSpecificPrice) {
			...
			if ($this->_validateSpecificPrice($idCompanys[$key], $idCurrencies[$key], $idCountries[$key], $idGroups[$key], $idCustomers[$key], $prices[$key], $fromQuantities[$key], $reductions[$key], $reductionTypes[$key], $froms[$key], $tos[$key], $idCombinations[$key])) {
	...
	protected function _validateSpecificPrice($idCompany, $idCurrency, $idCountry, $idGroup, $idCustomer, $price, $fromQuantity, $reduction, $reductionType, $from, $to, $idCombination = 0) {

		if (!Validate::isUnsignedId($idCompany) || !Validate::isUnsignedId($idCurrency) || !Validate::isUnsignedId($idCountry) || !Validate::isUnsignedId($idGroup) || !Validate::isUnsignedId($idCustomer)) {
		...
		if (SpecificPrice::exists((int) $this->object->id, $idCombination, $idCompany, $idGroup, $idCountry, $idCurrency, $idCustomer, $fromQuantity, $from, $to, false)) {
	...
	public function processProductCustomization() {
		...
		if (Validate::isLoadedObject($product = new Product((int) Tools::getValue('id_product')))) {
			...
			if (empty($this->errors)) {
				$this->confirmations[] = $this->la('Update successful');
			}
	...
	public function ajaxProcessDeleteProduct() {
		...
		if ($product->delete()) {
			$result = [
				'success' => true,
				'message' => $this->la('Product successfuly deleted'),
			];
		} else {
			$result = [
				'success' => false,
				'message' => $this->la('A problem occur deteleting the product'),
			];
		}
		...
		if ($this->tabAccess['delete'] === '1') {
			...
			if ($idProduct && Validate::isUnsignedId($idProduct) && Validate::isLoadedObject($product = new Product($idProduct))) {
				...
				if (($dependsOnStock = StockAvailable::dependsOnStock($idProduct)) && StockAvailable::getQuantityAvailableByProduct($idProduct, $idProductAttribute)) {
					$json = [
						'status'  => 'error',
						'message' => $this->la('It is not possible to delete a combination while it still has some quantities in the Advanced Stock Management. You must delete its stock first.'),
					];
				} else {
					...
					if ($dependsOnStock && !Stock::deleteStockByIds($idProduct, $idProductAttribute)) {
						$json = [
							'status'  => 'error',
							'message' => $this->la('Error while deleting the stock'),
						];
					} else {
						$json = [
							'status'               => 'ok',
				...
				$json = [
					'status'  => 'error',
					'message' => $this->la('You cannot delete this attribute.'),
				];
			}
	...
	public function ajaxProcessDefaultProductAttribute() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			if (Validate::isLoadedObject($product = new Product((int) Tools::getValue('id_product')))) {
				...
				$json = [
					'status'  => 'error',
					'message' => $this->la('You cannot make this the default attribute.'),
				];
			}
	...
	public function buildCategoriesSelector() {
		...
		$html .= '<option value="">' . $this->la('--Select--') . '</option>';
	...
	public function ajaxProcessBulkUpdateProduct() {
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('The selection has been successfully updated'),
			];
		}
	...
	public function ajaxProcessDeleteProductImage() {
		...
		if (file_exists(_EPH_TMP_IMG_DIR_ . 'product_mini_' . $image->id_product . '_' . $this->context->company->id . '.jpg')) {
			$res &= @unlink(_EPH_TMP_IMG_DIR_ . 'product_mini_' . $image->id_product . '_' . $this->context->company->id . '.jpg');
		}
	...
	public function checkProduct() {
		...
		if ($this->isProductFieldUpdated('id_category_default') && (!Tools::isSubmit('categoryBox') || !count(Tools::getValue('categoryBox')))) {
			$this->errors[] = $this->la('Products must be in at least one category.');
		}
		...
		if ($this->isProductFieldUpdated('id_category_default') && (!is_array(Tools::getValue('categoryBox')) || !in_array(Tools::getValue('id_category_default'), Tools::getValue('categoryBox')))) {
			$this->errors[] = $this->la('This product must be in the default category.');
		}
	...
	public function getPreviewUrl(Product $product) {
		...
		$idLang = Configuration::get('EPH_LANG_DEFAULT', null, null, $this->context->company->id);

		if (!Validate::isLoadedObject($product) || !$product->id_category_default) {
			return $this->la('Unable to determine the preview URL. This product has not been linked with a category, yet.');
		}

		if (!CompanyUrl::getMainShopDomain()) {
			return false;
		}
				...
				if (($idProductAttribute = (int) Tools::getValue('id_product_attribute')) || ($idProductAttribute = $product->productAttributeExists(Tools::getValue('attribute_combination_list'), false, null, true, true))) {
					...
					if ($this->tabAccess['edit'] === '1') {
						...
						if ($this->isProductFieldUpdated('available_date_attribute') && (Tools::getValue('available_date_attribute') != '' && !Validate::isDateFormat(Tools::getValue('available_date_attribute')))) {
							...
							StockAvailable::setProductDependsOnStock((int) $product->id, $product->depends_on_stock, (int) $idProductAttribute);
							StockAvailable::setProductOutOfStock((int) $product->id, $product->out_of_stock,  (int) $idProductAttribute);
						}
	...
	public function processPriceAddition() {
		...
		if ($this->_validateSpecificPrice($idCompany, $idCurrency, $idCountry, $idGroup, $idCustomer, $price, $fromQuantity, $reduction, $reductionType, $from, $to, $idProductAttribute)) {
	...
	public function processSpecificPricePriorities() {
		...
		if (Tools::isSubmit('specificPricePriorityToAll')) {
			...
			if (!SpecificPrice::setPriorities($priorities)) {
				...
				$this->confirmations[] = $this->la('The price rule has successfully updated');
			}
	...
	public function ajaxProcessUpdateByVal() {
		...
		if (Validate::isLoadedObject($product)) {
			...
			if (!isset($result) || !$result) {
				...
				$result = [
					'success' => true,
					'message' => $this->la('Update successful'),
				];
			}
	...
	public function initPageHeaderToolbar() {
		...
		if (empty($this->display)) {
			$this->page_header_toolbar_btn['filter_category'] = [
				'desc'       => $this->la('Filter by category', null, null, false),
				'identifier' => 'filter',
				'controller' => $this->controller_name,
				'icon'       => 'process-icon-filter',
			];
			$this->page_header_toolbar_btn['fields_edit'] = [
				'desc'       => $this->la('Choose available Fields to display', null, null, false),
				'identifier' => 'field',
				'controller' => $this->controller_name,
				'icon'       => 'process-icon-excel',
			];
			$this->page_header_toolbar_btn['reset_category'] = [
				'desc'       => $this->la('Reset category', null, null, false),
				'id'         => 'reset_category',
			...
			$this->page_header_toolbar_btn['new_product'] = [
				...
				'desc' => $this->la('Add new product', null, null, false),
			...
			if (($product = $this->loadObject(true)) && $product->isAssociatedToShop()) {
				...
				if ($url_preview = $this->getPreviewUrl($product)) {
					$this->page_header_toolbar_btn['preview'] = [
						'short'  => $this->la('Preview', null, null, false),
						'href'   => $url_preview,
						'desc'   => $this->la('Preview', null, null, false),
						'target' => true,
						'class'  => 'previewUrl',
					];
				}
				...
				'confirm_link(\'\', \'' . $this->la('This will copy the images too. If you wish to proceed, click "Yes". If not, click "No".', null, true, false) . '\', \'' . $this->la('Yes', null, true, false) . '\', \'' . $this->la('No', null, true, false) . '\', \'' . $this->context->link->getAdminLink('AdminProducts', true) . '&id_product=' . (int) $product->id . '&duplicateproduct' . '\', \'' . $this->context->link->getAdminLink('AdminProducts', true) . '&id_product=' . (int) $product->id . '&duplicateproduct&noimage=1' . '\')'
				:
				...
				if ($this->tabAccess['add']) {
					$this->page_header_toolbar_btn['duplicate'] = [
						'short'   => $this->la('Duplicate', null, null, false),
						'desc'    => $this->la('Duplicate', null, null, false),
						'confirm' => 1,
						'js'      => $js,
					];
				}
				...
				if ($this->tabAccess['delete']) {
					$this->page_header_toolbar_btn['delete'] = [
						'short'   => $this->la('Delete', null, null, false),
						'href'    => $this->context->link->getAdminLink('AdminProducts') . '&id_product=' . (int) $product->id . '&deleteproduct',
						'desc'    => $this->la('Delete this product', null, null, false),
						'confirm' => 1,
						'js'      => 'if (confirm(\'' . $this->la('Delete product?', null, true, false) . '\')){return true;}else{event.preventDefault();}',
					];
				}
	...
	public function renderForm() {
		...
		$this->tpl_form_vars['display_multishop_checkboxes'] = false;
		$this->fields_form = [''];
		...
		$this->tpl_form_vars['ps_force_friendly_product'] = Configuration::get('EPH_FORCE_FRIENDLY_PRODUCT');
		...
		$this->tpl_form_vars['ad'] = __EPH_BASE_URI__ . basename(_EPH_ROOT_DIR_);
	...
	protected function _displayDraftWarning($active) {
		...
		$content = '<div class="warn draft" style="' . ($active ? 'display:none' : '') . '">
                <span>' . $this->la('Your product will be saved as a draft.') . '</span>
                <a href="#" class="btn btn-default pull-right" onclick="submitAddProductAndPreview()" ><i class="icon-external-link-sign"></i> ' . $this->la('Save and preview') . '</a>
	...
	public function initFormAssociations($obj) {
		...
		$defaultCategory = $this->context->cookie->id_category_products_filter ? $this->context->cookie->id_category_products_filter : $this->context->company->id_category;
		...
		// Multishop block
		$data->assign('feature_shop_active', false);
		...
		$data->assign(
			[
				...
				'is_shop_context'     => Company::getContext(),
			]
		);
	...
	public function generateCategoryGridScript($category_tree) {
		...
		$paragrid->title = '\'' . $this->la('Associated Categories') . '\'';
	...
	public function ajaxProcessUploadProductFile() {
		...
		if (isset($_FILES['productFile']['name']) && !empty($_FILES['productFile']['name']) && !empty($_FILES['productFile']['tmp_name'])) {
			...
			$dir = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'fileimport' . DIRECTORY_SEPARATOR;
	...
	public function initFormPrices($obj) {
		...
		if ($obj->id) {
			...
			$this->displayWarning($this->la('You must save this product before adding specific pricing'));
		...
		$data->assign(
			[
				...
				'ps_use_ecotax'           => Configuration::get('EPH_USE_ECOTAX'),
			]
		);
		...
		$data->assign('ps_tax', Configuration::get('EPH_TAX'));
	...
	public function getSpecificPriceRequest($idProduct) {
		...
		foreach ($specificPrices as &$specificPrice) {
			...
			if ($specificPrice['id_currency'] > 0) {
				...
				$specificPrice['id_currency'] = $this->la('All currencies');
			}

			if ($specificPrice['id_group'] > 0) {
				$specificPrice['id_group'] = $groups[$specificPrice['id_group']]['name'];
			} else {
				$specificPrice['id_group'] = $this->la('All groups');
			}
			...
			$specificPrice['id_country'] = $specificPrice['id_country'] ? $countries[$specificPrice['id_country']]['name'] : $this->la('All countries');
			...
			if ($specificPrice['reduction'] > 0) {
				...
				if ($specificPrice['reduction_tax']) {
					$specificPrice['impact'] .= '(' . $this->la('Tax incl.') . ')';
				} else {
					$specificPrice['impact'] .= '(' . $this->la('Tax excl.') . ')';
				}
			...
			if ($specificPrice['from'] == '0000-00-00 00:00:00' && $specificPrice['to'] == '0000-00-00 00:00:00') {
				$specificPrice['period'] = $this->la('Unlimited');
			} else {
				$specificPrice['period'] = $this->la('From') . ' ' . ($specificPrice['from'] != '0000-00-00 00:00:00' ? $specificPrice['from'] : '0000-00-00 00:00:00') . '<br />' . $this->la('To') . ' ' . ($specificPrice['to'] != '0000-00-00 00:00:00' ? $specificPrice['to'] : '0000-00-00 00:00:00');
			}
			...
			if ($specificPrice['id_product_attribute']) {
				...
				$specificPrice['attributesName'] = $this->la('All combinations');
			}
			...
			if ($specificPrice['id_customer']) {
				...
				$specificPrice['id_customer'] = $this->la('All customers');
			}
	...
	public function AjaxProcessGetSpecificPriceFields() {
		...
		$fileds = [
			...
			[
				'title'    => $this->la('Fixed price (tax excl.)'),
				'dataIndx' => 'price',
				'align'    => 'left',
				'valign'   => 'center',
				'dataType' => 'string',
	...
	public function ajaxProcesseditSpecificPrice() {
		...
		if ($specificPrice->id_customer) {
			...
			$specificPrice->customerName = $this->la('All customers');
		}
		...
		$data->assign([
			...
			'product'         => $product,
			'combinations'    => $combinations,
			'tagHeader'       => $this->la('Add new specifi price for ') . $product->name[$this->context->language->id],
			'countries'       => Country::getCountries($this->context->language->id),
			'groups'          => Group::getGroups($this->context->language->id),
			'currencies'      => Currency::getCurrencies(),
			'currency'        => $this->context->currency,
		...
		$result = [
			'html'          => $data->fetch(),
			'currentText'   => $this->la('Now'),
			'closeText'     => $this->la('Done'),
			'timeOnlyTitle' => $this->la('Choose Time'),
			'timeText'      => $this->la('Time'),
			'hourText'      => $this->la('Hour'),
			'minuteText'    => $this->la('Minute'),
		];
	...
	public function ajaxProcessNewSpecificPrice() {
		...
		$idProduct = Tools::getValue('id_product');
		$idProductAttribute = Tools::getValue('sp_id_product_attribute');
		$idCompany = Tools::getValue('sp_id_shop');
		$idCurrency = Tools::getValue('sp_id_currency');
		$idCountry = Tools::getValue('sp_id_country');
		$idGroup = Tools::getValue('sp_id_group');
		$idCustomer = Tools::getValue('sp_id_customer');
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('News specific price has been added successfully'),
			];
		}
	...
	public function ajaxProcessdeleteSpecificPrice() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('The specific price has been deleted successfully'),
		];
		die(Tools::jsonEncode($result));
	}
	...
	public function initFormSeo($product) {
		...
		if (!Validate::isLoadedObject($product) || !$product->id_category_default) {
			...
			foreach ($this->_languages as $language) {
				$rewrittenLinks[(int) $language['id_lang']] = [$this->la('Unable to determine the preview URL. This product has not been linked with a category, yet.')];
			}
		...
		$data->assign(
			[
				...
				'languages'             => $this->_languages,
				'id_lang'               => $this->context->language->id,
				'ps_ssl_enabled'        => Configuration::get('EPH_SSL_ENABLED'),
				'curent_shop_url'       => $this->context->company->getBaseURL(),
				'default_form_language' => $this->default_form_language,
		...
		if (!$product) {
			...
			if (is_array($product->packItems)) {
				...
				foreach ($product->packItems as $packItem) {
					...
					//$pack_items[$i]['image'] = ImageManager::thumbnail($path_to_image, 'pack_mini_'.$pack_item->id.'_'.$this->context->company->id.'.jpg', 120);
					$i++;
				}
	...
	public function initFormCustomization($obj) {
		...
		if ((bool) $obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$this->displayWarning($this->la('You must save the product in this shop before adding customization.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before adding customization.'));
		}
	...
	public function initFormAttachments($obj) {
		...
		if ((bool) $obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$isoTinyMce = (file_exists(_EPH_ROOT_DIR_ . '/js/tinymce/langs/' . $isoTinyMce . '.js') ? $isoTinyMce : 'en');
				...
				$this->displayWarning($this->la('You must save the product in this shop before adding attachements.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before adding attachements.'));
		}
	...
	public function initFormShipping($obj) {
		...
		$data->assign(
			[
				'product'                   => $obj,
				'ps_dimension_unit'         => Configuration::get('EPH_DIMENSION_UNIT'),
				'ps_weight_unit'            => Configuration::get('EPH_WEIGHT_UNIT'),
	...
	public function initFormImages($obj) {
		...
		if ((bool) $obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$currentShopId = (int) $this->context->company->id;
				...
				$data->assign(
					[
						...
						'link'                => $this->context->link,
						'base_link'           => $this->context->link->getBaseFrontLink().'content/',
						'bo_imgdir'                 => __EPH_BASE_URI__ .  'content/themes/' . $this->bo_theme . '/img/',
					]
				);
				...
				$this->displayWarning($this->la('You must save the product in this shop before adding images.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before adding images.'));
		}
	...
	public function getImagesRequest($idProduct) {
		...
		foreach ($images as &$image) {
			...
			$path = $this->context->link->getBaseFrontLink() . '/content/img/p/' . $img->getExistingImgPath() . '-small_default.jpg';
			...
			$image['delete'] = '<a href="javascript:void(0)" onClick="deleteObject(\'' . $this->controller_name . '\', \'Image\', \'' . $this->la('Delete this image') . '\', \'' . $this->la('Are you sure you want to delete this Image?') . '\', \'' . $this->la('Yes, delete it') . '\', \'' . $this->la('Cancel') . '\', ' . $img->id . ');" class="pull-right btn btn-default" ><i class="fa fa-trash"></i> Supprimer cette image</a>';
		}
	...
	public function ajaxProcessUpdateImageByVal() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Default Image has been updated successfully'),
		];
	...
	public function ajaxProcessUpdateImagePosition() {
		...
		if ($this->tabAccess['edit'] === '0') {
			...
			$return = [
				'success' => false,
				'message' => $this->la('You do not have the right permission'),
			];
		...
		$return = [
			'success' => true,
			'message' => $this->la('Image position has been updated successfully'),
		];
	...
	public function ajaxProcessAddNewProductImage() {
		...
		foreach ($files as &$file) {
			...
			if ($result) {
				...
				@unlink(_EPH_TMP_IMG_DIR_ . 'product_mini_' . (int) $product->id . '_' . $this->context->company->id . '.jpg');
	...
	public function ajaxProcessLaunchNewDeclinaison() {
		...
		if (Validate::isLoadedObject($product)) {
			...
			$data->assign('tax_exclude_option', Tax::excludeTaxeOption());
			$data->assign('ps_weight_unit', Configuration::get('EPH_WEIGHT_UNIT'));
			$isoTinyMce = $this->context->language->iso_code;
			$isoTinyMce = (file_exists(_EPH_ROOT_DIR_ . '/js/tinymce/langs/' . $isoTinyMce . '.js') ? $isoTinyMce : 'en');
			$currency = $this->context->currency;
	...
	public function initFormAttributes($product) {
		...
		if (Validate::isLoadedObject($product)) {
			...
			if ($this->product_exists_in_shop) {
				...
				$this->displayWarning($this->la('You must save the product in this shop before adding declinaisons.'));
			}

		} else {
			$data->assign('product', $product);
			$this->displayWarning($this->la('You must save this product before adding declinaisons.'));
		}
		...
		$paragrid->contextMenu = [
			'#grid_' . $controller_name => [
                ...
                return {
                    ...
                    items: {
                        "add": {
                            name: \'' . $this->la('Ajouter une nouvelle déclinaison') . ' \',
                            icon: "add",
                            callback: function(itemKey, opt, e) {
                        ...
                        "edit": {
                            name : \'' . $this->la('Visualiser ou modifier la déclinaison: ') . '\'' . '+rowData.reference,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
						...
						"trash": {
                            name : \'' . $this->la('Supprimer la déclinaison: ') . '\'' . '+rowData.reference,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
	...
	public function renderListAttributes($product, $currency) {
		...
		$this->bulk_actions = ['delete' => ['text' => $this->la('Delete selected'), 'confirm' => $this->la('Delete selected items?')]];
		...
		$this->fields_list = [
			'attributes' => ['title' => $this->la('Attribute - value pair'), 'align' => 'left'],
			'price'      => ['title' => $this->la('Impact on price'), 'type' => 'price', 'align' => 'left'],
			'weight'     => ['title' => $this->la('Impact on weight'), 'align' => 'left'],
			'reference'  => ['title' => $this->la('Reference'), 'align' => 'left'],
			'ean13'      => ['title' => $this->la('EAN-13'), 'align' => 'left'],
			'upc'        => ['title' => $this->la('UPC'), 'align' => 'left'],
		];
	...
	public function initFormQuantities($obj) {
		...
		if ($obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$data->assign('ps_stock_management', Configuration::get('EPH_STOCK_MANAGEMENT'));
				...
				if ($advanced_stock_management_warning) {
					$this->displayWarning($this->la('If you wish to use the advanced stock management, you must:'));
					$this->displayWarning('- ' . $this->la('associate your products with warehouses.'));
					$this->displayWarning('- ' . $this->la('associate your warehouses with carriers.'));
					$this->displayWarning('- ' . $this->la('associate your warehouses with the appropriate shops.'));
				}
				...
				if (Pack::isPack($obj->id)) {
					...
					if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && !Warehouse::getPackWarehouses((int) $obj->id)) {
						$this->displayWarning($this->la('You must have a common warehouse between this pack and its product.'));
					}
				...
				$this->displayWarning($this->la('You must save the product in this shop before managing quantities.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before managing quantities.'));
		}
	...
	public function initFormSuppliers($obj) {
		...
		if ($obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$this->displayWarning($this->la('You must save the product in this shop before managing suppliers.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before managing suppliers.'));
		}
	...
	public function initFormWarehouses($obj) {
		...
		if ($obj->id) {
			...
			if ($this->product_exists_in_shop) {
				...
				$this->displayWarning($this->la('You must save the product in this shop before managing warehouses.'));
			}

		} else {
			$this->displayWarning($this->la('You must save this product before managing warehouses.'));
		}
	...
	public function initFormFeatures($obj) {
		...
		if (!Feature::isFeatureActive()) {
			$this->displayWarning($this->la('This feature has been disabled. ') . ' <a href="index.php?tab=AdminPerformance&token=' . Tools::getAdminTokenLite('AdminPerformance') . '#featuresDetachables">' . $this->la('Performances') . '</a>');
		} else {
			...
			if ($obj->id) {
				...
				if ($this->product_exists_in_shop) {
					...
					$this->displayWarning($this->la('You must save the product in this shop before adding features.'));
				}

			} else {
				$this->displayWarning($this->la('You must save this product before adding features.'));
			}
	...
	public function ajaxProcessProductQuantity() {
		...
		if (!Tools::getValue('actionQty')) {
			$this->ajaxDie(json_encode(['error' => $this->la('Undefined action')]));
		}
		...
		case 'depends_on_stock':
			...
			if (Tools::getValue('value') === false) {
				$this->ajaxDie(json_encode(['error' => $this->la('Undefined value')]));
			}

			if ((int) Tools::getValue('value') != 0 && (int) Tools::getValue('value') != 1) {
				$this->ajaxDie(json_encode(['error' => $this->la('Incorrect value')]));
			}

			if (!$product->advanced_stock_management && (int) Tools::getValue('value') == 1) {
				$this->ajaxDie(json_encode(['error' => $this->la('Not possible if advanced stock management is disabled. ')]));
			}
			...
			if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && (int) Tools::getValue('value') == 1 && (Pack::isPack($product->id) && !Pack::allUsesAdvancedStockManagement($product->id)
				...
				$this->ajaxDie(
					json_encode(
						[
							'error' => $this->la('You cannot use advanced stock management for this pack because') . '<br />' .
							$this->la('- advanced stock management is not enabled for these products') . '<br />' .
							$this->la('- you have chosen to decrement products quantities.'),
						]
					)
				);
			}
		...
		case 'pack_stock_type':
			...
			if ($product->depends_on_stock && !Pack::allUsesAdvancedStockManagement($product->id) && ((int) $value == 1
				...
				$this->ajaxDie(
					json_encode(
						[
							'error' => $this->la('You cannot use this stock management option because:') . '<br />' .
							$this->la('- advanced stock management is not enabled for these products') . '<br />' .
							$this->la('- advanced stock management is enabled for the pack'),
						]
					)
				);
			}
		...
		case 'set_qty':
			...
			if (Tools::getValue('id_product_attribute') === false) {
				$this->ajaxDie(json_encode(['error' => $this->la('Undefined id product attribute')]));
			}
	...
	public function getL($key) {
		...
		$trad = [
			'Default category:'                                                 => $this->la('Default category'),
			'Catalog:'                                                          => $this->la('Catalog'),
			'Consider changing the default category.'                           => $this->la('Consider changing the default category.'),
			'ID'                                                                => $this->la('ID'),
			'Name'                                                              => $this->la('Name'),
			'Mark all checkbox(es) of categories in which product is to appear' => $this->la('Mark the checkbox of each categories in which this product will appear.'),
		];
	...
	public function displayPreviewLink($id, $token = null, $name = null) {
		...
		if (!array_key_exists('Bad SQL query', static::$cache_lang)) {
			static::$cache_lang['Preview'] = $this->la('Preview', 'Helper');
		}
						...
						foreach ($products as $id_product) {
							...
							*/
							...
							if (!count($this->errors)) {
								...
								if ($product->delete()) {
									Logger::addLog(sprintf($this->la('%s deletion', 'AdminTab', false, false), $this->className), 1, null, $this->className, (int) $product->id, true, (int) $this->context->employee->id);
								} else {
									$success = false;
								}
	...
	public function ajaxProcessInactiveProduct() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Le produit a été désactivé avec succès'),
		];
	...
	public function ajaxProcessActiveProduct() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Le produit a été activé avec succès'),
		];
	...
	public function ajaxProcessAvailableProduct() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Product has been availabled for order'),
		];
	...
	public function ajaxProcessInavailableProduct() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Product has been tunr unavailabled for order'),
		];
