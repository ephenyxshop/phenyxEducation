...
class AdminCustomersControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->publicName = $this->la('Liste des Clients / Etudiants');
		$this->lang = false;
	...
	public function generateParaGridScript($regenerate = false) {
		...
		$this->paramTitle = '\'' . $this->la('Gérer mes clients') . '\'';
		...
		$this->paramContextMenu = [
			'#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        "add": {
                            name: \'' . $this->la('Ajouter un nouveau client') . ' \',
                            icon: "add",
						...
						"new": {
                            name : \'' . $this->la('Ajouter une formation pour ') . '\'' . '+rowData.firstname+" "+rowData.lastname,
                            icon: "educ",
							callback: function(itemKey, opt, e) {
                             	addNewEducation(rowData.id_customer);
                            }
                        },
						"suivi": {
                            name : \'' . $this->la('Fenêtre de suivi de l‘étudiant ') . '\'' . '+rowData.firstname+" "+rowData.lastname,
                            icon: "window",
                            ...
                            callback: function(itemKey, opt, e) {
								openStudentSuivie(rowData.id_customer);
                            }
                        },
						"turnAgent": {
                            name : \'' . $this->la('Convertir en agent commercial ') . '\'' . '+rowData.firstname+" "+rowData.lastname,
                            icon: "ass",
							visible: function(key, opt) {
							...
							callback: function(itemKey, opt, e) {
                             	turnAgent(rowData.id_customer);
                            }
                        },
                        "turnAdmin": {
                            name : \'' . $this->la('Convertir en Employee ') . '\'' . '+rowData.firstname+" "+rowData.lastname,
                            icon: "ass",
							visible: function(key, opt) {
                            if(rowData.user_admin_profile != 1) {
                                return false;
                            }
                        ...
                        "unselect": {
                            name: \'' . $this->la('Tous déselectionner') . '\',
                            icon: "list-ul",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 2) {
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer le client :') . '\'' . '+rowData.lastname,
                            icon: "delete",
                            /*
                            visible: function(key, opt) {
                                if (selected == 1) {
                            ...
                            */
                            callback: function(itemKey, opt, e) {
                              deleteObject("' . $this->controller_name . '", "' . $this->className . '", "Supprimer un client", "Etes-vous sûr(e) de vouloir supprimer "+rowData.firstname+" "+ rowData.lastname+ " ?", "Oui", "Annuler",rowData.id_customer, rowIndex);
                            }
                        },
                        "bulkdelete": {
                            name: \'' . $this->la('Delete the selected customer') . '\',
                            icon: "delete",
                            visible: function(key, opt) {
                                if (selected < 2) {
                                    return false;
	...
	public function getCustomerRequest() {
		...
		$customers = Db::getInstance()->executeS(
            (new DbQuery())
             ->select('a.`id_customer`, a.`customer_code`, a.`firstname`, a.`lastname`, a.`email`, a.`company`, a.`id_gender`, a.`is_admin`, grl.name AS `tarif`, a.`active` AS `active`, a.active as enable, a.`id_default_group`, a.birthday, a.`is_agent` ,  a.date_add, gl.name as title,  case when a.active = 1 then \'<div class="p-active"></div>\' else \'<div class="p-inactive"></div>\' end as active, IFNULL((SELECT el.name FROM eph_student_education as se LEFT JOIN `eph_education_lang` as el ON el.`id_education` = se.`id_education` WHERE se.`id_customer` = a.`id_customer` LIMIT 1), \'<div class="p-inactive"><i class="fa-duotone fa-xmark"></i></div>\') as `lastEducation`')
             ->from('customer', 'a')
             ->leftJoin('gender_lang', 'gl', 'a.`id_gender` = gl.`id_gender` AND gl.`id_lang` = ' . $this->context->language->id)
             ->leftJoin('group_lang', 'grl', 'grl.`id_group` = a.`id_default_group` AND grl.`id_lang` = ' . $this->context->language->id)
        ...
        foreach ($customers as &$customer) {
			...
			$customer['user_admin_profile'] = $this->context->employee->id_profile;
	...
	public function ajaxProcessgetCustomerRequest() {
		...
		$return = Tools::jsonEncode($this->getCustomerRequest());
		die($return);
	}
	...
	public function getCustomerFields() {
		...
		return [
			...
			[
				'title'    => $this->la('Date de naissance'),
			...
			[
				'title'    => $this->la('Dernière formation'),
				'width'    => 150,
			...
			[
				'title'    => $this->la('Date de création'),
				'width'    => 150,
				'exWidth'  => 20,
				'dataIndx' => 'date_add',
				'halign'   => 'HORIZONTAL_CENTER',
	...
	public function ajaxProcessBulkDelete() {
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('Selection of customers has been properly deleted'),
			];
		}
	...
	public function ajaxProcessAddNewAjaxObject() {
		...
		if (!$birth_city = Tools::getValue('birth_city')) {
			$return = [
				'success' => false,
				'message' => 'La ville ou le pays de naissance sont obligatoires, merci de renseigner ces champs.',
			];
			die(Tools::jsonEncode($return));
		}
		...
		if ($result) {
			$tpl = $this->context->smarty->createTemplate(_EPH_MAIL_DIR_ . '/fr/admin_account.tpl');
			$address = new Address();
	...
	public function ajaxProcessUpdateObject() {
		...
		if (!isset($result) || !$result) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('Le client a été mis à jour avec succès'),
			];
	...
	public function ajaxProcessaddNewStudentEducation() {
		...
		$_POST['targetController'] = 'AdminStudentEducations';

		$data = $this->createTemplate('controllers/customers/addEducation.tpl');
        $extraJs = [
            _EPH_JS_DIR_ . 'jquery/plugins/smartWizard/jquery.smartWizard.js',
            _EPH_JS_DIR_ . 'student_education_wizard.js',
        ];
		$id_customer = Tools::getValue('id_customer');
		$customer = new Customer($id_customer);
		$data->assign('customer', $customer);
        $data->assign('extraJs', $extraJs);
        ...
        $data->assign('labels', ['next' => $this->la('Suivant'), 'previous' => $this->la('Précédent'), 'finish' => $this->la('Réserver')]);
		$li = '<li id="uperAddAdminAddStudentEducations" data-controller="AdminDashboard"><a href="#contentAddAdminAddStudentEducations">Ajouter une formation pour ' . $customer->firstname . ' ' . $customer->lastname . '</a><button type="button" class="close tabdetail" data-id="uperAddAdminAddStudentEducations"><i class="icon icon-times-circle"></i></button></li>';
		$html = '<div id="contentAddAdminAddStudentEducations" class="panel col-lg-12" style="display; flow-root;">' . $data->fetch() . '</div>';
	...
	public function ajaxProcessOpenCustomerSuivie() {
		...
		$suivies = StudentSuivie::getSuivieById($customer->id);

		$employees = Employee::getEmployees();
		$data = $this->createTemplate('controllers/customers/suivie.tpl');
	...
	public function ajaxProcessEditStudentSuivie() {
		...
		$student = new Customer($suivie->id_customer);
		$data = $this->createTemplate('controllers/customers/editEvent.tpl');
	...
	public function ajaxProcessTurnAgent() {
		...
		$data->assign('countries', Country::getCountries($this->context->language->id, true));
		$data->assign('student', $student);
        $li = '<li id="uperAddAdminSaleAgent" data-controller="AdminDashboard"><a href="#contentAddAdminSaleAgent">Transformer ' . $student->firstname . ' en CEF</a><button type="button" class="close tabdetail" data-id="uperAddAdminSaleAgent"><i class="icon icon-times-circle"></i></button></li>';
        $html = '<div id="contentAddAdminSaleAgent" class="panel col-lg-12" style="display; flow-root;">' . $data->fetch() . '</div>';
	...
	public function ajaxProcessNewAgent() {
		...
		$agent = new SaleAgent();
		$id_student = Tools::getValue('id_customer');
		$customer = new Customer($id_student);
	...
	public static function ajaxProcessGetAutoCompleteEducation() {
		...
		WHERE (pl.name LIKE \'%' . pSQL($query) . '%\' OR p.reference LIKE \'%' . pSQL($query) . '%\')' .
			' AND p.deleted = 0 GROUP BY p.id_education';
		...
		if ($items) {
			...
			foreach ($items as $item) {
				...
				if ($item['cache_default_attribute']) {
					...
					WHERE pa.`active` = 1 AND pa.`id_education` = ' . (int) $item['id_education'] . '
