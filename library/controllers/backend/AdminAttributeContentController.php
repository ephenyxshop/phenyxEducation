...
class AdminAttributeContentControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->publicName = $this->la('Géstion des déclinaisons');
			...
			$paragrid->paragrid_option['paragrids'][] = [
        		...
        		return {
            		...
            		items: {
                		...
                		"edit": {
							name: \'' . $this->la('Modifier la déclinaison : ') . ' :\'+rowData.name,
							icon: "edit",
                			callback: function(itemKey, opt, e) {
						...
						"sep1": "---------",
           				"delete": {
           					name: \'' . $this->la('Supprimer le déclinaison ') . ' :\'+rowData.name,
           					icon: "delete",
	...
	public function ajaxProcessaddAttribute() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('La nouvelle déclinaison a été ajoutée avec succès'),
		];
	...
	public function ajaxProcessAddAttributeGroup() {
		...
		if ($result) {
			...
			$return = [
				'success' => true,
				'message' => $this->la('La catégorie Attribute a été ajoutée à jour avec succès'),
				'action'  => 'add',
				'li'      => $li,
				'html'    => $html,
			];
			...
			$return = [
				'success' => false,
				'message' => $this->la('L‘ajout de la catégorie de déclinaison à échouée'),
			];
		}
	...
	public function ajaxProcessUpdateAttribute() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('La Déclinaison ont mis à jour avec succès'),
		];
	...
	public function ajaxProcessUpdateAttributeGroup() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('La catégorie Attribute a été mis à jour avec succès'),
			'action'  => 'update',
		];
