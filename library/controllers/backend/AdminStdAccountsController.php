...
class AdminStdAccountsControllerCore extends AdminController {
    ...
    public $php_self = 'adminstdaccounts';
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Book keepings Accounts');
        $this->lang = true;
    ...
    public function setMedia() {
        ...
        $this->addCSS(_EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/account.css', 'all');
        $this->addJS(__EPH_BASE_URI__ . '/js/stdaccount.js');
    ...
    public function initContent() {
        ...
        $this->TitleBar = $this->la('Book account List');
    ...
    public function generateParaGridScript($regenerate = false) {
        ...
        $paragrid->title = '\'' . $this->la('Gestion des plan de comptes') . '\'';
            ...
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        ...
                        "add": {
                            name : \'' . $this->la('Ajouter un nouveau compte ') . '\',
                            icon: "add",
                         ...
                         "edit": {
                            name : \'' . $this->la('Modifier le compte  ') . '\'' . '+rowData.account,
                            icon: "edit",
                            visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                         ...
                         "delete": {
                            name : \'' . $this->la('Supprimer le compte  ') . '\'' . '+rowData.account,
                            icon: "delete",
                            visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
    ...
    public function getStdAccountFields() {
        ...
        return [
            ...
            [
                'title'    => $this->la('Compte de contrepartie'),
                'width'    => 150,
                'dataIndx' => 'counter',
                'valign'   => 'center',
                'dataType' => 'string',
    ...
    public function initPageHeaderToolbar() {
        ...
        if (empty($this->display)) {
            $this->page_header_toolbar_btn['new_stdaccount_account'] = [
                ...
                'desc'       => $this->la('Ajouter un nouveau compte', null, null, false),
                'identifier' => 'new',
                'controller' => $this->controller_name,
    ...
    public function renderForm() {
        ...
        $this->fields_form = [
            'legend' => [
                'title' => $this->la('Book Account'),
                'icon'  => 'icon-university',
            ],
            'input'  => [
                [
                    'type'     => 'text',
                    'label'    => $this->la('Book account number'),
                    'name'     => 'account',
                    'col'      => 2,
                    'required' => true,
                ],
                [
                    'type'     => 'text',
                    'label'    => $this->la('Book account name'),
                    'name'     => 'name',
                    'lang'     => true,
                    'required' => true,
                ],
	...
	public function ajaxProcessProcessSave() {
		...
		if ($result) {
			...
			$return = [
				'success' => false,
				'message' => $this->la('L‘ajout du compte à échouée'),
			];
		}
