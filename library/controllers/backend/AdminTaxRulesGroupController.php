...
class AdminTaxRulesGroupControllerCore extends AdminController {
    ...
    public function initContent() {
        ...
        $this->TitleBar = $this->la('Taxe rules Group');
    ...
    public function initPageHeaderToolbar() {
        ...
        if (empty($this->display)) {
            $this->page_header_toolbar_btn['new_tax_rules_group'] = [
                ...
                'desc'       => $this->la('Add new tax rules group', null, null, false),
    ...
    public function renderForm() {
        ...
        if (!isset($obj->id)) {
            ...
            $this->page_header_toolbar_btn['new'] = [
                'href' => '#',
                'desc' => $this->la('Add a new tax rule'),
            ];
            $content = parent::renderForm();
    ...
    public function initRuleForm() {
        ...
        $this->fields_form[0]['form'] = [
            'legend' => [
                'title' => $this->la('New tax rule'),
                'icon'  => 'icon-money',
            ],
            'input'  => [
                [
                    'type'    => 'select',
                    'label'   => $this->la('Country'),
                    'name'    => 'country',
                    'id'      => 'country',
                    'options' => [
                        ...
                        'default' => [
                            'value' => 0,
                            'label' => $this->la('All', 'AdminTaxRulesGroupController'),
                        ],
                    ],
                ],
                [
                ...
                [
                    'type'     => 'text',
                    'label'    => $this->la('Zip/postal code range'),
                    'name'     => 'zipcode',
                    'required' => false,
                    'hint'     => $this->la('You can define a range of Zip/postal codes (e.g., 75000-75015) or simply use one Zip/postal code.'),
                ],
                [
                    'type'     => 'select',
                    'label'    => $this->la('Behavior'),
                    'name'     => 'behavior',
                    'required' => false,
                    'options'  => [
                        'query' => [
                            [
                                'id'   => 0,
                                'name' => $this->la('This tax only'),
                            ],
                            [
                                'id'   => 1,
                                'name' => $this->la('Combine'),
                            ],
                            [
                                'id'   => 2,
                                'name' => $this->la('One after another'),
                            ],
                        ],
                        'id'    => 'id',
                        'name'  => 'name',
                    ...
                    'hint'     => [
                        $this->la('You must define the behavior if an address matches multiple rules:') . '<br>',
                        $this->la('- This tax only: Will apply only this tax') . '<br>',
                        $this->la('- Combine: Combine taxes (e.g.: 10% + 5% = 15%)') . '<br>',
                        $this->la('- One after another: Apply taxes one after another (e.g.: 0 + 10% = 0 + 5% = 5.5)'),
                    ],
                ],
                [
                    'type'     => 'select',
                    ...
                    'options'  => [
                        ...
                        'default' => [
                            'value' => 0,
                            'label' => $this->la('No Tax'),
                        ],
                    ],
                    'hint'     => sprintf($this->la('(Total tax: %s)'), '9%'),
                ],
                [
                    'type'  => 'text',
    ...
    public function initRulesList($idGroup) {
        ...
        $this->bulk_actions = [
            'delete' => ['text' => $this->la('Delete selected'), 'confirm' => $this->la('Delete selected items?'), 'icon' => 'icon-trash'],
        ];
