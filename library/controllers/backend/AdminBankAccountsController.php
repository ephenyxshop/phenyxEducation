...
class AdminBankAccountsControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Comptes bancaire');
        $this->lang = true;
    ...
    public function generateParaGridScript() {
        ...
        $this->paramTitle = '\'' . $this->la('Gestion des comptes bancaires') . '\'';
        ...
        $this->paramContextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    "edit": {
                            name: \'' . $this->la('Modifier la banque ') . ' \'+rowData.bank_name,
                            icon: "edit",
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer la banque') . ' \ : \'+rowData.bank_name,
                            icon: "trash",
    ...
    public function getBankAccountRequest() {
        ...
        if (!empty($bankaccounts)) {
            ...
            foreach ($bankaccounts as &$bankaccount) {
                ...
                if ($bankaccount['id_sale_agent'] > 0) {
                    $bankaccount['is_agent'] = '<div class="p-active"></div>';
                }

                if ($bankaccount['company_bank'] == 1) {
                    $bankaccount['is_company'] = '<div class="p-active"></div>';
                }

                if ($bankaccount['active'] == 1) {
                    $bankaccount['active'] = '<div class="p-active"></div>';
                } else {
                    $bankaccount['active'] = '<div class="p-inactive"></div>';
                }
    ...
    public function getBankAccountFields() {
        ...
        return [
            ...
            [
                'title'    => $this->la('Société ') . $this->context->company->name,
                'minWidth' => 100,
                'dataIndx' => 'is_company',
                'align'    => 'center',
                'valign'   => 'center',
            ...
            [
                'title'    => $this->la('Agent commercial'),
                'minWidth' => 100,
                'dataIndx' => 'is_agent',
                'align'    => 'center',
                'valign'   => 'center',
            ...
            [
                'title'    => $this->la('Nom de la banque'),
                'width'    => 150,
                'exWidth'  => 40,
                'dataIndx' => 'bank_name',
                'dataType' => 'string',
    ...
    public function ajaxProcessAddObject() {
		...
		if ($this->tabAccess['add'] == 1) {
			...
			$li = '<li id="uperAdd' . $targetController . '" data-controller="AdminDashboard"><a href="#contentAdd' . $targetController . '">Ajouter un compte bancaire</a><button type="button" class="close tabdetail" data-id="uperAdd' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
            ...
            $result = [
                'success' => false,
                'message' => 'Votre profile administratif ne vous permet pas d‘ajouter un compte bancaire',
            ];
        }
    ...
    public function ajaxProcessEditObject() {
        ...
        $li = '<li id="uperEdit' . $targetController . '" data-controller="AdminDashboard"><a href="#contentEdit' . $targetController . '">Edition d‘un compte bancaire</a><button type="button" class="close tabdetail" data-id="uperEdit' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
