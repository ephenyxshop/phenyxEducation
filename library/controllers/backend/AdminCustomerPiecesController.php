...
class AdminCustomerPiecesControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->publicName = $this->la('Factures Clients');
		$this->lang = false;
		...
		$this->pieceTypes = [
			'QUOTATION'    => $this->la('Devis'),
			'ORDER'        => $this->la('Commandes'),
			'DELIVERYFORM' => $this->la('Bons de Livraison'),
			'DOWNPINVOICE' => $this->la('Factures d‘accompte'),
			'INVOICE'      => $this->la('Factures'),
			'ASSET'        => $this->la('Avoirs'),
		];

		$this->pieceType = [
			'QUOTATION'    => $this->la('Devis'),
			'ORDER'        => $this->la('Commande'),
			'DELIVERYFORM' => $this->la('Bon de Livraison'),
			'DOWNPINVOICE' => $this->la('Facture d‘accompte'),
			'INVOICE'      => $this->la('Facture'),
			'ASSET'        => $this->la('Avoir'),
		];
	...
	public function generateParaGridScript() {
		...
		$this->paramPageModel = [
			'type'       => '\'local\'',
			'rPP'        => 40,
			'rPPOptions' => [10, 20, 40, 50, 100, 200, 500, 2000],
		];
		...
		$this->paramToolbar = [
				'items' => [
					...
					[
						'type'     => '\'button\'',
						'icon'     => '\'ui-icon-disk\'',
						'label'    => '\'' . $this->la('Corriger les écarts de numérotations') . '\'',
						'cls'      => '\'showFields changes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only\'',
						'listener' => 'correctNumberInvoice',
					],
					[
						'type'     => '\'button\'',
					...
					[
						'type'    => '\'select\'',
						'icon'    => '\'ui-icon-disk\'',
						'attr'    => '\'id="pieceTypeSelector"\'',
						'options' => '[
            			{"0": "Sélectionner le Type"},
						{"QUOTATION": "Devis"},
						{"ORDER": "Commande"},
		...
		$this->paramTitle = '\'' . $this->la('Gestion des pièces client') . '\'';
		$this->summaryData = '[{
                rank: \'Total\',
                summaryRow: true,
                pq_fn: {
		...
		$this->paramContextMenu = [
			'#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        "add": {
                            name: \'' . $this->la('Ajouter') . ' \',
                            icon: "add",
                            items: {
                                "quot": {
                                    name: \'' . $this->la('Un devis') . ' \',
                                    icon: "edit",
                                    callback: function(itemKey, opt, e) {
										generateNewPiece("QUOTATION");
                                    }
                                },
                                "pord": {
                                    name: \'' . $this->la('Une Commande') . ' \',
                                    icon: "edit",
                                    callback: function(itemKey, opt, e) {
                                        generateNewPiece("ORDER");
                                    }
                                },
                                "dform": {
                                    name: \'' . $this->la('Un bon de Livraison') . ' \',
                                    icon: "edit",
                                    callback: function(itemKey, opt, e) {
                                        generateNewPiece("DELIVERYFORM");
                                    }
                                },
                                "inv": {
                                    name: \'' . $this->la('Une facture') . ' \',
                                    icon: "edit",
                                    callback: function(itemKey, opt, e) {
                                        generateNewPiece("INVOICE");
                                    }
                                },
                                "asst": {
                                    name: \'' . $this->la('Un Avoir') . ' \',
                                    icon: "edit",
                                    callback: function(itemKey, opt, e) {
                                        generateNewPiece("ASSET");
                                    }
                                },
                            }
                        ...
                        "edit": {
                            name: \'' . $this->la('Modifier ou visualiser la pièce ') . ' \'+rowData.pieceType+ \' :\'+rowData.pieceNumber,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
                        ...
                        "validate": {
                            name: \'' . $this->la('Valider la Facture') . ' \ :\'+rowData.pieceNumber,
                            icon: "lock",
						...
						"bulkvalidate": {
                            name: \'' . $this->la('Valider les Factures sélectionnées') . '\',
                            icon: "lock",
                            visible: function(key, opt){
								var selected = selgrid' . $this->className . '.getSelection().length;
						...
						"regl": {
                            name: \'' . $this->la('Enregistrer le règlement de la') . ' \'+rowData.pieceType+ \' : \'+rowData.pieceNumber,
                            icon: "pay",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
					...
					"reglBulk": {
                            name: \'' . $this->la('Enregistrer le règlement des pièces sélectionnées') . ' \',
                            icon: "edit",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
							   var pieceSelected = selgrid' . $this->className . '.getSelection();
						...
						"book": {
                            name: \'' . $this->la('Comptabiliser la') . ' \'+rowData.pieceType+ \' : \'+rowData.pieceNumber,
                            icon: "book",
                            visible: function(key, opt){
								var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
						...
						"bulkbook": {
                            name: \'' . $this->la('Comptabiliser les Factures sélectionnées') . '\',
                            icon: "book",
                            visible: function(key, opt){
								var selected = selgrid' . $this->className . '.getSelection().length;
								var pieceSelected = selgrid' . $this->className . '.getSelection();
						...
						"print": {
                            name: \'' . $this->la('Imprimer la') . ' \'+rowData.pieceType+ \' : \'+rowData.pieceNumber,
                            icon: "print",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
						 ...
						 "bulkprint": {
                            name: \'' . $this->la('Imprimer les factures sélectionnées') . ' \',
                            icon: "print",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                 if(selected < 2) {
						 ...
						 "viewbook": {
                            name: \'' . $this->la('Ouvrir l‘écriture') . '\',
                            icon: "book",
                            visible: function(key, opt){
								var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                        ...
                        "sep1": "---------",
                        "select": {
                            name: \'' . $this->la('Tous sélectionner') . '\',
                            icon: "list-ul",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 2) {
                        ...
                        "unselect": {
                            name: \'' . $this->la('Tous désélectionner') . '\',
                            icon: "list-ul",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 2) {
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer la pièce ') . ' \ :\'+rowData.pieceNumber,
                            icon: "list-ul",
                            visible: function(key, opt){
								 var selected = selgrid' . $this->className . '.getSelection().length
                                var dataLenght = ' . 'grid' . $this->className . '.option(\'dataModel.data\').length;
	...
	public function getCustomerPiecesRequest() {
		...
		foreach ($orders as &$order) {
            ...
            $order['date_add'] = $date->format('d/m/Y');
			

			if (!empty($order['target_piece'])) {
				$order['last_transfert'] = $this->getStaticPrefix($order['target_type']) . $order['last_transfert'];
			}
	...
	public function getCustomerPiecesFields() {
		...
		return [
			...
			[
				'title'    => $this->la('Adresse de Livraison'),
				'width'    => 150,
				'exWidth'  => 20,
				'dataIndx' => 'address1',
				'align'    => 'left',
			...
			[
				'title'    => $this->la('Ville de livraison'),
				'width'    => 120,
				'exWidth'  => 20,
				'dataIndx' => 'city',
				'align'    => 'left',
			...
			[
				'title'    => $this->la('Code postal de Livraison'),
				'width'    => 100,
				'exWidth'  => 20,
				'dataIndx' => 'postcode',
				'align'    => 'left',
			...
			[
				'title'    => $this->la('Pays de livraison'),
				'width'    => 120,
				'exWidth'  => 30,
				'dataIndx' => 'country',
				'halign'   => 'HORIZONTAL_LEFT',
			...
			[
				'title'    => $this->la('Total Produit HT'),
				'width'    => 150,
			...
			[
				'title'    => $this->la('Frais de port HT.'),
				'width'    => 150,
			...
			[
				'title'        => $this->la('Total Produit TTC'),
				'width'        => 150,
			...
			[
				'title'    => $this->la('Marge de la pièce'),
				'width'    => 120,
			...
			[
				'title'    => $this->la('Date de la pièce'),
				'minWidth' => 150,
				'dataIndx' => 'date_add',
				'cls'      => 'rangeDate',
				'align'    => 'center',
	...
	public function ajaxProcessgetCustomerPiecesFields() {
		...
		die(Tools::jsonEncode($this->getCustomerPiecesFields()));
	}
	...
	public function getDetailCustomerPiecesRequest($idCustomerPiece) {
		...
		$file = fopen("testgetDetailCustomerPieces.txt","w");
		$piece = new CustomerPieces($idCustomerPiece);
		fwrite($file,(new DbQuery())
				->select('*')
				->from('customer_piece_detail')
				->where('`id_customer_piece` = ' . $piece->id)
				->orderBy('`id_customer_piece_detail` ASC').PHP_EOL);
		...
		fwrite($file, print_r($details, true));
		return $details;
	...
	public function ajaxProcessValidateCustomerPieces() {
		...
		$object = new CustomerPieces($id_customer_piece);
		$object->validate = 1;
		$result  =$object->update();
        if($result) {
          $return = [
			    'success' => true,
			 'message' => $this->la('La pièce a été validée avec succès '),
		  ];
        } else {
            $return = [
			    'success' => false,
			 'message' => $this->la('Un problème a été rencontré lors de la Validation de la Pièce'),
		  ];
        }
	...
	public function ajaxProcessDeleteCustomerPiece() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('La pièce à été supprimée avec succès'),
		];
		die(Tools::jsonEncode($result));
	}
	...
	public function ajaxProcessBulkDelete() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Les pièces clients ont été supprimées avec succès'),
		];
		die(Tools::jsonEncode($result));
	}
	...
	public function ajaxProcessEditObject() {
		...
		$li = '<li id="uperEdit' . $targetController . '" data-controller="AdminDashboard"><a href="#contentEdit' . $targetController . '">Modification de la : ' . $this->pieceType[$this->object->piece_type] . ' ' . $this->object->prefix . $this->object->piece_number . '</a><button type="button" class="close tabdetail" data-id="uperEdit' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
		...
		$result = [
            ...
            $result = [
				'success' => false,
				'message'   => 'Votre profile administratif ne vous permet pas d‘éditer les factures',
			];
        }
	...
	public function ajaxProcessGenerateNewPiece() {
		...
		$targetController = 'AdminCustomerPieces';
		...
		$li = '<li id="uperAdd' . $targetController . '" data-controller="AdminDashboard"><a href="#contentAdd' . $targetController . '">Ajouter une pièce client de type : ' . $this->pieceType[$type] . '</a><button type="button" class="close tabdetail" data-id="uperAdd' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
	...
	public function ajaxProcessSaveNewPiece() {
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('New pieces successfully added'),
			];
		}
	...
	public function ajaxProcessGetCustomerInformation() {
		...
		$customerLastAddress = $this->getLastCustomerAddressId((int) $customer->id);
		$addresses = $customer->getAddresses($this->context->language->id);
		$html = '<option value="0">' . $this->la('Select Address') . '</option>';
	...
	public function getStaticPieceName($pieceType) {
		...
		case 'QUOTATION':
			return $this->la('Devis');
			break;
		case 'ORDER':
			return $this->la('Commande');
			break;
		case 'DELIVERYFORM':
			return $this->la('Bon de Livraison');
			break;
		case 'DOWNPINVOICE':
			return $this->la('Facture Accompte');
			break;
		case 'INVOICE':
			return $this->la('Facture');
			break;
		case 'ASSET':
			return $this->la('Avoir');
			break;
		}
	...
	public function ajaxProcessMergeOrderTable() {
		...
		if (CustomerPieces::mergeOrderTable($idOrder)) {
			$response = [
				'success' => true,
				'message' => $this->la('Order ') . ' ' . $idOrder . ' ' . $this->la(' has been successfully merged'),
			];
		} else {
			$response = [
				'success' => false,
				'message' => $this->la('Order ') . ' ' . $idOrder . ' ' . $this->la(' has not been merged'),
			];
		}
	...
	public function ajaxProcessGenerateReglement() {
		...
		if ($studentPiece->is_education) {
			$date = new DateTime($studentPiece->date_add);
			$date->modify('+1 month');
			$dateAdd = $date->format('Y-m-d');
			$account = StdAccount::getAccountByName('411CDC');
	...
	public function ajaxProcessGenerateBulkReglement() {
		...
		foreach ($pieces as $key => $idPiece) {
			...
			if ($studentPiece->is_education > 0) {
				$date = new DateTime($studentPiece->date_add);
				$date->modify('+1 month');
				$dateAdd = $date->format('Y-m-d');
				$account = StdAccount::getAccountByName('411CDC');
				$reglement = 'Règlement Caisse dépot et consignation';
			} else {
				$dateAdd = date('Y-m-d');
				$account = new StdAccount($student->id_stdaccount);
					...
					foreach (Tools::getValue('orderBox') as $idCustomerPieces) {
						...
						if (!Validate::isLoadedObject($order)) {
							...
							if ($currentCustomerPiecesState->id == $orderState->id) {
								...
								if ($history->id_order_state == Configuration::get('EPH_OS_SHIPPING') && $order->shipping_number) {
									$templateVars = [
			...
			foreach ($products as &$product) {
				...
				foreach ($attributes as $attribute) {
					...
					if (!isset($combinations[$attribute['id_product_attribute']]['qty_in_stock'])) {
						$combinations[$attribute['id_product_attribute']]['qty_in_stock'] = StockAvailable::getQuantityAvailableByProduct((int) $product['id_product'], $attribute['id_product_attribute'], (int) $this->context->company->id);
					}
				...
				$product['stock'][0] = StockAvailable::getQuantityAvailableByProduct((int) $product['id_product'], 0, (int) $this->context->company->id);
	...
	public function ajaxProcessSendMailValidateCustomerPieces() {
		...
		if ($this->tabAccess['edit'] === '1') {
			...
			if (Validate::isLoadedObject($cart)) {
				...
				if (Validate::isLoadedObject($customer)) {
					...
					if (Mail::Send(
						...
						$this->ajaxDie(json_encode(['errors' => false, 'result' => $this->la('The email was sent to your customer.')]));
					}
			...
			$this->content = json_encode(['errors' => true, 'result' => $this->la('Error in sending the email to your customer.')]);
		}
	...
	protected function reinjectQuantity($orderDetail, $qtyCancelProduct, $delete = false) {
		...
		if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && $product->advanced_stock_management && $orderDetail->id_warehouse != 0) {
			...
			foreach ($movements as $movement) {
				...
				if (Pack::isPack((int) $product->id)) {
					...
					if ($product->pack_stock_type == 1 || $product->pack_stock_type == 2 || ($product->pack_stock_type == 3 && Configuration::get('EPH_PACK_STOCK_TYPE') > 0)) {
						$productsPack = Pack::getItems((int) $product->id, (int) Configuration::get('EPH_LANG_DEFAULT'));
						// Foreach item
	...
	public function ajaxProcessBookCustomerPiece() {
		...
		if ($piece->is_education) {

			$studentEducation = new StudentEducation($piece->id_student_education);
			$record->name = "Facture " . $piece->prefix . $piece->piece_number . ' Dossier n°' . $studentEducation->reference_edof . ' ' . $student->lastname . ' ' . $student->firstname;
			$record->piece_type = 'Pièce étudiant';
			$studentAccount = StdAccount::getAccountByName('411CDC');
	...
	public function ajaxProcessBulkBook() {
		...
		$result = [
			'success' => true,
			'message' => $this->la('Les pièces ont été comptabilisées avec succès'),
		];
	...
	public function ajaxProcessBulkValidate() {
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('Les pièces ont été mises à jour avec succès'),
			];
	...
	public function ajaxProcessPrintPieceToPdf() {
		...
		if ($customerPiece->is_education) {
			$model = Configuration::get('EPH_STUDENT_MODEL');
			$template = new InvoiceModel($model);
			$studentEducation = new StudentEducation($customerPiece->id_student_education);
			$educationType = new EducationType($studentEducation->educationType);
			$address = new Address();
		...
		$tag = date("H-i-s");
		$filePath = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'fileExport' . DIRECTORY_SEPARATOR;
		$fileName = $customerPiece->prefix . $customerPiece->piece_number .$tag. '.pdf';
		...
		$link = '<a  target="_blank"  class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="pieceDownloadFile" class="btn btn-default" href="' . $fileToUpload . '">' . $this->la('Voir, imprimer ou télécharger') . '</a>';
	...
	public function renderPdf($idPiece, $template) {
		...
		if ($customerPiece->is_education > 0) {
		...
		$tag = date("H-i-s");
		$filePath = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'invoices' . DIRECTORY_SEPARATOR;
		$fileName = $customerPiece->prefix . $customerPiece->piece_number .$tag. '.pdf';
		$mpdf->SetTitle($context->company->company_name . " " . $this->getStaticPieceName($customerPiece->piece_type) . " " . $customerPiece->prefix . $customerPiece->piece_number);
		$mpdf->SetAuthor($context->company->company_name);
		$mpdf->showWatermarkText = true;
	...
	public function ajaxProcessPrintPdf() {
		...
		$tag = date("H-i-s");
		$fileName = "Facture_" . $prefix . $customerPiece->piece_number . '_' . $customer->lastname . '_' . $customer->firstname . $tag.'.pdf';
	...
	public function ajaxProcessgenerateBulkprint() {
		...
		if ($zip->open(_EPH_EXPORT_DIR_ . 'export_facture' . $tag . '.zip', ZipArchive::CREATE) === TRUE) {
	...
	public function ajaxProcessExportSessionInvoices() {
		...
		$idPieces = CustomerPieces::getInvoicesbyidSession($session->id);
		...
		foreach ($idPieces as $idPiece) {
			...
			$customerPiece = new CustomerPieces($idPiece['id_customer_piece']);
			$prefix = $this->getStaticPrefix($customerPiece->piece_type);
			$customerPiece->nameType = $this->getStaticPieceName($customerPiece->piece_type);
			$productDetails = $customerPiece->getProductsDetail();
			$studentEducation = '';
			...
			$tag = date("H-i-s");
			$fileName = "Facture_" . $prefix . $customerPiecee->piece_number . '_' . $customer->lastname . '_' . $customer->firstname . $tag.'.pdf';
		...
		if ($zip->open('fileExport' . DIRECTORY_SEPARATOR . 'facture_' . $session->session_date . $tag . '.zip', ZipArchive::CREATE) === TRUE) {
			...
			$response = [
				'fileExport' => 'fileExport' . DIRECTORY_SEPARATOR . 'facture_' . $session->session_date . $tag . '.zip',
			];
			die(Tools::jsonEncode($response));
		}
	...
	public function ajaxProcessGeneratePhenyxInvoices() {
		...
		foreach ($invoices as $of => $sessions) {
			...
			if (is_array($sessions) && count($sessions)) {
				...
				foreach ($sessions as $session => $details) {
					...
					$newPiece->id_shop = (int) $this->context->company->id;
					$newPiece->id_shop_group = (int) $this->context->company->id_shop_group;
					...
					if ($result) {
						...
						if ($pack1 > 0) {
							...
							$object->product_name = $formatpack->name[$this->context->language->id];
						...
						if ($pack4 > 0) {
							$unitCost4 = round($piceCost4 / $pack4, 3);
							...
							$object->total_tax = $object->total_tax_incl - $object->total_tax_excl;
							$object->product_reference = $formatpack->reference;
							$object->product_wholesale_price = $unitCost4;
							$result = $object->add();
						}
	...
	public function ajaxProcessCorrectNumberInvoice() {

		$invoices = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				->select('id_customer_piece')
				->from('customer_pieces')
				->orderBy('`id_customer_piece` ASC ')
		);
		$k = 0;

		foreach ($invoices as $piece) {
			$k++;
			$invoice = new CustomerPieces($piece['id_customer_piece']);

			$year = substr($invoice->piece_number, 0, 4);
			$invoice->piece_number = $year . sprintf("%06s", $k);
			$invoice->update();
		}

		$result = [
			'success' => true,
			'message' => $this->la('La numérotation des pièces ont été mises à jour avec succès'),
		];
