...
class AdminAddressesControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->publicName = $this->la('Adresses Clients');
		$this->lang = false;
	...
	public function generateParaGridScript($regenerate = false) {
		...
		$this->countryAddressSelector = '<div class="pq-theme"><select id="countryAddressSelect"><option value="">' . $this->la('--Select--') . '</option>';
		...
		$paragrid->toolbar = [
			'items' => [
				...
				[
					'type'     => '\'button\'',
					'icon'     => '\'ui-icon-disk\'',
					'label'    => '\'' . $this->la('Ajouter une nouvelle adresse') . '\'',
		...
		$paragrid->contextMenu = [
			'#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        "add": {
                            name: \'' . $this->la('Ajouter une nouvelle adresse') . ' \',
                            icon: "add",
                        ...
                        "edit": {
                            name : \'' . $this->la('Visualiser ou modifier : ') . '\'' . '+rowData.alias,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
                        ...
                        "sep1": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer :') . '\'' . '+rowData.alias,
                            icon: "delete",
	...
	public function getAddressFields() {
		...
		return [
			...
			[
				...
				'hidden'   => false,
				'filter'   => [
					'attr'   => "id=\"countryAddressSelector\", placeholder=" . $this->la('--Select--') . " readonly",
					'crules' => [['condition' => "equal"]],
				],
			],
		];
	...
	public function renderForm() {
		...
		$this->fields_form = [
			...
			'input'  => [
				...
				[
					'type'     => 'text_customer',
					'label'    => $this->la('Customer'),
					'name'     => 'id_customer',
					'required' => false,
				],
				[
					'type'     => 'text',
					'label'    => $this->la('Identification Number'),
					'name'     => 'dni',
					'required' => false,
					'col'      => '4',
					'hint'     => $this->la('DNI / NIF / NIE'),
				],
				[
					'type'     => 'text',
					'label'    => $this->la('Address alias'),
					'name'     => 'alias',
					'required' => true,
					'col'      => '4',
		...
		foreach ($addressesFields as $addrFieldItem) {
			...
			if ($addrFieldItem == 'company') {
				...
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('VAT number'),
					'col'      => '2',
					'name'     => 'vat_number',
					'required' => in_array('vat_number', $requiredFields),
				];
			} else if ($addrFieldItem == 'lastname') {
			...
			} else if ($addrFieldItem == 'firstname') {
			...
			} else if ($addrFieldItem == 'address1') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('Address'),
					'name'     => 'address1',
					'col'      => '6',
					'required' => true,
				];
			} else if ($addrFieldItem == 'address2') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('Address') . ' (2)',
					'name'     => 'address2',
					'col'      => '6',
					'required' => in_array('address2', $requiredFields),
				];
			} else if ($addrFieldItem == 'postcode') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('Zip/Postal Code'),
					'name'     => 'postcode',
					'col'      => '2',
					'required' => true,
				];
			} else if ($addrFieldItem == 'city') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('City'),
					'name'     => 'city',
			...
			} else if ($addrFieldItem == 'country' || $addrFieldItem == 'Country:name') {
				$tempFields[] = [
					'type'          => 'select',
			...
			} else if ($addrFieldItem == 'phone') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('Home phone'),
					'name'     => 'phone',
					'required' => in_array('phone', $requiredFields) || Configuration::get('EPH_ONE_PHONE_AT_LEAST'),
					'col'      => '4',
					'hint'     => Configuration::get('EPH_ONE_PHONE_AT_LEAST') ? sprintf($this->la('You must register at least one phone number.')) : '',
				];
			} else if ($addrFieldItem == 'phone_mobile') {
				$tempFields[] = [
					'type'     => 'text',
					'label'    => $this->la('Mobile phone'),
					'name'     => 'phone_mobile',
	...
	public function ajaxProcessUpdateAddress() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('L‘adresse a mis à jour avec succès'),
		];
	...
	public function ajaxProcessAddAddress() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('L‘adresse a été ajoutée avec succès'),
		];
	...
	public function processSave() {
		...
		} else if ($idCustomer = Tools::getValue('id_customer')) {
			$customer = new Customer((int) $idCustomer);
