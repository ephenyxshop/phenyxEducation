...
class AdminModulesPositionsControllerCore extends AdminController {
    ...
    public function ajaxProcessSubmitAddToHook() {
        ...
        if (!$module->isHookableOn($hook->name)) {
            ...
            if (count($this->errors)) {
                ...
                $result = [
                    'success' => true,
                    'message' => $this->la('Le module a été greffé avec succès'),
                ];
            }
    ...
    public function renderForm() {
        ...
        foreach ($exceptsList as $company_id => $fileList) {
            $exceptionListDiff[] = $this->displayModuleExceptionList($fileList, $company_id);
        }
    ...
    public function displayModuleExceptionList($fileList, $idCompany) {
        ...
        $content = '<p><input type="text" name="exceptions[' . $idCompany . ']" value="' . implode(', ', $fileList) . '" id="em_text_' . $idCompany . '" placeholder="' . $this->la('E.g. address, addresses, attachment') . '"/></p>';
    ...
    public function initMain() {
        ...
        $adminDir = basename(_EPH_ROOT_DIR_);
        $modules = Module::getModulesInstalled();
        ...
        $this->toolbar_btn['save'] = [
            ...
            'desc' => $this->la('Transplant a module'),
        ];
    ...
    public function ajaxProcessSaveHook() {
        ...
        if ($this->tabAccess['edit'] === '1') {
            ...
            foreach ($hooksList as $idHook => $modules) {
                ...
                $sql = 'DELETE FROM `' . _DB_PREFIX_ . 'hook_module` WHERE `id_hook` =  ' . (int) $idHook;
                ...
                if (is_array($modules) && count($modules)) {
                    ...
                    foreach ($modules as $idModule) {
                        ...
                        if ($idModule && !in_array($idModule, $ids)) {
                            $ids[] = (int) $idModule;
                            $value .= '(' . (int) $idModule . ', ' . (int) $idHook . ', ' . (int) $i . '),';
                        }
                    ...
                    if ($value) {
                        $value = rtrim($value, ',');
                        $res &= Db::getInstance()->execute('INSERT INTO  `' . _DB_PREFIX_ . 'hook_module` (id_module, id_hook, position) VALUES ' . $value);
                    }
