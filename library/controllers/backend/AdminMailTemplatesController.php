...
class AdminMailTemplatesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Gestion des modèles de mails');
        $this->lang = true;
    ...
    public function generateParaGridScript() {
		...
		$paragrid->toolbar = [
			'items' => [
				...
				[
					'type'     => '\'button\'',
					'label'    => '\'' . $this->la('Scanner le repertoir Mail') . '\'',
        ...
        $paragrid->title = '\'' . $this->la('Gestion des modèle d‘email') . '\'';
        ...
        $paragrid->contextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    "edit": {
                            name: \'' . $this->la('Visualiser ou modifier ce modèle ') . ' \'+rowData.template,
                            icon: "edit",
					...
					"delete": {
           					name: \'' . $this->la('Supprimer la table :') . '\'+rowData.name,
           					icon: "delete",
    ...
    public function getMailTemplateFields() {
        ...
        return [
            ...
            [
                'title'    => $this->la('Fichier Template '),
                'maxWidth' => 300,
                'minWidth' => 300,
                'exWidth'  => 40,
                'dataIndx' => 'template',
    ...
    public function ajaxProcessScanDirectoryMail() {
		...
		$iterator->append(new DirectoryIterator(_SHOP_ROOT_DIR_ . '/mails/fr'));
    ...
    public function ajaxProcessEditMailTemplate() {
        ...
        $data->assign(
            [
                ...
                'tinymce'      => true,
                'iso'          => file_exists(_EPH_CORE_DIR_ . '/js/tiny_mce/langs/' . $iso . '.js') ? $iso : 'en',
	...
	public function ajaxProcessDeleteTemplate() {
		...
		$filePath = _SHOP_ROOT_DIR_.'/mails/fr/'.$template->template;
    ...
    public function ajaxProcessUpdateMailTemplate() {
        ...
        $file = fopen(_SHOP_ROOT_DIR_ . "/mails/fr/" . $mailTemplate->template, "w");
        fwrite($file, $output);
