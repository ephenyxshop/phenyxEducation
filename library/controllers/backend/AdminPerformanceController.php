...
class AdminPerformanceControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Mise en Cache et compression');
    ...
    public function initServerCaching() {
        ...
        $warningMemcache = ' ' . $this->la('(you must install the [a]Memcache PECL extension[/a])');
        ...
        $warningMemcached = ' ' . $this->la('(you must install the [a]Memcached PECL extension[/a])');
        ...
        $warningApc = ' ' . $this->la('(you must install the [a]APC PECL extension[/a])');
        ...
        $warningRedis = ' ' . $this->la('(you must install the [a]redis extension[/a])');
        ...
        $warningFs = ' ' . sprintf($this->la('(the directory %s must be writable)'), realpath(_EPH_CACHEFS_DIRECTORY_));
    ...
    public function initFieldsetSmarty() {
        ...
        $this->fields_form[0]['form'] = [
            ...
            'input'  => [
                ...
                [
                    'type'   => 'radio',
                    'label'  => $this->la('Template compilation'),
                    'name'   => 'smarty_force_compile',
                    'values' => [
                        [
                            'id'    => 'smarty_force_compile_' . _EPH_SMARTY_NO_COMPILE_,
                            'value' => _EPH_SMARTY_NO_COMPILE_,
                            'label' => $this->la('Never recompile template files'),
                            'hint'  => $this->la('This option should be used in a production environment.'),
                        ],
                        [
                            'id'    => 'smarty_force_compile_' . _EPH_SMARTY_CHECK_COMPILE_,
                            'value' => _EPH_SMARTY_CHECK_COMPILE_,
                            'label' => $this->la('Recompile templates if the files have been updated'),
                            'hint'  => $this->la('Templates are recompiled when they are updated. If you experience compilation troubles when you update your template files, you should use Force Compile instead of this option. It should never be used in a production environment.'),
                        ],
                        [
                            'id'    => 'smarty_force_compile_' . _EPH_SMARTY_FORCE_COMPILE_,
                            'value' => _EPH_SMARTY_FORCE_COMPILE_,
                            'label' => $this->la('Force compilation'),
                            'hint'  => $this->la('This forces Smarty to (re)compile templates on every invocation. This is handy for development and debugging. Note: This should never be used in a production environment.'),
                        ],
                    ],
                ],
                [
                    ...
                    'hint'    => $this->la('Should be enabled except for debugging.'),
                ],
                [
                    'type'   => 'radio',
                    'label'  => $this->la('Caching type'),
                    'name'   => 'smarty_caching_type',
                    'values' => [
                        [
                            'id'    => 'smarty_caching_type_filesystem',
                            'value' => 'filesystem',
                            'label' => $this->la('File System') . (is_writable(_EPH_CACHE_DIR_ . 'smarty/cache') ? '' : ' ' . sprintf($this->la('(the directory %s must be writable)'), realpath(_EPH_CACHE_DIR_ . 'smarty/cache'))),
                        ],
                ...
                [
                    'type'   => 'radio',
                    'label'  => $this->la('Clear cache'),
                    'name'   => 'smarty_clear_cache',
                    'values' => [
                        [
                            'id'    => 'smarty_clear_cache_never',
                            'value' => 'never',
                            'label' => $this->la('Never clear cache files'),
                        ],
                        [
                            'id'    => 'smarty_clear_cache_everytime',
                            'value' => 'everytime',
                            'label' => $this->la('Clear cache everytime something has been modified'),
                        ],
                    ],
                ],
            ],
    ...
    public function initFieldsetDebugMode() {
        ...
        $this->fields_form[1]['form'] = [
            'legend' => [
                'title' => $this->la('Debug mode'),
                'icon'  => 'icon-bug',
            ],
            'input'  => [
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Disable non ephenyx modules'),
                    'name'    => 'native_module',
                    'class'   => 't',
                    'is_bool' => true,
                    'values'  => [
                    ...
                    'hint'    => $this->la('Enable or disable non ephenyx modules.'),
                ],
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Disable all overrides'),
                    'name'    => 'overrides',
                    'class'   => 't',
                    'is_bool' => true,
                    'values'  => [
                    ...
                    'hint'    => $this->la('Enable or disable all classes and controllers overrides.'),
                ],
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Debug mode'),
                    'name'    => 'debug_mode',
                    'class'   => 't',
                    'is_bool' => true,
                    'values'  => [
                    ...
                    'hint'    => $this->la('Enable or disable debug mode.'),
                ],
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Profiling'),
                    'name'    => 'profiling',
                    'class'   => 't',
                    'is_bool' => true,
                    'values'  => [
                    ...
                    'hint'    => $this->la('Enable or disable profiling.'),
                ],
            ],
            'submit' => [
                'title' => $this->la('Save'),
    ...
    public function initFieldsetFeaturesDetachables() {
        ...
        $this->fields_form[2]['form'] = [
            'legend'      => [
                'title' => $this->la('Optional features'),
                'icon'  => 'icon-puzzle-piece',
            ],
            'description' => $this->la('Some features can be disabled in order to improve performance.'),
            'input'       => [
                [
                    'type' => 'hidden',
                ...
                [
                    ...
                    'hint'     => $this->la('Choose "No" to disable Product Combinations.'),
                    'desc'     => Combination::isCurrentlyUsed() ? $this->la('You cannot set this parameter to No when combinations are already used by some of your products') : null,
                ],
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Features'),
                    'name'    => 'feature',
                    'is_bool' => true,
                    'values'  => [
                        [
                    ...
                    'hint'    => $this->la('Choose "No" to disable Product Features.'),
                ],
    ...
    public function initFieldsetCCC() {
        ...
        $this->fields_form[3]['form'] = [
            'legend'      => [
                'title' => $this->la('Compression'),
                'icon'  => 'icon-fullscreen',
            ],
            'description' => $this->la('CCC allows you to reduce the loading time of your page. With these settings you will gain performance without even touching the code of your theme. Make sure, however, that your theme is compatible with ephenyx 1.0.x. Otherwise, CCC will cause problems.'),
            'input'       => [
                [
                    'type' => 'hidden',
                    'name' => 'ccc_up',
                ...
                [
                    'type'   => 'switch',
                    'label'  => $this->la('Smart cache for CSS'),
                    'name'   => 'EPH_CSS_THEME_CACHE',
                    'values' => [
                        [
                            'id'    => 'EPH_CSS_THEME_CACHE_1',
                            'value' => 1,
                            'label' => $this->la('Use CCC for CSS'),
                        ],
                        [
                            'id'    => 'EPH_CSS_THEME_CACHE_0',
                            'value' => 0,
                            'label' => $this->la('Keep CSS as original'),
                        ],
                    ],
                ],
                [
                    'type'   => 'switch',
                    'label'  => $this->la('Smart cache for JavaScript'),
                    'name'   => 'EPH_JS_THEME_CACHE',
                    'values' => [
                        [
                            'id'    => 'EPH_JS_THEME_CACHE_1',
                            'value' => 1,
                            'label' => $this->la('Use CCC for JavaScript'),
                        ],
                        [
                            'id'    => 'EPH_JS_THEME_CACHE_0',
                            'value' => 0,
                            'label' => $this->la('Keep JavaScript as original'),
                        ],
                    ],
                ],
                [
                    'type'   => 'switch',
                    'label'  => $this->la('Compress inline JavaScript in HTML'),
                    'name'   => 'EPH_JS_HTML_THEME_COMPRESSION',
                    'values' => [
                        [
                            'id'    => 'EPH_JS_HTML_THEME_COMPRESSION_1',
                            'value' => 1,
                            'label' => $this->la('Compress inline JavaScript in HTML after "Smarty compile" execution'),
                        ],
                        [
                            'id'    => 'EPH_JS_HTML_THEME_COMPRESSION_0',
                            'value' => 0,
                            'label' => $this->la('Keep inline JavaScript in HTML as original'),
                        ],
                    ],
                ],
                [
                    'type'   => 'switch',
                    'label'  => $this->la('Move JavaScript to the end'),
                    'name'   => 'EPH_JS_DEFER',
                    'values' => [
                        [
                            'id'    => 'EPH_JS_DEFER_1',
                            'value' => 1,
                            'label' => $this->la('Move JavaScript to the end of the HTML document'),
                        ],
                        [
                            'id'    => 'EPH_JS_DEFER_0',
                            'value' => 0,
                            'label' => $this->la('Keep JavaScript in HTML at its original position'),
                        ],
                    ],
                ],
            ],
        ...
        $this->fields_form[3]['form']['input'][] = [
            'type'   => 'switch',
            'label'  => $this->la('Apache optimization'),
            'name'   => 'EPH_HTACCESS_CACHE_CONTROL',
            'hint'   => $this->la('This will add directives to your .htaccess file, which should improve caching and compression.'),
            'values' => [
                [
        ...
        $this->fields_form[3]['form']['input'][] = [
            'type'   => 'switch',
            'label'  => $this->la('Keep JS and CSS files'),
            'desc'   => $this->la('Keep old JS and CSS files on the server, to make sure e.g. Google\'s cache still renders correctly (improves SEO).'),
    ...
    public function initFieldsetMediaServer() {
        ...
        $this->fields_form[4]['form'] = [
            'legend'      => [
                'title' => $this->la('Media servers (use only with CCC)'),
                'icon'  => 'icon-link',
            ],
            'description' => $this->la('You must enter another domain, or subdomain, in order to use cookieless static content.'),
            'input'       => [
                [
                    'type' => 'hidden',
                ...
                [
                    'type'  => 'text',
                    'label' => $this->la('Media server #1'),
                    'name'  => '_MEDIA_SERVER_1_',
                    'hint'  => $this->la('Name of the second domain of your shop, (e.g. myshop-media-server-1.com). If you do not have another domain, leave this field blank.'),
                ],
                [
                    'type'  => 'text',
                    'label' => $this->la('Media server #2'),
                    'name'  => '_MEDIA_SERVER_2_',
                    'hint'  => $this->la('Name of the third domain of your shop, (e.g. myshop-media-server-2.com). If you do not have another domain, leave this field blank.'),
                ],
                [
                    'type'  => 'text',
                    'label' => $this->la('Media server #3'),
                    'name'  => '_MEDIA_SERVER_3_',
                    'hint'  => $this->la('Name of the fourth domain of your shop, (e.g. myshop-media-server-3.com). If you do not have another domain, leave this field blank.'),
                ],
            ],
            'submit'      => [
                'title' => $this->la('Save'),
    ...
    public function initFieldsetCiphering() {
        ...
        $warningMcrypt = ' ' . $this->la('(You must install the [a]Mcrypt extension[/a])');
        ...
        $warningOpenssl = ' ' . $this->la('(You must install the [a]openssl extension[/a])');
        ...
        $this->fields_form[5]['form'] = [
            ...
            'legend'      => [
                'title' => $this->la('Ciphering'),
                'icon'  => 'icon-desktop',

            ],
            'description' => $this->la('Keep in mind that changing this setting will log everyone out!'),
            'input'       => [
                [
                    'type' => 'hidden',
                    'name' => 'ciphering_up',
                ...
                [
                    'type'   => 'radio',
                    'label'  => $this->la('Algorithm'),
                    'name'   => 'EPH_CIPHER_ALGORITHM',
                    'hint'   => $this->la('The Rijndael is faster than our custom BlowFish class, but requires the "mcrypt" or "openssl" PHP extension. If you change this configuration option, all cookies will be reset.'),
                    'values' => [
                        [
                            'id'    => 'EPH_CIPHER_ALGORITHM_2',
                            'value' => 2,
                            'label' => sprintf($this->la('Use the PHP Encryption library with the %s extension (highest security).'), $usePhpEncryptionWith) . (extension_loaded('openssl') ? '' : $warningOpenssl),
                        ],
                        [
                            'id'    => 'EPH_CIPHER_ALGORITHM_1',
                            'value' => 1,
                            'label' => sprintf($this->la('Use Rijndael with the %s extension.'), $useRijndaelWith) . (!extension_loaded('openssl') && !function_exists('mcrypt_encrypt') ? $warningOpenssl : (!extension_loaded('openssl') ? (!function_exists('mcrypt_encrypt') ? $warningMcrypt : '') : '')),
                        ],
                        [
                            'id'    => 'EPH_CIPHER_ALGORITHM_0',
                            'value' => 0,
                            'label' => $this->la('Use the custom BlowFish class.'),
                        ],
                    ],
                ],
            ],
    ...
    public function initFieldsetCaching() {
        ...
        $this->fields_form[6]['form'] = [
            'legend'           => [
                'title' => $this->la('Server Side Caching'),
                'icon'  => 'icon-desktop',
            ],
            'input'            => [
                [
                ...
                [
                    'type'   => 'radio',
                    'label'  => $this->la('Caching system'),
                    'name'   => 'EPH_CACHE_SYSTEM',
                    'hint'   => $this->la('The CacheFS system should be used only when the infrastructure contains one front-end server. If you are not sure, ask your hosting company.'),
                    'values' => [
                        [
                            'id'    => 'CacheFs',
                            'value' => 'CacheFs',
                            'label' => $this->la('File System') . (is_writable(_EPH_CACHEFS_DIRECTORY_) ? '' : $warningFs),
                        ],
                        [
                            'id'    => 'CacheMemcache',
                            'value' => 'CacheMemcache',
                            'label' => $this->la('Memcache via PHP::Memcache') . (extension_loaded('memcache') ? '' : $warningMemcache),
                        ],
                        [
                            'id'    => 'CacheMemcached',
                            'value' => 'CacheMemcached',
                            'label' => $this->la('Memcached via PHP::Memcached') . (extension_loaded('memcached') ? '' : $warningMemcached),
                        ],
                        [
                            'id'    => 'CacheApc',
                            'value' => 'CacheApcu',
                            'label' => $this->la('APC') . (extension_loaded('apcu') ? '' : $warningApc),
                        ],
                        [
                            'id'    => 'CacheRedis',
                            'value' => 'CacheRedis',
                            'label' => $this->la('redis') . (extension_loaded('redis') ? '' : $warningRedis),
                        ],
                    ],
                ],
                [
                    'type'  => 'text',
                    'label' => $this->la('Directory depth'),
    ...
    public function initFieldsetFullPageCache() {
        ...
        $this->fields_form[7]['form'] = [
            'legend'         => [
                'title' => $this->la('Full page cache'),
                'icon'  => 'icon-rocket',
            ],
            'description'    => $this->la('Before enabling the full page cache, make sure you have chosen a caching system in the panel above.'),
            'input'          => [
                [
                    'type'     => 'switch',
                    'label'    => $this->la('Use full page cache'),
                ...
                [
                    'type'    => 'switch',
                    'label'   => $this->la('Debug mode'),
                    'hint'    => $this->la('Enable this option to see the "X-ephenyx-PageCache" debug header'),
                ...
                [
                    'type'       => 'tags',
                    'label'      => $this->la('Ignore query parameters'),
                    'name'       => 'EPH_PAGE_CACHE_IGNOREPARAMS',
                    'hint'       => [
                        $this->la('To add parameters click in the field, write something, and then press "Enter."'),
                        $this->la('Invalid characters:') . ' &lt;&gt;;=#{}',
                    ],
                    'tagPrompt'  => $this->la('Add param'),
    ...
    public function displayControllerList($fileList, $idCompany) {
        ...
        $content = '<p><input type="text" name="EPH_PAGE_CACHE_CONTROLLERS" value="' . implode(', ', $fileList) . '" id="em_text_' . $idCompany . '" placeholder="' . $this->la('E.g. address, addresses, attachment') . '"/></p>';
        ...
        $content .= '<p>
                    ...
                    <option disabled="disabled">' . $this->la('___________ CUSTOM ___________') . '</option>';
        ...
        $content .= '<option disabled="disabled">' . $this->la('____________ CORE ____________') . '</option>';

        foreach ($controllers as $k => $v) {
            $content .= '<option value="' . $k . '">' . $k . '</option>';
        }

        $modulesControllersType = ['admin' => $this->la('Admin modules controller'), 'front' => $this->la('Front modules controller')];
