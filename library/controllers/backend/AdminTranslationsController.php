...
class AdminTranslationsControllerCore extends AdminController {
	...
	const DEFAULT_THEME_NAME = _THEME_NAME_;
	...
	public function __construct() {
		...
		$this->bootstrap = true;
		$this->publicName = $this->la('Translations');
	...
	public function ajaxProcessOpenTargetController() {
		...
		$targetController = 'AdminTranslations';
		...
		$li = '<li id="uper' . $targetController . '" data-controller="AdminDashboard" data-self="' . $this->link_rewrite . '" data-name="' . $this->page_title . '"><a href="#content' . $targetController . '">Gestion des Traductions</a><button type="button" class="close tabdetail" data-id="uper' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
	...
	public function getTranslationsInformations() {
		...
		$this->translations_informations = [
			'frontcontroller' => [
				'name' => $this->la('Front Controller translations'),
				'var'  => '_LANGFRONT',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'front.php',
			],
			'front'           => [
				'name' => $this->la('Front office translations'),
				'var'  => '_LANG',
				'dir'  => defined('_EPH_THEME_SELECTED_DIR_') ? _EPH_THEME_SELECTED_DIR_ . 'lang/' : '',
				'file' => $this->lang_selected->iso_code . '.php',
			],
			'class'           => [
				'name' => $this->la('Class translations'),
				'var'  => '_LANGCLASS',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'class.php',
			],
			'back'            => [
				'name' => $this->la('Back office translations'),
				'var'  => '_LANGADM',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'admin.php',
			],
			'errors'          => [
				'name' => $this->la('Error message translations'),
				'var'  => '_ERRORS',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'errors.php',
			],
			'fields'          => [
				'name' => $this->la('Field name translations'),
				'var'  => '_FIELDS',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'fields.php',
			],
			'modules'         => [
				'name' => $this->la('Installed modules translations'),
				'var'  => '_MODULES',
				'dir'  => _EPH_MODULE_DIR_,
				'file' => '',
			],
			'pdf'             => [
				'name' => $this->la('PDF translations'),
				'var'  => '_LANGPDF',
				'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
				'file' => 'pdf.php',
			],
			'mails'   => [
                'name' => $this->la('Email templates translations'),
                'var'  => '_LANGMAIL',
                'dir'  => _EPH_TRANSLATIONS_DIR_ . $this->lang_selected->iso_code . '/',
                'file' => 'mail.php',
            ],
		...
		if (defined('_EPH_THEME_SELECTED_DIR_')) {
			$this->translations_informations['modules']['override'] = ['dir' => _EPH_THEME_SELECTED_DIR_ . 'modules/', 'file' => ''];
	...
	public function processModuleTranslation($fieldTranslations, $missingTranslations) {
		...
		foreach ($fieldTranslations as $key => $values) {
			...
			foreach ($values as $media => $traductions) {
				...
				foreach ($traductions as $expression => $content) {
					...
					if (!empty($content['use_sprintf'])) {
						$use_sprintf = $this->la('This expression uses a special syntax:') . ' ' . $content['use_sprintf'];
					}
			...
			if (isset($missingTranslations[$key])) {
				$missing = ' ' . $missingTranslations[$key] . ' ' . $this->la('Missings');
				$collapse = 0;
			} else {
				$missing = '0 ' . $this->la('Missing');
			}

			$title = $key . ' : ' . sizeof($values) . ' ' . $this->la('Expressions') . $missing;
	...
	public function returnSpreadSheet($tabsArray, $missingTranslations) {
		...
		$name = $this->la('Translations-') . $this->lang_selected->iso_code . '-' . $this->type_selected . '.xlsx';
		$tag = $this->la('Translations') . $this->lang_selected->iso_code . $this->type_selected;

		$spreadsheet = new Spreadsheet();
		$spreadsheet->getProperties()->setCreator('Ephenyx Shop')
			->setTitle($this->type_selected)
			->setSubject($this->lang_selected->iso_code)
			->setDescription($this->la('Translations') . ' ' . $this->type_selected . ' ' . $this->la('Language:') . ' ' . $this->lang_selected->iso_code)
			->setKeywords((isset($this->theme_selected)) ? $this->theme_selected : '')
			->setCategory($this->la('Translations'));
		...
		foreach ($tabsArray as $key => $value) {
			...
			$richText->createText('');
			$richTextContent = $richText->createTextRun($key . ' ' . $nbField . ' ' . $this->la('Expressions '));
			$richTextContent->getFont()->setBold(true);
			$richTextContent->getFont()->setSize(14);

			$payable = $richText->createTextRun($missing . ' ' . $this->la('Missings'));
			...
			foreach ($value as $expression => $translation) {
				...
				if (isset($translation['use_sprintf']) && $translation['use_sprintf']) {
					$use_sprintf = $this->la('This expression uses a special syntax:') . ' {' . $translation['use_sprintf'] . '}';
				}
		...
		$filePath = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'translations' . DIRECTORY_SEPARATOR . $this->lang_selected->iso_code . DIRECTORY_SEPARATOR;
		...
		$result = [
			'success' => true,
			'link'    => '<a download="' . $name . '" id="translationFile" class="btn btn-default" href="' . $fileToUpload . '"><i class="process-export-excel"></i>' . $this->la('Click here to Download the file') . '</a>',
		];
		die(Tools::jsonEncode($result));
	...
	public function processGridModuleTranslate() {
		...
		if (method_exists($this, $methodName)) {
			...
			if (isset($this->translations_informations[$this->type_selected])) {
				...
				$result = [
					'success' => true,
					'message' => $this->la('Modules translations have been updated with success'),
				];
	...
	public function initForm($methodName) {
		...
		$title = sprintf(
			$this->la('%1$s (Language: %2$s, Theme: %3$s)'),
			$this->translations_informations[$this->type_selected]['name'],
			$this->lang_selected->name,
			$this->theme_selected ? $this->theme_selected : $this->la('none')
		);
	...
	public function displayToggleButton($closed = false) {
		...
		$strOutput .= '
            var openAll = \'' . html_entity_decode($this->la('Expand all fieldsets'), ENT_NOQUOTES, 'UTF-8') . '\';
            var closeAll = \'' . html_entity_decode($this->la('Close all fieldsets'), ENT_NOQUOTES, 'UTF-8') . '\';
        </script>
        <button type="button" class="btn btn-default" id="buttonall" data-status="open" onclick="toggleDiv(\'' . $this->type_selected . '_div\', $(this).data(\'status\')); toggleButtonValue(this.id, openAll, closeAll);"><i class="process-icon-compress"></i> <span>' . $this->la('Close all fieldsets') . '</span></button>';
	...
	public function getInformations() {
		...
		if ($this->theme_selected) {
			define('_EPH_THEME_SELECTED_DIR_', _EPH_ROOT_DIR_ . '/frontShop/' . $this->theme_selected . '/');
		} else {
	...
	public function submitCopyLang() {
		...
		if (!($fromLang = Tools::getValue('fromLang')) || !($toLang = Tools::getValue('toLang'))) {
			$this->errors[] = $this->la('You must select two languages in order to copy data from one to another.');
		} else

		if (!($fromTheme = Tools::getValue('fromTheme')) || !($toTheme = Tools::getValue('toTheme'))) {
			$this->errors[] = $this->la('You must select two themes in order to copy data from one to another.');
		} else

		if (!Language::copyLanguageData(Language::getIdByIso($fromLang), Language::getIdByIso($toLang))) {
			$this->errors[] = $this->la('An error occurred while copying data.');
		} else

		if ($fromLang == $toLang && $fromTheme == $toTheme) {
			$this->errors[] = $this->la('There is nothing to copy (same language and theme).');
		} else {
			...
			if ($themeExists['from_theme'] == false || $themeExists['to_theme'] == false) {
				$this->errors[] = $this->la('Theme(s) not found');
			}
		...
		foreach ($items as $source => $dest) {
			...
			if (!$this->checkDirAndCreate($dest)) {
				$this->errors[] = sprintf($this->la('Impossible to create the directory "%s".'), $dest);
			} else

			if (!copy($source, $dest)) {
				$this->errors[] = sprintf($this->la('Impossible to copy "%s" to "%s".'), $source, $dest);
			} else

			if (strpos($dest, 'modules') && basename($source) === $fromLang . '.php' && $bool !== false) {

				if (!$this->changeModulesKeyTranslation($dest, $fromTheme, $toTheme)) {
					$this->errors[] = sprintf($this->la('Impossible to translate "$dest".'), $dest);
				}
		...
		$this->errors[] = $this->la('A part of the data has been copied but some of the language files could not be found.');
	}
	...
	protected function checkDirAndCreate($dest) {
		...
		if (!file_exists($path)) {
			...
			if (!mkdir($path, 0777, true)) {
				$bool &= false;
				$this->errors[] = sprintf($this->la('Cannot create the folder "%s". Please check your directory writing permissions.'), $path);
			}
			...
			foreach ($files as $file) {
				...
				if (preg_match('/^(.*).tpl$/', $file) && (file_exists($filePath = $dir . $file))) {
					...
					foreach ($matches as $key) {
						...
						if (empty($key)) {
							$this->errors[] = sprintf($this->la('Empty string found, please edit: "%s"'), $filePath);
							$newLang[$key] = '';
						} else {
	...
	public function getFileToParseByTypeTranslation() {
		...
		case 'back':
			$directories = [
				...
				'tpl'      => $this->listFiles(_EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'template/'),
				'specific' => [
					_EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR => [
						'index.php',
					],
				],
			];
		...
		case 'errors':
			$directories['php'] = [
				...
				_EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR     => scandir(_EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR),
				_EPH_FRONT_CONTROLLER_DIR_                => scandir(_EPH_FRONT_CONTROLLER_DIR_),
				_EPH_ADMIN_CONTROLLER_DIR_                => scandir(_EPH_ADMIN_CONTROLLER_DIR_),
				_EPH_OVERRIDE_DIR_ . 'controllers/front/' => scandir(_EPH_OVERRIDE_DIR_ . 'controllers/front/'),
				_EPH_OVERRIDE_DIR_ . 'controllers/admin/' => scandir(_EPH_OVERRIDE_DIR_ . 'controllers/admin/'),
		...
		foreach ($filesPerDirectory['tpl'] as $dir => $files) {
			...
			foreach ($files as $file) {
				...
				if (preg_match('/^(.*).tpl$/', $file) && file_exists($filePath = $dir . $file)) {
					...
					$prefixKey = Tools::toCamelCase(str_replace(_EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'template', '', $filePath), true);
					...
					foreach ($matches as $englishString) {
						...
						if (empty($englishString)) {
							$this->errors[] = sprintf($this->la('There is an error in template, an empty string has been found. Please edit: "%s"'), $filePath);
