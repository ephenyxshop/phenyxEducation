...
class AdminCustomerThreadsControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Customer Thread');
        $this->lang = false;

        $this->fields_options = [
            'contact' => [
                'title'  => $this->la('Contact options'),
                'fields' => [
                    'EPH_CUSTOMER_SERVICE_FILE_UPLOAD' => [
                        'title' => $this->la('Allow file uploading'),
                        'hint'  => $this->la('Allow customers to upload files using the contact page.'),
                        'type'  => 'bool',
                    ],
                    'EPH_CUSTOMER_SERVICE_SIGNATURE'   => [
                        'title' => $this->la('Default message'),
                        'hint'  => $this->la('Please fill out the message fields that appear by default when you answer a thread on the customer service page.'),
                        'type'  => 'textareaLang',
                        'lang'  => true,
                    ],
                ],
            ...
            'general' => [
                'title'  => $this->la('Customer service options'),
                'fields' => [
                    'EPH_SAV_IMAP_URL'                 => [
                        'title' => $this->la('IMAP URL'),
                        'hint'  => $this->la('URL for your IMAP server (ie.: mail.server.com).'),
                        'type'  => 'text',
                    ],
                    'EPH_SAV_IMAP_PORT'                => [
                        'title'        => $this->la('IMAP port'),
                        'hint'         => $this->la('Port to use to connect to your IMAP server.'),
                        'type'         => 'text',
                        'defaultValue' => 143,
                    ],
                    'EPH_SAV_IMAP_USER'                => [
                        'title' => $this->la('IMAP user'),
                        'hint'  => $this->la('User to use to connect to your IMAP server.'),
                        'type'  => 'text',
                    ],
                    'EPH_SAV_IMAP_PWD'                 => [
                        'title' => $this->la('IMAP password'),
                        'hint'  => $this->la('Password to use to connect your IMAP server.'),
                        'type'  => 'text',
                    ],
                    'EPH_SAV_IMAP_DELETE_MSG'          => [
                        'title' => $this->la('Delete messages'),
                        'hint'  => $this->la('Delete messages after synchronization. If you do not enable this option, the synchronization will take more time.'),
                        'type'  => 'bool',
                    ],
                    'EPH_SAV_IMAP_CREATE_THREADS'      => [
                        'title' => $this->la('Create new threads'),
                        'hint'  => $this->la('Create new threads for unrecognized emails.'),
                        'type'  => 'bool',
                    ],
                    'EPH_SAV_IMAP_OPT_NORSH'           => [
                        'title' => $this->la('IMAP options') . ' (/norsh)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Do not use RSH or SSH to establish a preauthenticated IMAP sessions.'),
                    ],
                    'EPH_SAV_IMAP_OPT_SSL'             => [
                        'title' => $this->la('IMAP options') . ' (/ssl)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Use the Secure Socket Layer (TLS/SSL) to encrypt the session.'),
                    ],
                    'EPH_SAV_IMAP_OPT_VALIDATE-CERT'   => [
                        'title' => $this->la('IMAP options') . ' (/validate-cert)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Validate certificates from the TLS/SSL server.'),
                    ],
                    'EPH_SAV_IMAP_OPT_NOVALIDATE-CERT' => [
                        'title' => $this->la('IMAP options') . ' (/novalidate-cert)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Do not validate certificates from the TLS/SSL server. This is only needed if a server uses self-signed certificates.'),
                    ],
                    'EPH_SAV_IMAP_OPT_TLS'             => [
                        'title' => $this->la('IMAP options') . ' (/tls)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Force use of start-TLS to encrypt the session, and reject connection to servers that do not support it.'),
                    ],
                    'EPH_SAV_IMAP_OPT_NOTLS'           => [
                        'title' => $this->la('IMAP options') . ' (/notls)',
                        'type'  => 'bool',
                        'hint'  => $this->la('Do not use start-TLS to encrypt the session, even with servers that support it.'),
                    ],
                ],
                'submit' => ['title' => $this->la('Save')],
            ],
    ...
    public function setMedia() {
        ...
        $this->addCSS(_EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/sav.css', 'all', 0);

        $this->addJS([
            __EPH_BASE_URI__ . '/js/pgrid/pqSelect/pqselect.min.js',
            __EPH_BASE_URI__ . '/js/pgrid/pqgrid.min.js',
            __EPH_BASE_URI__ . '/js/pgrid/localize/pq-localize-fr.js',
            __EPH_BASE_URI__ . '/js/pgrid/pqTouch/pqtouch.min.js',
            __EPH_BASE_URI__ . '/js/pgrid/jsZip-2.5.0/jszip.min.js',
            __EPH_BASE_URI__ . '/js/pgrid/FileSaver.js',
            __EPH_BASE_URI__ . '/js/pgrid/javascript-detect-element-resize/jquery.resize.js',
    ...
    public function initContent() {
        ...
        if (isset($_GET['filename']) && file_exists(_EPH_UPLOAD_DIR_ . $_GET['filename']) && Validate::isFileName($_GET['filename'])) {
        ...
        $this->TitleBar = $this->la('Client relations List');
    ...
    public function generateParaGridScript($regenerate = false) {
        ...
        $contacts = '<div class="pq-theme"><select id="contactSelect"><option value="">' . $this->la('--Select--') . '</option>';
        ...
        $languages = '<div class="pq-theme"><select id="languageSelect"><option value="">' . $this->la('--Select--') . '</option>';
        ...
        $showStatus = '<div class="pq-theme"><select id="showStatusSelect"><option value="">' . $this->la('--Select--') . '</option>';
        $showStatus .= '<option value="open" data-content="icon-circle text-success">' . $this->la('Open') . '</option>';
        $showStatus .= '<option value="closed" data-content="icon-circle text-danger">' . $this->la('Closed') . '</option>';
        $showStatus .= '<option value="pending1" data-content="icon-circle text-warning">' . $this->la('Pending 1') . '</option>';
        $showStatus .= '<option value="pending2" data-content="icon-circle text-warning">' . $this->la('Pending 2') . '</option>';
        $showStatus .= '</select></div>';

        $employees = '<div class="pq-theme"><select id="employeeSelect"><option value="">' . $this->la('--Select--') . '</option>';
        ...
        $showPrivate = '<div class="pq-theme"><select id="showPrivateSelect"><option value="">' . $this->la('--Select--') . '</option>';
        $showPrivate .= '<option value="0" data-content="icon-remove">' . $this->la('No') . '</option>';
        $showPrivate .= '<option value="1" data-content="icon-check">' . $this->la('Yes') . '</option>';
        ...
        $paragrid->title = '\'' . $this->la('Management of') . ' ' . $this->publicName . '\'';
        $paragrid->fillHandle = '\'all\'';
        $paragrid->contextMenuoption = [

            'view'       => [
                'name' => '\'' . $this->la('View the message from: ') . '\'' . '+rowData.email',
                'icon' => '"edit"',
            ],
            'sep'        => [
                'sep1' => "---------",
            ...
            'delete'     => [
                'name' => '\'' . $this->la('Delete the selected message: ') . '\'' . '+rowData.email',
                'icon' => '"delete"',
            ],
            'bulkdelete' => [
                'name' => '\'' . $this->la('Delete all the selected message') . '\'',
                'icon' => '"delete"',
            ],
    ...
    public function getCustomerThreadFields() {
        ...
        return [
            ...
            [
                ...
                'dataType' => 'string',
                'filter'   => [
                    'attr'   => "id=\"contactSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
            ...
            [
                ...
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"languageSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
            ...
            [
                ...
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"showStatusSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
            ...
            [
                'title'    => $this->la('Employee'),
                'width'    => 200,
                'dataIndx' => 'employee',
                'dataType' => 'string',
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"employeeSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
                'title'    => $this->la('Messages'),
                'width'    => 200,
                'dataIndx' => 'messages',
                'dataType' => 'string',
                'editable' => false,
            ...
            [
                ...
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"showPrivateSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
                'title'    => $this->la('Last message'),
                'minWidth' => 150,
                'exWidth'  => 20,
                'dataIndx' => 'date_upd',
                'cls'      => 'rangeDate',
            ...
            if (isset($_POST['id_employee_forward'])) {
                ...
                if ($idEmployee && $employee && Validate::isLoadedObject($employee)) {
                    ...
                    if (Mail::Send(
                        ...
                        $cm->private = 1;
                        $cm->message = $this->la('Message forwarded to') . ' ' . $employee->firstname . ' ' . $employee->lastname . "\n" . $this->la('Comment:') . ' ' . $message;
                        $cm->add();
                    }
                ...
                if ($email && Validate::isEmail($email)) {
                    ...
                    if (Mail::Send(
                        ...
                        $cm->message = $this->la('Message forwarded to') . ' ' . Tools::convertEmailFromIdn($email) . "\n" . $this->la('Comment:') . ' ' . $message;
                        $cm->add();
                    }
    ...
    protected function displayMessage($message, $email = false, $idEmployee = null) {
        ...
        $tpl->assign(
            [
                'thread_url'        => Tools::getAdminUrl(basename(_EPH_ROOT_DIR_) . '/' . $this->context->link->getAdminLink('AdminCustomerThreads') . '&amp;id_customer_thread=' . (int) $message['id_customer_thread'] . '&amp;viewcustomer_thread=1'),
    ...
    public function renderView() {
        ...
        if ($nextThread) {
            $nextThread = [
                ...
                'name' => $this->la('Reply to the next unanswered message in this thread'),
            ];
        }
        ...
        if ($thread->status != 'pending1') {
            $actions['pending1'] = [
                ...
                'label' => $this->la('Mark as "pending 1" (will be answered later)'),
                'name'  => 'setstatus',
                'value' => 3,
            ];
        } else {
            $actions['pending1'] = [
                'href'  => static::$currentIndex . '&viewcustomer_thread&setstatus=1&id_customer_thread=' . (int) Tools::getValue('id_customer_thread') . '&viewmsg&token=' . $this->token,
                'label' => $this->la('Disable pending status'),
                'name'  => 'setstatus',
                'value' => 1,
            ];
        }
        ...
        if ($thread->status != 'pending2') {
            $actions['pending2'] = [
                ...
                'label' => $this->la('Mark as "pending 2" (will be answered later)'),
                'name'  => 'setstatus',
                'value' => 4,
            ];
        } else {
    ...
    public function getTimeline($messages, $idOrder) {
        ...
        foreach ($messages as $message) {
            ...
            if (!$message['private']) {
                $content .= $this->la('Message to: ') . ' <span class="badge">' . (!$message['id_employee'] ? $message['subject'] : $message['customer_name']) . '</span><br/>';
            }

            if (Validate::isLoadedObject($product)) {
                $content .= '<br/>' . $this->la('Product: ') . '<span class="label label-info">' . $product->name . '</span><br/><br/>';
            }
        ...
        if (Validate::isLoadedObject($order)) {
            ...
            foreach ($orderHistory as $history) {
                ...
                $content = '<a class="badge" target="_blank" href="' . Tools::safeOutput($linkOrder) . '">' . $this->la('Order') . ' #' . (int) $order->id . '</a><br/><br/>';

                $content .= '<span>' . $this->la('Status:') . ' ' . $history['ostate_name'] . '</span>';
