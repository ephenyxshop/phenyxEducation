...
class AdminPFGSubmissionsController extends AdminController
{
	...
	public function __construct()
	{
		...
		$this->bulk_actions = array('delete' => array('text' => $this->la('Delete selected'), 'confirm' => $this->la('Delete selected items?')));
	...
	public function initBreadcrumbs($tab_id = null, $tabs = null)
	{
		...
		$this->breadcrumbs[] = $this->la('Submissions');
	}
	...
	public function setFieldsList()
	{
		...
		{
			if ($i > 5) break; // Max number of columns to display
	...
	public function renderView()
	{
		...
		$view .= '<h1>'.sprintf($this->la('Submission of %s'), date($this->context->language->date_format_full, strtotime($result->date_add))).'</h1><hr />';
		$view .= '<ul style="margin-top: 40px; list-style-type: none">';
		foreach ($this->fields as $field)
		{
			if (!isset($entry[$field['name']])) continue;
			if ($field['type'] === 'separator' || $field['type'] === 'captcha') continue;
			...
			if ($field['type'] === 'email')
				$view .= '<a href="mailto:'.$entry[$field['name']].'" title="'.$this->la('Send an email to this person').'">'.$entry[$field['name']].'</a>';
			else if ($field['type'] === 'url' || $field['type'] === 'file')
			{
				if (is_array($entry[$field['name']]))
				{
					...
					foreach ($entry[$field['name']] as $element)
						$view .= '<li><a href="'.$element.'" title="'.$this->la('Click to open this link in a new window.').'" target="_blank">'.$element.'</a></li>';

					$view .= '</ul>';
				}
				else
					$view .= '<a href="'.$entry[$field['name']].'" title="'.
							$this->la('Click to open this link in a new window.').'" target="_blank">'.$entry[$field['name']].'</a>';
			}
			else
			{
				if (is_array($entry[$field['name']]))
				{
					...
					foreach ($entry[$field['name']] as $element)
					{
						if ($element === 'true') $element = $this->la('Yes');
						else if ($element === 'false') $element = $this->la('No');
				...
				{
					if ($entry[$field['name']] === 'true') $entry[$field['name']] = $this->la('Yes');
					else if ($entry[$field['name']] === 'false') $entry[$field['name']] = $this->la('No');
		...
		{
			$view .= '<li style="margin: 10px 0"><strong style="float: left">'.$this->la('Customer').' :</strong><div style="margin-left: 150px;">';
		...
		{
			$view .= '<li style="margin: 10px 0"><strong style="float: left">'.$this->la('Product').' :</strong><div style="margin-left: 150px;">';
		...
		$view .= '<p style="margin-top: 20px; padding-left: 185px;"><a href="'.Context::getContext()->link->getAdminLink('AdminPFGSubmissions').'&id_pfg='.(int)Tools::getValue('id_pfg').'" title="'.$this->la('Go back to the list of submissions').'" class="btn btn-default">'.$this->la('Go back to the list of submissions').'</a></p>';
