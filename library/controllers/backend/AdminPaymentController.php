...
class AdminPaymentControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Réglements');
        $this->lang = true;
    ...
    public function setMedia() {
        ...
        parent::setMedia();
        $this->addJS([
            __EPH_BASE_URI__ . '/js/payment.js',
        ]);
        Media::addJsDef([
    ...
    public function initContent() {
        ...
        $rangeMonths = Tools::getExerciceMonthRange();
        $sessions = EducationSession::getInvoicedEducationSession();
        $this->TitleBar = $this->la('Liste des règlements');
        ...
        $paragrid->contextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    "edit": {
                            name: \'' . $this->la('Modifier le ') . '\'+rowData.libelle,
                            icon: "edit",
                    ...
                    "view": {
                            name: \'' . $this->la('Consulter le ') . '\'+rowData.libelle,
                            icon: "view",
                            visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                         ...
                         "book": {
                            name: \'' . $this->la('Comptabiliser le ') . '\'+rowData.libelle,
                            icon: "book",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                        ...
                        "bulkbook": {
                            name: \'' . $this->la('Comptabiliser les Réglements sélectionnées') . '\',
                            icon: "book",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                         ...
                         "viewbook": {
                            name: \'' . $this->la('Voir l‘écriture de règlement du ') . '\'+rowData.libelle,
                            icon: "book",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer la Règlement ') . '\'+rowData.libelle,
                            icon: "list-ul",
                            visible: function(key, opt){
                                if(rowData.isBooked ==true) {
                                    return false;
    ...
    public function getPaymentFields() {
        ...
        return [
            ...
            [
                'title'        => $this->la('Montant'),
                'minWidth'     => 150,
                'exWidth'      => 40,
                'dataIndx'     => 'amount',
                'numberFormat' => '#,##0.00_-"€ ' . $this->la('TTC') . '"',
                'valign'       => 'center',
                'dataType'     => 'float',
                'format'       => '# ##0,00 € ' . $this->la('TTC'),
                'summary'      => [
                    'type' => 'sum',
                ],
            ],
    ...
    public function renderForm() {
        ...
        $bookdiary[] = [
            'id_book_diary' => 0,
            'name'          => $this->la('Select Associated Diary'),
        ];
        ...
        $this->fields_form = [
            ...
            'input'  => [
                ...
                [
                    'type'    => 'select',
                    'label'   => $this->la('Module payment association:'),
                    'name'    => 'id_module',
                    'col'     => 6,
                    'options' => [
    ...
    public function ajaxProcessBulkBook() {
        ...
        $result = [
            'success' => true,
            'message' => $this->la('Les Réglement ont été comptabilisées avec succès'),
        ];
