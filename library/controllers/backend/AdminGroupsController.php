...
class AdminGroupsControllerCore extends AdminController {
	...
	public function __construct() {
        ...
        $this->publicName = $this->la('Groups');
        $this->lang = true;
	...
	public function generateParaGridScript($regenerate = false) {
		...
		$showPrice = '<div class="pq-theme"><select id="showPriceSelect"><option value="">' . $this->la('--Select--') . '</option>';
        $showPrice .= '<option value="1">' . $this->la('Yes') . '</option>';
        $showPrice .= '<option value="0">' . $this->la('No') . '</option>';
        $showPrice .= '</select></div>';

        $displayMethod = '<div class="pq-theme"><select id="displayMethodSelect"><option value="">' . $this->la('--Select--') . '</option>';
        $displayMethod .= '<option value="' . EPH_TAX_EXC . '">' . $this->la('Tax excluded') . '</option>';
        $displayMethod .= '<option value="' . EPH_TAX_INC . '">' . $this->la('Tax included') . '</option>';
		...
		$paragrid->toolbar = [
			'items' => [
				...
				[
					'type'     => '\'button\'',
					'label'    => '\'' . $this->la('Ajouter un Groupe') . '\'',
		...
		$paragrid->title = '\'' .$this->la('Géstion des groupes Client') . '\'';
            ...
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        ...
                        "edit": {
                            name : \'' . $this->la('Modifier le groupe : ') . '\'' . '+rowData.name,
                            icon: "edit",
						 ...
						 "view": {
                            name : \'' . $this->la('Consulter le groupe : ') . '\'' . '+rowData.name,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
                        ...
                        "sep1": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer le groupe :') . '\'' . '+rowData.name,
                            icon: "delete",
    ...
    public function getGroupRequest() {
        ...
        foreach ($groups as &$group) {
            ...
            if ($group['price_display_method'] == EPH_TAX_EXC) {
                ...
                $group['price_display_method'] = $this->la('Tax excluded');
            } else {
                $group['displayMethod'] = EPH_TAX_INC;
                $group['price_display_method'] = $this->la('Tax included');
            }
    ...
    public function getGroupFields() {
        ...
        return [
            ...
            [
                ...
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"showPriceSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],
            [
            ...
            [
                ...
                'editable' => false,
                'filter'   => [
                    'attr'   => "id=\"displayMethodSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
            ],

            [
                'title'    => $this->la('Creation date'),
                'minWidth' => 150,
                'exWidth'  => 20,
				'cls' => 'rangeDate',
                'dataIndx' => 'date_add',
	...
	protected function renderCustomersList($group) {
        ...
        $this->fields_list = (array(
            'id_customer' => array('title' => $this->la('ID'), 'align' => 'center', 'filter_key' => 'c!id_customer', 'class' => 'fixed-width-xs'),
            'id_gender' => array('title' => $this->la('Social title'), 'icon' => $genders_icon, 'list' => $genders),
            'firstname' => array('title' => $this->la('First name')),
            'lastname' => array('title' => $this->la('Last name')),
            'email' => array('title' => $this->la('Email address'), 'filter_key' => 'c!email', 'orderby' => true),
            'birthday' => array('title' => $this->la('Birth date'), 'type' => 'date', 'class' => 'fixed-width-md', 'align' => 'center'),
            'date_add' => array('title' => $this->la('Registration date'), 'type' => 'date', 'class' => 'fixed-width-md', 'align' => 'center'),
            'active' => array('title' => $this->la('Enabled'), 'align' => 'center', 'class' => 'fixed-width-sm', 'type' => 'bool', 'search' => false, 'orderby' => false, 'filter_key' => 'c!active', 'callback' => 'printOptinIcon')
        ));
        ...
        $this->fields_form = [
            ...
            'input'  => [
                ...
                [
                    ...
                    'lang'     => true,
                    'col'      => 4,
                    'hint'     => $this->la('Forbidden characters:') . ' 0-9!&amp;lt;&amp;gt;,;?=+()@#"�{}_$%:',
                ],
                [
                    'type'   => 'text',
                    'label'  => $this->la('Discount'),
                    'name'   => 'reduction',
                    'suffix' => '%',
                    'col'    => 1,
                    'hint'   => $this->la('Automatically apply this value as a discount on all products for members of this customer group.'),
                ],
                [
                    'type'    => 'select',
                    'label'   => $this->la('Price display method'),
                    'name'    => 'price_display_method',
                    'col'     => 2,
                    'hint'    => $this->la('How prices are displayed in the order summary for this customer group.'),
                    'options' => [
                        'query' => [
                            [
                                'id_method' => EPH_TAX_EXC,
                                'name'      => $this->la('Tax excluded'),
                            ],
                            [
                                'id_method' => EPH_TAX_INC,
                                'name'      => $this->la('Tax included'),
                            ],
                        ],
                        'id'    => 'id_method',
                        'name'  => 'name',
                ...
                [
                    ...
                    'hint'     => $this->la('Customers in this group can view prices.'),
                ],
                [
                    'type'   => 'group_discount_category',
                    'label'  => $this->la('Category discount'),
                    'name'   => 'reduction',
                    'values' => ($group->id ? $this->formatCategoryDiscountList((int) $group->id) : [])
                ],
                [
                    'type'   => 'modules',
                    'label'  => $this->la('Modules Authorization'),
                    'name'   => 'auth_modules',
    ...
    public function displayEditLink($id, $token = null,  $name = null) {
        ...
        if (!array_key_exists('Edit', static::$cache_lang)) {
            static::$cache_lang['Edit'] = $this->la('Edit', 'Helper');
        }
