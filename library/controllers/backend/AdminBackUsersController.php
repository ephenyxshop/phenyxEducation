...
class AdminBackUsersControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Gestion des équipes back office');
        $this->lang = false;
        $this->context = Context::getContext();

        $path = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'themes' . DIRECTORY_SEPARATOR;
    ...
    public function generateParaGridScript() {
        ...
        $this->paramTitle = '\'' . $this->la('Gestion du personnels') . '\'';
        ...
        $this->paramContextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    "edit": {
                            name: \'' . $this->la('Modifier l‘employee ') . ' \'+rowData.firstname,
                            icon: "edit",
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer l‘employee ') . ' \ : \'+rowData.firstname,
                            icon: "trash",
                            visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                            ...
                            callback: function(itemKey, opt, e) {
								 if(currentProfileId > 2) {
									showErrorMessage( " Votre niveau d‘habilitation ne vous autorise pas à supprimer un employée" );
                                    return false;
                                }
                                if(rowData.id_profile ==1) {
									showErrorMessage("On ne peut pas supprimer un super Admin");
                                 return false;
                                }
    ...
    public function getEmployeeRequest() {
        ...
        $employees = Db::getInstance()->executeS(
            (new DbQuery())
                ->select('e.id_employee, e.id_profile, e.firstname, e.lastname, e.email, e.employee_type, e.active, case when e.active = 1 then \'<div class="p-active"><i class="fa fa-check" aria-hidden="true" style="color:green;"></i></div>\' else \'<div class="p-inactive"><i class="fa fa-times" aria-hidden="true" style="color:red;"></i></div>\' end as emplyee_state, pl.`name`')
                ->from('employee', 'e')
    ...
    public function ajaxProcessAddObject() {
		...
		if ($this->tabAccess['add'] == 1) {
        	...
        	$li = '<li id="uperAdd' . $targetController . '" data-controller="AdminDashboard"><a href="#contentAdd' . $targetController . '">Ajouter un eployé</a><button type="button" class="close tabdetail" data-id="uperAdd' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
            ...
            $result = [
                'success' => false,
                'message' => 'Votre profile administratif ne vous permet pas d‘ajouter un employee',
            ];
        }
    ...
    public function ajaxProcessEditObject() {
		...
		if ($this->tabAccess['edit'] == 1) {   
			...
			if(empty($employee->workin_plan)) {
				$fakeEmployee = new Employee();
				$employee->workin_plan = $fakeEmployee->workin_plan;
				$sql = 'INSERT INTO `' . _DB_PREFIX_ .	'employee_settings` (`id_employee`, `working_plan`) 
				VALUES ('.$employee->id.', \''. $fakeEmployee->workin_plan . '\')';
        	...
        	$li = '<li id="uperEdit' . $targetController . '" data-controller="AdminDashboard"><a href="#contentEdit' . $targetController . '">Modifier ' . $employee->firstname . ' ' . $employee->lastname . '</a><button type="button" class="close tabdetail" data-id="uperEdit' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
    ...
    public function ajaxProcessUpdateObject() {
		...
		if($result) {
            ...
            $result = [
            	'success' => true,
                'message' => $object->firstname.' '.$object->lastname.$this->la(' a été mis à jour avec succès'),
			];
    ...
    public function ajaxProcessAddNewAjaxObject() {
        ...
        if($result) {
            ...
            $result = [
            	'success' => true,
                'message' => $object->firstname.' '.$object->lastname.$this->la(' a été ajouté avec succès'),
			];
    ...
    protected function afterImageUpload() {
        ...
        if (($id_employee = (int) Tools::getValue('id_employy')) &&
            ...
            $current_file = _EPH_TMP_IMG_DIR_ . 'emplyee_mini_' . $id_employee . '_' . $this->context->company->id . '.jpg';
