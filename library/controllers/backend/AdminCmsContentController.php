...
class AdminCmsContentControllerCore extends AdminController {
	...
	public function __construct() {
		...
		$this->publicName = $this->la('Gestion des contenus de type CMS');
	...
	public function ajaxProcessOpenTargetController() {
		...
		$cmsCategories = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				...
				->from('cms_category', 'a')
				->leftJoin('cms_category_lang', 'b', 'b.`id_cms_category` = a.`id_cms_category` AND b.`id_lang` = ' . $this->context->language->id)
		...
		$li = '<li id="uper' . $targetController . '" data-controller="AdminDashboard"><a href="#content' . $targetController . '">Gestion des Contenu CMS</a><button type="button" class="close tabdetail" data-id="uper' . $targetController . '"><i class="icon icon-times-circle"></i></button></li>';
		...
		foreach ($cmsCategories as $category) {
			...
			$paragrid->paragrid_option['paragrids'][] = [
        		...
        		return {
            		...
            		items: {
						"add": {
							name: \'' . $this->la('Ajouter une nouvelle page CMS') . '\',
							icon: "add",
                		...
                		"edit": {
							name: \'' . $this->la('Modifier la page ') . ' : \'+rowData.meta_title,
							icon: "edit",
                			callback: function(itemKey, opt, e) {
						...
						"open": {
							name: \'' . $this->la('Voir dans une nouvelle fenêtre ') . ' : \'+rowData.meta_title,
							icon: "eye",
                			callback: function(itemKey, opt, e) {
           				...
           				"delete": {
           					name: \'' . $this->la('Supprimer la page') . ' : \'+rowData.meta_title,
           					icon: "delete",
	...
	public function getCMSRequest($id_cms_category) {
		...
		$cmsPages = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				->select('b.*, a.*, cml.`name`')
				->from('cms', 'a')
				->leftJoin('cms_lang', 'b', 'b.`id_cms` = a.`id_cms` AND b.`id_lang` = ' . $this->context->language->id)
				->leftJoin('cms_category_lang', 'cml', 'cml.`id_cms_category` = a.`id_cms_category` AND cml.`id_lang` = ' . $this->context->language->id)
	...
	public function ajaxProcessUpdateCms() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('La page CMS a été mise à jour avec succès'),
		];
	...
	public function ajaxProcessAddPageCms() {
		...
		if ($result) {
			$return = [
				'success' => true,
				'message' => $this->la('La page CMS a été ajoutée avec succès'),
			];
		} else {
			$return = [
				'success' => false,
				'message' => $this->la('L‘ajout de la catégorie de page CMS à échouée'),
			];
		}
	...
	public function ajaxProcessAddPageCmsCategorie() {
		...
		if ($result) {
			...
			$return = [
				'success' => false,
				'message' => $this->la('L‘ajout de la catégorie de CMS à échouée'),
			];
		}
	...
	public function ajaxProcessupdateCmsCategorie() {
		...
		$return = [
			'success' => true,
			'message' => $this->la('La catégorie CMS a été mise à jour avec succès'),
			'action'  => 'update',
		];
	...
	public function ajaxProcessUpdateCmsPositions() {
		...
		if (empty($this->errors)) {
			$result = [
				'success' => true,
				'message' => $this->la('La position des pages a été mise à jour avec succès.'),
			];
		} else {
