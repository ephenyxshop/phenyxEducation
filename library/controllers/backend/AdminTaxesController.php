...
class AdminTaxesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        if (Configuration::get('EPH_USE_ECOTAX')) {
            $ecotaxDesc = $this->la('If you disable the ecotax, the ecotax for all your products will be set to 0.');
        }
        ...
        $this->fields_options = [
            'general' => [
                'title'  => $this->la('Tax options'),
                'fields' => [
                    'EPH_TAX'                             => [
                        'title' => $this->la('Enable tax'),
                        'desc'  => $this->la('Select whether or not to include tax on purchases.'),
                        'cast'  => 'intval', 'type' => 'bool',
                    ],
                    'EPH_TAX_DISPLAY'                     => [
                        'title' => $this->la('Display tax in the shopping cart'),
                        'desc'  => $this->la('Select whether or not to display tax on a distinct line in the cart.'),
                        'cast'  => 'intval',
                        'type'  => 'bool',
                    ],
                    'EPH_TAX_ADDRESS_TYPE'                => [
                        'title'      => $this->la('Based on'),
                        'cast'       => 'pSQL',
                        'type'       => 'select',
                        'list'       => [
                            [
                                'name' => $this->la('Invoice address'),
                                'id'   => 'id_address_invoice',
                            ],
                            [
                                'name' => $this->la('Delivery address'),
                    ...
                    'EPH_DEFAULT_SPECIFIC_PRICE_RULE_TAX' => [
                        'title'      => $this->la('Default tax for specific price rules'),
                        'desc'       => $this->la('This is the default tax that applies to specific price rules. If you enter a specific price rule with tax and the customer can checkout without paying taxes, then this tax will be subtracted from the specific price rule amount. Does not apply to percentage discounts.'),
                        'validation' => 'isInt',
                        'cast'       => 'intval',
                        'type'       => 'select',
                        'list'       => $availableTaxes,
        ...
        if (Configuration::get('EPH_USE_ECOTAX') || Tools::getValue('EPH_USE_ECOTAX')) {
            $this->fields_options['general']['fields']['EPH_ECOTAX_TAX_RULES_GROUP_ID'] = [
                'title'      => $this->la('Ecotax'),
                'hint'       => $this->la('Define the ecotax (e.g. French ecotax: 19.6%).'),
                'cast'       => 'intval',
                'type'       => 'select',
    ...
    public function generateParaGridScript($regenerate = false) {
        ...
        $activeSelector = '<div class="pq-theme"><select id="activeSelect"><option value="">' . $this->la('--Select--') . '</option><option value="0">' . $this->la('Disable') . '</option><option value="1">' . $this->la('Enable') . '</option></select></div>';
        ...
        $paragrid->contextMenuoption = [
            ...
            'add'        => [
                'name' => '\'' . $this->la('Add new ') . $this->publicName . '\'',
                'icon' => '"add"',
            ],
            'edit'       => [
                'name' => '\'' . $this->la('Edit the ') . $this->publicName . ' :\'' . '+rowData.name',
                'icon' => '"edit"',
            ],
            'sep'        => [
                'sep1' => "---------",
            ...
            'delete'     => [
                'name' => '\'' . $this->la('Delete the ') . $this->publicName . ' :\'' . '+rowData.name',
                'icon' => '"delete"',
            ],
            'bulkdelete' => [
                'name' => '\'' . $this->la('Delete the selected ') . $this->publicName . '\'',
                'icon' => '"delete"',
            ],
    ...
    public function getTaxFields() {
        ...
        return [
            ...
            [
                ...
                'dataType' => 'html',
                'filter'   => [
                    'attr'   => "id=\"activeSelector\", placeholder=" . $this->la('--Select--') . " readonly",
                    'crules' => [['condition' => "equal"]],
                ],
    ...
    public function initPageHeaderToolbar() {
        ...
        $this->page_header_toolbar_btn['new_tax'] = [
            ...
            'desc'       => $this->la('Add new tax', null, null, false),
            'identifier' => 'new',
    ...
    public function displayDeleteLink($id, $token = null) {
        ...
        if (!array_key_exists('Delete', static::$cache_lang)) {
            static::$cache_lang['Delete'] = $this->la('Delete');
        }

        if (!array_key_exists('DeleteItem', static::$cache_lang)) {
            static::$cache_lang['DeleteItem'] = $this->la('Delete item #', __CLASS__, true, false);
        }

        if (TaxRule::isTaxInUse($id)) {
            $confirm = $this->la('This tax is currently in use as a tax rule. Are you sure you\'d like to continue?', null, true, false);
        }
    ...
    public function displayEnableLink($token, $id, $value, $active, $idCategory = null, $idProduct = null) {
        ...
        if ($value && TaxRule::isTaxInUse($id)) {
            $confirm = $this->la('This tax is currently in use as a tax rule. If you continue, this tax will be removed from the tax rule. Are you sure you\'d like to continue?', null, true, false);
        }
        ...
        $this->fields_form = [
            ...
            'input'  => [
                [
                    ...
                    'required' => true,
                    'lang'     => true,
                    'hint'     => $this->la('Tax name to display in carts and on invoices (e.g. "VAT").') . ' - ' . $this->la('Invalid characters') . ' <>;=#{}',
                ],
                [
                    'type'      => 'text',
                    'label'     => $this->la('Rate'),
                    ...
                    'maxlength' => 6,
                    'required'  => true,
                    'hint'      => $this->la('Format: XX.XX or XX.XXX (e.g. 19.60 or 13.925)') . ' - ' . $this->la('Invalid characters') . ' <>;=#{}',
                ],
                [
                    'type'     => 'switch',
                    'label'    => $this->la('Enable'),
                    'name'     => 'active',
                    'required' => false,
                    'is_bool'  => true,
                    'values'   => [
            ...
            if (!count($this->errors)) {
                ...
                if (isset($id) && !empty($id)) {
                    ...
                    if (Validate::isLoadedObject($object)) {
                        ...
                        } else if ($this->postImage($object->id)) {
