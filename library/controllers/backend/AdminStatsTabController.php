...
abstract class AdminStatsTabControllerCore extends AdminPreferencesControllerCore {
    ...
    public function initContent() {
        ...
        $this->toolbar_title = $this->la('Stats', 'AdminStatsTab');
    ...
    public function displayMenu() {
        ...
        foreach ($modules as $index => $module) {
            ...
            } else if ($moduleInstance[$module['name']] = Module::getInstanceByName($module['name'])) {
                $modules[$index]['displayName'] = $moduleInstance[$module['name']]->displayName;
            } else {
    ...
    public function displayCalendar() {
        ...
        return AdminStatsTabController::displayCalendarForm(
            [
                'Calendar' => $this->la('Calendar', 'AdminStatsTab'),
                'Day'      => $this->la('Day', 'AdminStatsTab'),
                'Month'    => $this->la('Month', 'AdminStatsTab'),
                'Year'     => $this->la('Year', 'AdminStatsTab'),
                'From'     => $this->la('From:', 'AdminStatsTab'),
                'To'       => $this->la('To:', 'AdminStatsTab'),
                'Save'     => $this->la('Save', 'AdminStatsTab'),
            ],
            $this->token
        );
    }
    ...
    public function processDateRange() {
        ...
        if (Tools::getValue('submitDateDay')) {
            $from = date('Y-m-d');
            $to = date('Y-m-d');
        }

        if (Tools::getValue('submitDateDayPrev')) {
            $yesterday = time() - 60 * 60 * 24;
            $from = date('Y-m-d', $yesterday);
            $to = date('Y-m-d', $yesterday);
        }

        if (Tools::getValue('submitDateMonth')) {
            $from = date('Y-m-01');
            $to = date('Y-m-t');
        }

        if (Tools::getValue('submitDateMonthPrev')) {
        ...
        if (Tools::getValue('submitDateYear')) {
            $from = date('Y-01-01');
            $to = date('Y-12-31');
        }

        if (Tools::getValue('submitDateYearPrev')) {
    ...
    public function ajaxProcessSetMainboardDateRange() {
        ...
        if ($this->isXmlHttpRequest()) {
            fwrite($file, "isXmlHttpRequest" . PHP_EOL);
    ...
    protected function displayEngines() {
        ...
        $autocleanPeriod = [
            'never' => $this->la('Never', 'AdminStatsTab'),
            'week'  => $this->la('Week', 'AdminStatsTab'),
            'month' => $this->la('Month', 'AdminStatsTab'),
            'year'  => $this->la('Year', 'AdminStatsTab'),
        ];
