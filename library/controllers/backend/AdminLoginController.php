...
class AdminLoginControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->meta_title = $this->la('Administration panel');
        $this->css_files = [];
        parent::__construct();
        $this->layout = _EPH_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'template' . DIRECTORY_SEPARATOR . 'controllers' . DIRECTORY_SEPARATOR . 'login' . DIRECTORY_SEPARATOR . 'layout.tpl';
    ...
    public function setMedia() {
        ...
        $this->addJS('/js/jquery/plugins/validate/localization/messages_' . $this->context->language->iso_code . '.js');
        ...
        Media::addJsDef(['img_dir' => _EPH_IMG_]);
        Media::addJsDefL('one_error', $this->la('There is one error.', null, true, false));
        Media::addJsDefL('more_errors', $this->la('There are several errors.', null, true, false));
    ...
    public function ajaxProcessPassswordForgot() {
        ...
        if (!count($this->errors)) {
            $this->ajaxDie(json_encode([
                'hasErrors' => false,
                'confirm'   => sprintf($this->la('A new password has been emailed to the given email address, if it wasn\'t done within the last %s minutes before.', 'AdminTab', false, false), Configuration::get('EPH_PASSWD_TIME_BACK')),
            ]));
