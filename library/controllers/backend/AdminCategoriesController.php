...
class AdminCategoriesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Categories');
        $this->lang = true;
    ...
    public function generateParaGridScript() {
        ...
        $this->filterModel = [];
        $this->paramPageModel = [];
        $this->paramTitle = '\'' . $this->la('Management of Product Categories') . '\'';
        $this->paramToolbar = [
            'items' => [
                ...
                [
                    'type'     => '\'button\'',
                    'icon'     => '\'ui-icon-disk\'',
                    'label'    => '\'' . $this->la('Ajouter une famille') . '\'',
        ...
        $this->paramContextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                        "add": {
                            name: \'' . $this->la('Ajouter une nouvelle famille de produit') . '\',
                            icon: "add",
                        ...
                        "edit": {
                            name: \'' . $this->la('Editer ou modifier la famille  ') . '\'+rowData.name,
                            icon: "edit",
                            callback: function(itemKey, opt, e) {
                        ...
                        "delete": {
                            name: \'' . $this->la('Supprimer la famille de formation ') . '\'+rowData.name,
                            icon: "delete",
    ...
    public function ajaxProcessAddNewCategory() {
        ...
        $cacheId = 'scriptHeaderCategory_BackOfficeHeader';
        ...
        $cacheId = 'scriptFooterCategory_BackOfficeHeader';
        if (!Cache::isStored($cacheId)) {
    ...
    public function renderForm() {
        ...
        $unidentifiedGroupInformation = sprintf($this->la('%s - All people without a valid customer account.'), '<b>' . $unidentified->name[$this->context->language->id] . '</b>');
        $guestGroupInformation = sprintf($this->la('%s - Customer who placed an order with the guest checkout.'), '<b>' . $guest->name[$this->context->language->id] . '</b>');
        $defaultGroupInformation = sprintf($this->la('%s - All people who have created an account on this site.'), '<b>' . $default->name[$this->context->language->id] . '</b>');
        ...
        $this->fields_form = [
            ...
            'input'   => [
                ...
                [
                    'type'     => 'switch',
                    'label'    => $this->la('Display products from subcategories'),
                ...
                [
                    'type'          => 'image',
                    'label'         => $this->la('Category Cover Image'),
                    'name'          => 'image',
                    'display_image' => true,
                ...
                [
                    ...
                    'rows'    => 5,
                    'cols'    => 100,
                    'hint'    => $this->la('Forbidden characters:') . ' <>;=#{}',
                ],
                [
                    'type'    => 'textarea',
                ...
                [
                    ...
                    'name'  => 'meta_keywords',
                    'lang'  => true,
                    'hint'  => $this->la('To add "tags," click in the field, write something, and then press "Enter."') . '&nbsp;' . $this->la('Forbidden characters:') . ' <>;=#{}',
                ],
                [
                    'type'     => 'text',
                    ...
                    'lang'     => true,
                    'required' => true,
                    'hint'     => $this->la('Only letters, numbers, underscore (_) and the minus (-) character are allowed.'),
                ],
                [
                    'type'              => 'group',
                    'label'             => $this->la('Group access'),
                    'name'              => 'groupBox',
                    'values'            => Group::getGroups($this->context->language->id),
                    'info_introduction' => $this->la('You now have three default customer groups.'),
                    'unidentified'      => $unidentifiedGroupInformation,
                    'guest'             => $guestGroupInformation,
                    'customer'          => $defaultGroupInformation,
                    'hint'              => $this->la('Mark all of the customer groups which you would like to have access to this category.'),
                ],
            ],
            'submit'  => [
                'title' => $this->la('Save'),
        ...
        if ($this->object->id == 0) {
            $this->fields_form['input'][] = [
                'type'   => 'radio',
                'label'  => $this->la('Position of this new Category'),
                    'name'   => 'wayAdd',
                    'default_val' => 'up',
                    'class'  => 't',
                    'values' => [
                        [
                            'id'    => 'up',
                            'value' => 'up',
                            'label' => $this->la('First position relaying on parent'),
                        ],
                        [
                            'id'    => 'down',
                            'value' => 'down',
                            'label' => $this->la('Last position relaying on parent'),
                        ],
                    ],
            ];
        }
        ...
        if ($this->object->id > 0) {
            ...
            $this->editObject = $this->la('Edit and update the category ') . $obj->name[$this->context->language->id];
        } else {
            $this->form_action = 'addCategory';
            $this->editObject = $this->la('Add new Category');
        }
    ...
    public function ajaxProcessAddCategory() {
        ...
        if ($result) {
            ...
            $return = [
                'success' => true,
                'message' => $this->la('La catégorie a été ajoutée avec succès'),
            ];
        } else {
            $return = [
                'success' => false,
        ...
        if ($idCategory && Validate::isUnsignedId($idCategory)) {
            ...
            if (Validate::isLoadedObject($category)) {
                ...
                if ($result) {
                    ...
                    $return = [
                        'success' => true,
                        'message' => $this->la('La catégorie a été mise à jour avec succès'),
                    ];
                }
    ...
    public function ajaxProcessCleanPositions() {
        ...
        if (empty($this->errors)) {
            ...
            $result = [
                'success' => true,
                'message' => $this->la('The data base position fields has been cleaned'),
            ];
        } else {
    ...
    public function ajaxProcessUpdatePositions() {
        ...
        if (empty($this->errors)) {
            $result = [
                'success' => true,
                'message' => $this->la('Tab position has been successfully updated.'),
            ];
        } else {
            $this->errors = array_unique($this->errors);
            $result = [
    ...
    public function ajaxProcessStatusCategory() {
        ...
        if (!$idCategory = (int) Tools::getValue('id_category')) {
            $this->ajaxDie(json_encode(['success' => false, 'error' => true, 'text' => $this->la('Failed to update the status')]));
        } else {
            ...
            if (Validate::isLoadedObject($category)) {
                ...
                $category->save() ?
                $this->ajaxDie(json_encode(['success' => true, 'text' => $this->la('The status has been updated successfully')])) :
                $this->ajaxDie(json_encode(['success' => false, 'error' => true, 'text' => $this->la('Failed to update the status')]));
            }
