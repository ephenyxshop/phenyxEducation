...
class AdminEmployeeMenusControllerCore extends AdminController {
	...
	public function __construct() {
		...
		//$this->extracss = $this->pushCSS([_EPH_ADMIN_THEME_DIR_.  $this->bo_theme . '/css/employee_menus.css']);
	...
	public function generateParaGridScript() {
		...
		$this->paramTitle = '\'' . $this->la('Gestion du Menu Back Office') . '\'';
		...
		$this->paramContextMenu = [
			'#grid_' . $this->controller_name => [
        		...
        		return {
            		...
            		items: {
                        "add": {
							name: \'' . $this->la('Add a new Supplier:') . '\',
							icon: "add",
                			callback: function(itemKey, opt, e) {
								addChildAjaxObject("' . $this->controller_name . '", rowData.id_employee_menu)
                			}
						},

                		"edit": {
							name: \'' . $this->la('Edit the menu: ') . '\'+rowData.name,
							icon: "edit",
                			callback: function(itemKey, opt, e) {
						...
						"sep2": "---------",
           				"delete": {
           					name: \'' . $this->la('Delete the selected menu:') . '\'+rowData.name,
           					icon: "delete",
							visible: function(key, opt){
	...
	public function getEmployeeMenuRequest($idParent = 0) {
		...
		$employee_menus = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
			(new DbQuery())
				->select('b.name, a.id_employee_menu, a.class_name, a.id_parent, a.id_employee_menu as id,  a.function, a.active, a.position')
		...
		foreach ($employee_menus as &$employee_menu) {
			...
			if ($employee_menu['active'] == 1) {
				$employee_menu['active'] = '<div class="toggle-on" onClick="menuInactive(' . $employee_menu['id_employee_menu'] . ')"></div>';
				$employee_menu['enable'] = true;
			} else {
				$employee_menu['active'] = '<div class="toggle-off" onClick="menuActive(' . $employee_menu['id_employee_menu'] . ')"></div>';
	...
	public function ajaxProcessUpdateEmployeeMenu() {
		...
		if ($result) {
			...
			$return = [
				'success' => true,
				'message' => $this->la('Le  menu a été mis à jour avec succès'),
			];
		} else {
			$return = [
				'success' => false,
				'message' => $this->la('Il y a eu un problème lors de la mise à jour du menu'),
			];
		}
	...
	public function ajaxProcessNewEmployeeMenu() {
		...
		if ($result) {
			...
			$return = [
				'success' => true,
				'message' => $this->la('Le nouveau menu a été crée avec succès'),
			];
		} else {

			$return = [
				'success' => false,
				'message' => $this->la('Il y a eu un problème lors de la création du nouveau menu'),
			];
		}
