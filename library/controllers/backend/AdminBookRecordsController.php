...
class AdminBookRecordsControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Liste des écritures');
        $this->lang = true;
    ...
    public function generateParaGridScript() {
        ...
        $this->paramPageModel = [
            'type'       => '\'local\'',
            'rPP'        => 20,
            'rPPOptions' => [10, 20, 40, 50, 100, 200, 500, 1000],
        ];

        $this->paramTitle = '\'' . $this->la('Gestion des Lignes d‘écriture comptable') . '\'';
        ...
        $this->paramContextMenu = [
            '#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    items: {
                         "add": {
                            name: \'' . $this->la('Saisie au kilomètre ') . '\',
                            icon: "book",
                        ...
                        "viewbook": {
                            name: \'' . $this->la('Voir ') . '\'+rowData.piece_type,
                            icon: "book",
                            visible: function(key, opt){
                                var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer le ') . '\'+rowData.libelle,
                            icon: "list-ul",
                            visible: function(key, opt){
    ...
    public function getBookRecordsRequest() {
        ...
        $bookRecords = Db::getInstance()->executeS(
            (new DbQuery())
                ->select('brd.*, s.account, sl.`name` as accountName, br.date_add as recordDate, bdl.code as diaryCode, br.id_book_diary, br.piece_type, br.validate')
                ...
                ->where('br.date_add >= \''.$this->context->company->accounting_period_start.'\'')
                ->where('br.date_add <= \''.$this->context->company->accounting_period_end.'\'')
                ->orderBy('br.`date_add` DESC')
        );
        ...
        foreach ($bookRecords as &$bookRecord) {
            ...
            $date = new DateTime($bookRecord['recordDate']);
            $bookRecord['recordDate'] = $date->format('d/m/Y');
            ...
            if ($bookRecord['piece_number'] > 0) {
                $piece = new CustomerPieces($bookRecord['piece_number']);
    ...
    public function getBookRecordsFields() {
        ...
        return [
            ...
            [
                'title'    => $this->la('Libellé du compte'),
                'maxWidth' => 250,
    ...
    public function ajaxProcessgetBookRecordsFields() {
        ...
        die(Tools::jsonEncode($this->getBookRecordsFields()));
    }
