...
class AdminSupplierPiecesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Pièces Fournisseur');
        $this->lang = false;
    ...
    public function setMedia() {
        ...
        Media::addJsDef([
            'AjaxLinkAdminSupplierPieces' => $this->context->link->getAdminLink('AdminSupplierPieces'),
        ]);
	...
	public function ajaxProcessOpenTargetController() {
		...
		foreach($agents as $agent) {
			if($agent->sale_commission_amount > 0) {
				$agent->due =  StudentEducation::getCommissionDueBySaleAgent($agent->id);
				if($agent->due['grandTotal'] > 0) {
	...
	public function generateParaGridScript($regenerate = false) {
		...
		$this->paramToolbar = [
            'items' => [               
				...
				[
                    'type'     => '\'button\'',
                    'icon'     => '\'ui-icon-disk\'',
                    'label'    => '\'' . $this->la('Ajouter une facture pour un CEF') . '\'',
		...
		$this->paramTitle = '\'' .$this->la('Gestion des pièces Fournisseur') .  '\'';
		$this->summaryData = '[{
                rank: \'Total\',
                summaryRow: true,
                pq_fn: {
                    ...
                    items: {
                        ...
                        "edit": {
                            ...
                            callback: function(itemKey, opt, e) {
								...
								editAjaxObject("' . $this->controller_name . '", rowData.id_supplier_piece)
                            }
                        },
    ...
    public function getDetailSupplierPiecesFields() {
        ...
        return [
            ...
            [
                'title'    => $this->la('Produit / Formation'),
                'width'    => 180,
                'align'    => 'left',
    ...
    public function getDetailSupplierPiecesRequest($idSupplierPiece) {
        ...
        $details = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
            (new DbQuery())
            ->select('spd.*, CONCAT(c.firstname, \' \', c.lastname) as student')
            ->from('supplier_piece_detail', 'spd')
            ->leftJoin('sale_agent_commission', 'sac', 'sac.`id_sale_agent_commission` = spd.`id_sale_agent_commission`')
            ->leftJoin('student_education', 'se', 'se.`id_student_education` = sac.`id_student_education`')
            ->leftJoin('customer', 'c', 'c.`id_customer` = se.`id_customer`')
            ->where('spd.`id_supplier_piece` = ' . $idSupplierPiece)
            ->orderBy('spd.`id_supplier_piece_detail` ASC')
        );
   ...
   public function ajaxProcessValidateSupplierPieces() {
	...
	public function ajaxProcessBulkValidate() {
		...
		if (count($this->errors)) {
			...
			$result = [
				'success' => true,
				'message' => $this->la('Les pièces ont été validées avec succès'),
			];
	...
	public function ajaxProcessDeleteSupplierPiece() {
		...
		$result = [
        	'success' => true,
            'message' => $this->la('La pièce a été supprimée avec succès'),
        ];
		die(Tools::jsonEncode($result));
	}
    ...
    public function ajaxProcessEditObject() {
		...
		$targetController = $this->targetController;
        $id_supplier_piece = Tools::getValue('idObject');
        $this->object = new $this->className($id_supplier_piece);
                    ...
                    return {
                        ...
                        toolbar: {
                                ...
                                },
                                {
                                    type: \'button\',
                                    cls: \'ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only\',
                                    label: \'' . $this->la('Delete the product') . '\',
                                    listener: function() {
	...
	public function ajaxProcessUpdateSupplierPiece() {
		...
		$file = fopen("testProcessUpdateSupplierPiece.txt","w");
		$id_supplier_piece = Tools::getValue('id_supplier_piece');
		...
		$old_balance = $piece->balance_due;
        fwrite($file, 'old Balance '.$old_balance.PHP_EOL);
        $old_total_paid = $piece->total_paid;
        fwrite($file, 'old total paid '.$old_total_paid.PHP_EOL);
        $total_paid = Tools::getValue('total_paid');
        fwrite($file, 'total paid '.$total_paid.PHP_EOL);
        ...
        if($total_paid > $old_balance) {
            $piece->total_paid = $old_balance;
        }
		...
		if($result) {
			...
			foreach ($pieceDetails as $details) {
                $object = new SupplierPieceDetail($details['id_supplier_piece_detail']);
            ...
            $generateOverDue = Tools::getValue('generateOverDue');
            if($generateOverDue) {
                if($this->generateReglement($piece->id)) {
                    $piece->validate = 1;
                    $piece->update();
                }
                $over_paid = Tools::getValue('over_paid');
                fwrite($file, 'over_paid '.$over_paid.PHP_EOL);
                $saleAgent = new SaleAgent($piece->id_sale_agent);
                if($saleAgent->is_tax) {
                    $base_tax_excl = $over_paid /1.2;
                    $total_tax = $over_paid - $base_tax_excl;
                    $tax_rate = 20;
                } else {
                    $base_tax_excl = $over_paid;
                    $total_tax = 0;
                    $tax_rate = 0;
                }
                    
                $newPiece = new SupplierPieces();
                $newPiece->id_piece_origine = $piece->id;
                $newPiece->piece_type = 'ASSET';
                $newPiece->id_shop_group = 1;
		        $newPiece->id_shop = 1;
		        $newPiece->id_lang = 1;
		        $newPiece->id_currency = 1;
		        $newPiece->id_sale_agent = $piece->id_sale_agent;
                $newPiece->base_tax_excl = $base_tax_excl;
                $newPiece->total_products_tax_excl = $base_tax_excl;
                $newPiece->total_products_tax_incl = $over_paid;
                $newPiece->total_with_freight_tax_excl = $base_tax_excl;
                $newPiece->total_tax_excl = $base_tax_excl;
                $newPiece->total_tax = $total_tax;
                $newPiece->total_tax_incl = $over_paid;
                $newPiece->id_payment_mode = $piece->id_payment_mode;
                $newPiece->id_address_delivery = $piece->id_address_delivery;
                $newPiece->id_address_invoice = $piece->id_address_invoice;
                $newPiece->conversion_rate = $piece->conversion_rate;
                $newPiece->round_mode = $piece->round_mode;
                $newPiece->round_type = $piece->round_type;
                $newPiece->piece_number = SupplierPieces::getIncrementByType($newPiece->piece_type);
                $newPiece->validate = 1;
                $newPiece->observation = 'Trop Payé pour '.$saleAgent->firstname.' '.$saleAgent->lastname;
                $newPiece->date_add = date("Y-m-d H:i:s");
                $result = $newPiece->add();
                if($result) {
                    $detail = new SupplierPieceDetail();
				    $detail->id_supplier_piece = $newPiece->id;
				    $detail->product_name = 'Trop Payé';
				    $detail->product_quantity = 1;
				    $detail->product_reference = 'OVERPAYMENT';
				    $detail->tax_rate = $tax_rate;
				    $detail->id_tax_rules_group = 1;
				    $detail->original_price_tax_excl = $base_tax_excl;
				    $detail->original_price_tax_incl = $over_paid;
				    $detail->unit_tax_excl = $base_tax_excl;
				    $detail->unit_tax_incl = $over_paid;
				    $detail->total_tax_excl = $base_tax_excl;
				    $detail->total_tax = $total_tax;
				    $detail->total_tax_incl = $over_paid;
                    $result = $detail->add();
                }
            }
            
            $generatePayment = Tools::getValue('generatePayment');
            if($generatePayment) {
                if($this->generateReglement($piece->id)) {
                    $piece->validate = 1;
                    $piece->update();
            ...
            $generatePartialPayment = Tools::getValue('generatePartialPayment');
            fwrite($file, 'generatePartialPayment '.$generatePartialPayment.PHP_EOL);
        ...
        if (count($this->errors)) {
            ...
            $result = [
                'success' => true,
                'message' => $this->la('La pièce à été mise à jour avec succès'),
            ];
        }
    ...
    public function generateReglement($idPiece) {
		...
		if($supplierPiece->id_sale_agent > 0) {
			...
			foreach($details as $detail) {
				$studeneducation = new StudentEducation($detail['id_student_education']);
                $studeneducation->commission_paid = 1;
                $studeneducation->update();
	...
	public function ajaxProcessgenerateBulkprint() {
		...
		foreach ($idPieces as $idPiece) {
        	...
        	if ($customerPiece->validate == 0 && $customerPiece->piece_type == 'INVOICE') {
				$watermark = $this->la('Provisoire');
            	$mpdf->SetWatermarkText($watermark);
        	} else  if ($customerPiece->validate == 1 && $customerPiece->piece_type == 'INVOICE') {
            	$mpdf->SetProtection(['copy', 'print'], '', _DB_PASSWD_);
			}
	...
	public function ajaxProcessGenerateReglement() {
		...
		if($supplierPiece->id_sale_agent > 0) {
			...
			foreach($details as $detail) {
				$studentEducation = new StudentEducation($detail['id_student_education']);
                $studentEducation->commission_paid = 1;
	...
	public function ajaxProcessGetDetailSaleAgentRequest() {
		...
		foreach($commissions as &$commission) {
            ...
            if($commission['student'] == 'Trop payé Financier') {
                
                $commission['id_sale_agent_commission'] = $commission['id_student_education'];
			    $commission['educationName'] = $commission['sessionName'];
			    $commission['amount_tax_incl'] = $commission['amount']*$rate;
			    $commission['total_line'] = $commission['amount']*$rate;
			    $commission['sessionName'] = $commission['sessionName'];
	...
	public function ajaxProcessGenerateCefFacture() {
		...
		if($result) {
			...
			foreach($pieces as $invoice) {
                if($invoice['sessionName'] == 'Non Applicable') {
                    $suplierPiece = new SupplierPieces($invoice['id_sale_agent_commission']);
                    $suplierPiece->total_paid = $suplierPiece->total_tax_incl;
                    $suplierPiece->validate = 1;
                    $suplierPiece->update();
                } else {
                    ...
                    $detail->id_student_education = $studentEducation->id;
	...
	public function ajaxProcessDeleteCommission() {
        ...
        $studentEducation = new StudentEducation($commission->id_student_education);
        $studentEducation->commission_amount = 0;
        $studentEducation->commission_due = 0;
        $studentEducation->update();
		$commission->delete();
	...
	public function ajaxProcessRemoveCommission() {
		...
		$return = [
			'success' => true,
            'message' => 'Cette commission a été définitivement supprimée'
		];
