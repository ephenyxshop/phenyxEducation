...
class AdminProfilesControllerCore extends AdminController {
    ...
    public function __construct() {
        ...
        $this->publicName = $this->la('Employee profiles');
	...
	public function initContent() {
		...
		$this->TitleBar = $this->la('Liste des agents commerciaux');
    ...
    public function generateParaGridScript($regenerate = false) {
		...
		$paragrid->toolbar = [
			'items' => [
				...
				[
					'type'     => '\'button\'',
					'label'    => '\'' . $this->la('Ajouter un Profile') . '\'',
				...
				[
					'type'     => '\'button\'',
					'label'    => '\'' . $this->la('Supprimer ce Profile') . '\'',
		...
		$paragrid->title = '\'' .$this->la('Gestion des profils utilisateurs du back office') .  '\'';
	...
	public function ajaxProcessUpdateByVal() {
		...
		if (Validate::isLoadedObject($sponsor)) {
			...
			if (!isset($result) || !$result) {
				...
				$result = [
					'success' => true,
					'message' => $this->la('Le champ a été mis à jour avec succès'),
				];
			}
    ...
    public function postProcess() {
        ...
        if (isset($_GET['delete' . $this->table]) && $_GET[$this->identifier] == (int) (_EPH_ADMIN_PROFILE_)) {
            $this->errors[] = $this->la('For security reasons, you cannot delete the Administrator\'s profile.');
        } else {
            parent::postProcess();
        }
