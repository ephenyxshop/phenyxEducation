...
class AdminPFGController extends AdminController {
			...
			'#grid_' . $this->controller_name => [
                ...
                return {
                    ...
                    "edit": {
						...
						"manage": {
                            name: \'' . $this->la('Configurer les champs de  ') . ' \'+rowData.title,
                            icon: "config",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
						...
						"detail": {
                            name: \'' . $this->la('Voir les  Soummissions de ') . ' \'+rowData.title,
                            icon: "view",
							visible: function(key, opt){
                               var selected = selgrid' . $this->className . '.getSelection().length;
                                if(selected > 1) {
	...
	public function getPFGModelRequest() {
		...
		$shop = new Shop($this->context->company->id);
	...
	public function getPFGModelFields() {
		...
		return [
			...
			[
				'title'    => $this->la('Lien Front Office'),
				'minWidth' => 250,
				'exWidth'  => 20,
				'dataIndx' => 'form_link',
				'align'    => 'left',
			...
			[
				'title'    => $this->la('Template Front Office'),
				'minWidth' => 100,
				'exWidth'  => 20,
				'dataIndx' => 'template',
				'align'    => 'left',
	...
	public function ajaxProcessAddPFG() {
		...
		$return = [
        	'success' => true,
            'message' => $this->la('Le Formulaire a été ajouté avec succès'),
        ];
	...
	public function ajaxProcessUpdatePFG() {
		...
		$return = [
        	'success' => true,
            'message' => $this->la('Le formulaire a été mis à jour avec succès'),
        ];
	...
	public function renderForm() {
		...
		$context->controller->addJS([
			__EPH_BASE_URI__ . '/js/tinymce/tinymce.min.js',
			_EPH_JS_DIR_ . 'tinymce.inc.js',
		]);

		$actions = [
			['value' => 'form', 'name' => $this->la('Send the form')],
			['value' => 'message', 'name' => $this->la('Send a specific message')],
			['value' => null, 'name' => $this->la('Do nothing')],
		];
		...
		$this->fields_form = [
			...
			'input'   => [
				...
				[
					'type'     => 'text',
					'label'    => $this->la('Nom du formulaire :'),
					'name'     => 'title',
					'lang'     => true,
					'required' => true,
				...
				[
                    'type'     => 'text',
                    'label'    => $this->la('Template Front Office'),
                    'name'     => 'template',
                    'required' => true,
                    'hint'     => $this->la('Fichier utilisé dans le repertoir du thème.'),
                ],
				[
					'type'    => 'select',
					'label'   => $this->la('Type of email for the sender'),
					'name'    => 'action_sender',
					'options' => [
						'query' => $actions,
						'id'    => 'value',
					...
					'desc'    => $this->la('What kind of email to send to the sender after the form has been successfully submitted.'),
					'class'   => 'fixed-width-xl',
				],
				[
					'type'     => 'text',
					'label'    => $this->la('Send form to :'),
					'name'     => 'send_mail_to',
					'required' => true,
					'class'    => 'fixed-width-xl',
					'desc'     => $this->la('List of the admins emails, separated by comma (",").<br />Ex: email@example.com,second@example.com'),
					'size'     => 50,
				],
				[
					'type'    => 'select',
					'label'   => $this->la('Type of email for the admin(s)'),
					'name'    => 'action_admin',
					'options' => [
						'query' => $actions,
						'id'    => 'value',
					...
					'desc'    => $this->la('What kind of email to send to the admin(s) after the form has been successfully submitted.'),
					'class'   => 'fixed-width-xl',
				],
				[
					'type'     => 'switch',
					'label'    => $this->la('Active:'),
					'name'     => 'active',
					'required' => false,
					'is_bool'  => true,
					'class'    => 't',
					'values'   => [
						...
						[
							'id'    => 'active_off',
							'value' => 0,
							'label' => $this->la('Désactivé'),
						],
					],
				],
				[
                    'type'     => 'switch',
                    'label'    => $this->la('Restreint à un seul envoi'),
				...
				[
					'type'     => 'select',
					'label'    => $this->la('Allow only to :'),
					'name'     => 'is_only_connected',
					'required' => false,
					'options'  => [
						'query' => [
							['value' => '0', 'name' => $this->la('Everybody')],
							['value' => '1', 'name' => $this->la('Connected user only.')],
						],
						'id'    => 'value',
						'name'  => 'name',
					],
				...
				[
					'type'     => 'text',
					'label'    => $this->la('Redirect URL :'),
					'name'     => 'unauth_redirect_url',
					'lang'     => true,
					'required' => false,
					'desc'     => $this->la('Redirect URL for non authenticated users. Leave empty to show a 404 page instead.'),
					'class'    => 'fixed-width-xxl redirect_url',
					'size'     => 98,
				],
				[
					'type'     => 'select',
					'label'    => $this->la('Accessible via :'),
					'name'     => 'accessible',
					'required' => false,
					'options'  => [
						'query' => [
							['value' => '1', 'name' => $this->la('Via URL only.')],
							['value' => '2', 'name' => $this->la('Via HOOK only.')],
							['value' => '0', 'name' => $this->la('Both URL and HOOK.')],
						],
						'id'    => 'value',
						'name'  => 'name',
					],
				...
				[
					'type'     => 'text',
					'label'    => $this->la('Default subject (sender) :'),
					'name'     => 'subject_sender',
					'lang'     => true,
					'required' => false,
					'desc'     => html_entity_decode($this->la('Default subject for the email sent to the sender if no subject field is defined.<br /><strong>You can use variables from the fields you configured in this form.</strong> Like this : <code>&lcub;$firstname&rcub;</code> for a field with the name "firstname".')),
					'class'    => 'fixed-width-xxl',
					'size'     => 98,
				],
				[
					'type'     => 'text',
					'label'    => $this->la('Default subject (admin) :'),
					'name'     => 'subject_admin',
					'lang'     => true,
					'required' => false,
					'desc'     => html_entity_decode($this->la('Default subject for the email sent to the admin if no subject field is defined.<br /><strong>You can use variables from the fields you configured in this form.</strong> Like this : <code>&lcub;$firstname&rcub;</code> for a field with the name "firstname".')),
					'class'    => 'fixed-width-xxl',
					'size'     => 98,
				],
				[
					'type'         => 'textarea',
					'label'        => $this->la('Header message :'),
					'name'         => 'header',
					'autoload_rte' => true,
					'lang'         => true,
					'required'     => false,
					'rows'         => '5',
					'cols'         => '48',
					'desc'         => $this->la('Message to display before the form.'),
				],
				[
					'type'         => 'textarea',
					'label'        => $this->la('Footer message :'),
					'name'         => 'footer',
					'autoload_rte' => true,
					'lang'         => true,
					'required'     => false,
					'rows'         => '5',
					'cols'         => '48',
					'desc'         => $this->la('Message to display after the form.'),
				],
				[
					'type'         => 'textarea',
					'label'        => $this->la('Success message :'),
					'name'         => 'success',
					'autoload_rte' => true,
					'lang'         => true,
					'required'     => false,
					'rows'         => '5',
					'cols'         => '48',
					'desc'         => html_entity_decode($this->la('Message to display after the form has been submitted. If your message starts with http, the user will be redirected to that address.<br /><strong>You can use variables from the fields you configured in this form.</strong> Like this : <code>&lcub;$firstname&rcub;</code> for a field with the name "firstname".')),
				],
				[
					'type'     => 'text',
					'label'    => $this->la('Send button value :'),
					'name'     => 'send_label',
					'lang'     => true,
					'required' => false,
					'desc'     => $this->la('The value to show in the "send" button.'),
					'class'    => 'fixed-width-xxl',
					'size'     => 98,
				],
				[
					'type'         => 'textarea',
					'label'        => $this->la('Message to send to the sender :'),
					'name'         => 'message_sender',
					'autoload_rte' => true,
					'lang'         => true,
					'required'     => false,
					'rows'         => '5',
					'cols'         => '48',
					'desc'         => html_entity_decode($this->la('Message to send to the sender.<br /><strong>You can use variables from the fields you configured in this form.</strong> Like this : <code>&lcub;$firstname&rcub;</code> for a field with the name "firstname".')),
					'class'        => 'message_senders',
				],
				[
					'type'         => 'textarea',
					'label'        => $this->la('Message to send to the admins :'),
					'name'         => 'message_admin',
					'autoload_rte' => true,
					'lang'         => true,
					'required'     => false,
					'rows'         => '5',
					'cols'         => '48',
					'desc'         => html_entity_decode($this->la('Message to send to the admins.<br /><strong>You can use variables from the fields you configured in this form.</strong> Like this : <code>&lcub;$firstname&rcub;</code> for a field with the name "firstname".')),
	...
	public function ajaxProcessManageFormulaire() {
		...
		$data = $this->createTemplate('controllers/formulaire_fields.tpl');
	...
	public function ajaxProcessViewSubmissions() {
		...
		$data = $this->createTemplate('controllers/formulaire_submissions.tpl');
	...
	public function buildPFGSubmissionScript($idFormulaire) {
		...
		$paragrid->showTitle = 1;
		$paragrid->title = '\'' . $this->la('Liste des entrées de formulaire') . '\'';
		$paragrid->fillHandle = '\'all\'';
	...
	public function setFieldsList($fields) {
		...
		$fieldList[] = [
				'title'      => $this->la('Date de démarrage'),
				'maxWidth'   => 200,
				'dataIndx'   => 'date_add',
				'align'      => 'center',
				'valign'     => 'center',
		...
		$paragrid->contextMenu = [
			'#grid_' . $controller_name => [
                ...
                return {
                    ...
                    "edit": {
                            name: \'' . $this->la('Modifier le champ ') . ' \'+rowData.name,
                            icon: "edit",
							visible: function(key, opt){
                        ...
                        "sep2": "---------",
                        "delete": {
                            name: \'' . $this->la('Supprimer le champ ') . ' \ : \'+rowData.name,
                            icon: "trash",
