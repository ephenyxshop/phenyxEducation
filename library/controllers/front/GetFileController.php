...
class GetFileControllerCore extends FrontController {
    ...
    public function init() {
        ...
        if (isset($this->context->employee) && $this->context->employee->isLoggedBack() && Tools::getValue('file')) {
            ...
            $productDeadline = strtotime($info['download_deadline']);

            if ($now > $productDeadline && $info['download_deadline'] != '0000-00-00 00:00:00') {
                $this->displayCustomError('The product deadline is in the past.');
            }

            $customerDeadline = strtotime($info['date_expiration']);

            if ($now > $customerDeadline && $info['date_expiration'] != '0000-00-00 00:00:00') {
    ...
    protected function displayCustomError($msg) {
        ...
        $customMessage = isset($translations[$msg]) ? html_entity_decode($translations[$msg], ENT_QUOTES, 'utf-8') : html_entity_decode($msg, ENT_QUOTES, 'utf-8');
        echo '
